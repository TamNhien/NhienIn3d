...
test("v3.13.0 dong bo version runtime browser CI va probe tools", () => {
  ...
  assert.equal(pkg.scripts["probe:keygen"], "node scripts/probe-keygen-v3250.mjs");
  assert.match(pkg.scripts["probe:fleet"], /probe-fleet-v3250\.ps1/);
  ...
  assert.match(read(".github/workflows/ci.yml"), /Browser E2E Admin HTTPS v3\.25\.0/);
});
