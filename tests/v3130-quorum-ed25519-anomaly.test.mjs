...
test("v3.13.0 dong bo version runtime browser CI va probe tools", () => {
  ...
  assert.equal(pkg.scripts["probe:keygen"], "node scripts/probe-keygen-v3290.mjs");
  ...
  assert.match(read(".github/workflows/ci.yml"), /Browser E2E Admin HTTPS v3\.29\.0/);
});
...
test("v3.13.0 giu 23 migration va khong ghi private key vao repo", () => {
  ...
  assert.ok(migrationDirs.length >= 23);
