...
test("v3.13.0 dong bo version runtime browser CI va probe tools", () => {
  ...
  assert.equal(pkg.scripts["probe:keygen"], "node scripts/probe-keygen-v3210.mjs");
  assert.match(pkg.scripts["probe:fleet"], /probe-fleet-v3210\.ps1/);
  ...
  assert.match(read(".github/workflows/ci.yml"), /Browser E2E Admin HTTPS v3\.21\.0/);
});
