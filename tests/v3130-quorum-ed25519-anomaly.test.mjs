...
test("v3.13.0 dong bo version runtime browser CI va probe tools", () => {
  ...
  assert.equal(read("VERSION").trim(), "3.13.0");
  assert.equal(pkg.version, "3.13.0");
  assert.equal(JSON.parse(read("apps/api/package.json")).version, "3.13.0");
  assert.equal(JSON.parse(read("apps/web/package.json")).version, "3.13.0");
  assert.equal(pkg.scripts["e2e:browser"], "node scripts/e2e-browser-v3130.mjs");
  assert.equal(pkg.scripts["probe:agent"], "node scripts/probe-agent-v3130.mjs");
  assert.equal(pkg.scripts["probe:keygen"], "node scripts/probe-keygen-v3130.mjs");
  assert.match(pkg.scripts["probe:fleet"], /probe-fleet-v3130\.ps1/);
  assert.equal(existsSync("scripts/e2e-runtime-v3130.ps1"), true);
  assert.match(read(".github/workflows/ci.yml"), /e2e-runtime-v3130\.ps1/);
  assert.match(read(".github/workflows/ci.yml"), /Browser E2E Admin HTTPS v3\.13\.0/);
});
...
test("v3.13.0 probe ho tro Ed25519 va giu HMAC nonce anti-replay", () => {
  ...
  const agent = read("scripts/probe-agent-v3130.mjs");
  const keygen = read("scripts/probe-keygen-v3130.mjs");
  const env = read(".env.example");
  assert.match(service, /SYSTEM_SLO_AGENT_PUBLIC_KEYS_JSON/);
  assert.match(service, /verifySignature\(null/);
  assert.match(service, /ED25519-v3130/);
...
test("v3.13.0 multi-region quorum va anomaly detection co config runtime", () => {
  ...
  assert.match(service, /trang_thai_ops_v3130/);
