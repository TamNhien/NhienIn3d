...
test("v3.6.3 healthcheck fix duoc giu khi nang patch tiep theo", () => {
  ...
  assert.match(root.version, /^(?:3\.6\.[34567]|3\.7\.[012]|3\.8\.0|3\.9\.0)$/);
  assert.equal(api.version, root.version);
  assert.equal(web.version, root.version);
  assert.match(root.scripts["e2e:browser"], /^node scripts\/e2e-browser-v(?:36[34567]|37[012]|380|390)\.mjs$/);
  ...
  assert.match(ci, /e2e-runtime-v(?:36[34567]|37[012]|380|390)\.ps1/);
  assert.match(ci, /Browser E2E Admin HTTPS v(?:3\.6\.[34567]|3\.7\.[012]|3\.8\.0|3\.9\.0)/);
});
