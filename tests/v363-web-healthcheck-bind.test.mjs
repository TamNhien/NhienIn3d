...
test("v3.6.3 healthcheck fix duoc giu khi nang patch tiep theo", () => {
  ...
  assert.match(root.version, /^3\.6\.[345]$/);
  assert.equal(api.version, root.version);
  assert.equal(web.version, root.version);
  assert.match(root.scripts["e2e:browser"], /^node scripts\/e2e-browser-v36[345]\.mjs$/);
  ...
  assert.match(ci, /e2e-runtime-v36[345]\.ps1/);
  assert.match(ci, /Browser E2E Admin HTTPS v3\.6\.[345]/);
});
