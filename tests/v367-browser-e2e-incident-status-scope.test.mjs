...
test("v3.6.7 CI va package dung runtime browser patch moi", () => {
  const pkg = JSON.parse(read("package.json"));
  const ci = read(".github/workflows/ci.yml");
  const runtime = read("scripts/e2e-runtime-v367.ps1");
  const browser = read("scripts/e2e-browser-v367.mjs");
  assert.equal(pkg.version, "3.6.7");
  assert.equal(pkg.scripts["e2e:browser"], "node scripts/e2e-browser-v367.mjs");
  assert.match(ci, /e2e-runtime-v367\.ps1/);
  assert.match(ci, /Browser E2E Admin HTTPS v3\.6\.7/);
