...
test("v3.11.0 dong bo Runtime Browser CI va health", () => {
  ...
  assert.equal(root.version, "3.11.0");
  assert.equal(root.scripts["e2e:browser"], "node scripts/e2e-browser-v3110.mjs");
  ...
  assert.match(health, /phien_ban: "v3\.11\.0"/);
  assert.match(main, /setVersion\("3\.11\.0"\)/);
