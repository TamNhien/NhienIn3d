...
test("v3.11.0 dong bo Runtime Browser CI va health", () => {
  ...
  assert.equal(root.version, "3.20.0");
  assert.equal(root.scripts["e2e:browser"], "node scripts/e2e-browser-v3200.mjs");
  assert.equal(existsSync("scripts/e2e-runtime-v3110.ps1"), true);
  assert.equal(existsSync("scripts/e2e-browser-v3110.mjs"), true);
  assert.match(ci, /e2e-runtime-v3200\.ps1/);
  assert.match(ci, /Browser E2E Admin HTTPS v3\.20\.0/);
  assert.match(health, /phien_ban: "v3\.20\.0"/);
  assert.match(main, /setVersion\("3\.20\.0"\)/);
