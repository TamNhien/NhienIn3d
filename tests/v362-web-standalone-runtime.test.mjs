...
test("v3.6.2 E2E va CI dung phien ban patch moi", () => {
  ...
  assert.match(root.scripts["e2e:browser"], /^node scripts\/e2e-browser-v(?:36[234567]|37[012]|380|390)\.mjs$/);
  assert.match(runtime, /không phải v3\.6\.2/);
  assert.match(browser, /không phải v3\.6\.2/);
  assert.match(ci, /e2e-runtime-v(?:36[234567]|37[012]|380|390)\.ps1/);
  assert.match(ci, /Browser E2E Admin HTTPS v(?:3\.6\.[234567]|3\.7\.[012]|3\.8\.0|3\.9\.0)/);
});
