...
test("v3.6.2 E2E va CI dung phien ban patch moi", () => {
  ...
  assert.match(root.scripts["e2e:browser"], /^node scripts\/e2e-browser-v36[23456]\.mjs$/);
  assert.match(runtime, /không phải v3\.6\.2/);
  assert.match(browser, /không phải v3\.6\.2/);
  assert.match(ci, /e2e-runtime-v36[23456]\.ps1/);
  assert.match(ci, /Browser E2E Admin HTTPS v3\.6\.[23456]/);
});
