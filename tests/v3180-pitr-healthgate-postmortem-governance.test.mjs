...
test("v3.20.0 dong bo current version scripts CI va grouped verify", () => {
  ...
  assert.match(pkg.scripts["recovery:pitr"], /recovery-pitr-drill-v3290\.ps1/);
  assert.match(read(".github/workflows/ci.yml"), /e2e-runtime-v3290\.ps1/);
  assert.match(read("apps/api/src/suc-khoe/suc-khoe.controller.ts"), /v3\.29\.0/);
  assert.match(read("apps/api/src/main.ts"), /setVersion\("3\.29\.0"\)/);
  const compose = read("docker-compose.yml");
  assert.match(compose, /SYSTEM_SLO_PROBE_DESIRED_TARGET_VERSION: \${SYSTEM_SLO_PROBE_DESIRED_TARGET_VERSION:-3\.29\.0}/);
