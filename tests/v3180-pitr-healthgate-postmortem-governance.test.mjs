...
test("v3.20.0 dong bo current version scripts CI va grouped verify", () => {
  ...
  assert.match(read("apps/api/src/suc-khoe/suc-khoe.controller.ts"), /v3\.24\.0/);
  assert.match(read("apps/api/src/main.ts"), /setVersion\("3\.24\.0"\)/);
  const compose = read("docker-compose.yml");
  assert.match(compose, /SYSTEM_SLO_PROBE_DESIRED_TARGET_VERSION: \${SYSTEM_SLO_PROBE_DESIRED_TARGET_VERSION:-3\.24\.0}/);
