...
test("v3.15.0 dong bo version probe enrollment E2E va verify gom lenh", () => {
  const pkg = JSON.parse(read("package.json"));
  assert.equal(read("VERSION").trim(), "3.15.0");
  assert.equal(pkg.version, "3.15.0");
  assert.equal(JSON.parse(read("apps/api/package.json")).version, "3.15.0");
  assert.equal(JSON.parse(read("apps/web/package.json")).version, "3.15.0");
  ...
  assert.match(pkg.scripts["probe:fleet"], /probe-fleet-v3150\.ps1/);
  assert.match(pkg.scripts["verify:v315"], /verify-v3150\.ps1/);
  assert.match(pkg.scripts["verify:full:v315"], /verify-v3150\.ps1 -Full/);
  assert.equal(pkg.scripts.verify, "npm run verify:v315");
  assert.equal(pkg.scripts["verify:full"], "npm run verify:full:v315");
  assert.equal(existsSync("scripts/e2e-runtime-v3150.ps1"), true);
  assert.equal(existsSync("scripts/probe-enroll-v3150.mjs"), true);
  assert.equal(existsSync("scripts/probe-enrollment-token-v3150.ps1"), true);
  assert.match(read(".github/workflows/ci.yml"), /e2e-runtime-v3150\.ps1/);
});
