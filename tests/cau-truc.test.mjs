...
test("version v3.19.0 dong bo root API va Web", () => {
  assert.equal(readFileSync("VERSION", "utf8").trim(), "3.19.0");
  assert.equal(docJson("package.json").version, "3.19.0");
  assert.equal(docJson("apps/api/package.json").version, "3.19.0");
  assert.equal(docJson("apps/web/package.json").version, "3.19.0");
});
...
test("v2.19.3 Docker HTTPS thong nhat khong con web va Caddy tranh cong 3000", () => {
  ...
  const ps1 = readFileSync("scripts/https-local.ps1", "utf8");
  const env = readFileSync(".env.example", "utf8");
  for (const src of [compose, composeHttps]) {
    assert.match(src, /NEXT_PUBLIC_API_URL:\s*\/api\/v1/);
    assert.match(src, /https:\s*[\s\S]*127\.0\.0\.1:\$\{WEB_PORT:-3000\}:443/);
    assert.match(src, /web:\s*[\s\S]*expose:\s*[\s\S]*- "3000"/);
    assert.doesNotMatch(src, /\$\{WEB_PORT:-3000\}:3000/);
  }
  assert.match(ps1, /docker compose up -d --build --remove-orphans/);
  assert.match(env, /WEB_PUBLIC_URL=https:\/\/localhost:3000/);
});

test("README co lich su phien ban tang dan den v3.19.0", () => {
  const readme = readFileSync("README.md", "utf8");
  const viTri = ["## v1.0.0", "## v1.0.1", "## v1.0.2", "## v1.0.3", "## v1.0.4", "## v1.0.5", "## v1.0.6", "## v1.0.7", "## v2.0.0", "## v2.1.0", "## v2.1.1", "## v2.2.0", "## v2.2.1", "## v2.3.0", "## v2.4.0", "## v2.4.1", "## v2.5.0", "## v2.6.0", "## v2.6.1", "## v2.7.0", "## v2.8.0", "## v2.8.1", "## v2.8.2", "## v2.8.3", "## v2.8.4", "## v2.8.5", "## v2.8.6", "## v2.8.7", "## v2.8.8", "## v2.8.9", "## v2.9.0", "## v2.9.1", "## v2.9.2", "## v2.9.3", "## v2.9.4", "## v2.9.5", "## v2.9.6", "## v2.9.7", "## v2.9.8", "## v2.9.9", "## v2.10.0", "## v2.10.1", "## v2.10.2", "## v2.11.0", "## v2.12.0", "## v2.12.1", "## v2.12.2", "## v2.12.3", "## v2.13.0", "## v2.14.0", "## v2.15.0", "## v2.15.1", "## v2.15.2", "## v2.15.3", "## v2.15.4", "## v2.15.5", "## v2.16.0", "## v2.17.0", "## v2.17.1", "## v2.18.0", "## v2.18.1", "## v2.18.2", "## v2.18.3", "## v2.19.0", "## v2.19.1", "## v2.19.2", "## v2.19.3", "## v3.0.0", "## v3.0.1", "## v3.1.0", "## v3.2.0", "## v3.2.1", "## v3.2.2", "## v3.2.3", "## v3.3.0", "## v3.3.1", "## v3.3.2", "## v3.4.0", "## v3.4.1", "## v3.5.0", "## v3.5.1", "## v3.5.2", "## v3.5.3", "## v3.5.4", "## v3.6.0", "## v3.6.1", "## v3.6.2", "## v3.6.3", "## v3.6.4", "## v3.6.5", "## v3.6.6", "## v3.6.7", "## v3.7.0", "## v3.7.1", "## v3.7.2", "## v3.8.0", "## v3.9.0", "## v3.10.0", "## v3.10.1", "## v3.10.2", "## v3.10.3", "## v3.10.4", "## v3.10.5", "## v3.11.0", "## v3.12.0", "## v3.13.0", "## v3.14.0", "## v3.15.0", "## v3.16.0", "## v3.17.0", "## v3.19.0"].map(x => readme.indexOf(x));
...
test("v2.17.1 form tao bien the co layout 4 cot responsive va field khong tran", () => {
  const css = readFileSync("apps/web/app/globals.css", "utf8");
...
test("v2.18.0 cau hinh email ton kho duoc dua vao env va Docker", () => {
  ...
  const readme = readFileSync("README.md", "utf8");
  assert.match(readme, /Phiên bản hiện tại: \*\*v3\.19\.0\*\*/u);
...
test("v2.18.1 co favicon va logo truoc NhienIn3d", () => {
  const layout = readFileSync("apps/web/app/layout.tsx", "utf8");
  const nav = readFileSync("apps/web/components/thanh-dieu-huong.tsx", "utf8");
  const footer = readFileSync("apps/web/components/chan-trang.tsx", "utf8");
  const css = readFileSync("apps/web/app/globals.css", "utf8");
  assert.equal(existsSync("apps/web/app/icon.svg"), true);
  assert.equal(existsSync("apps/web/public/brand/nhienin3d-logo.svg"), true);
  assert.match(layout, /icons:\s*\{/);
  assert.match(layout, /\/brand\/nhienin3d-logo\.svg/);
  assert.match(nav, /site-brand-logo/);
  assert.match(nav, /NhienIn3d - Trang chủ/u);
  assert.match(footer, /site-brand-logo/);
  assert.match(css, /v2\.18\.1 - logo thương hiệu/u);
  assert.match(css, /\.site-brand-logo\{/);
});

test("v2.18.2 Admin select vat lieu mau co che do toi va option ro", () => {
  const admin = readFileSync("apps/web/app/quan-tri/page.tsx", "utf8");
  const css = readFileSync("apps/web/app/globals.css", "utf8");
  assert.match(admin, /cine-select-readable-v2182/);
  assert.match(admin, /ma_vat_lieu/);
  assert.match(css, /color-scheme:dark/);
  assert.match(css, /cine-select-readable-v2182 option:checked/);
});


test("v2.18.3 co HTTPS local tin cay qua Caddy va script Windows", () => {
  assert.equal(existsSync("docker-compose.https.yml"), true);
  assert.equal(existsSync("infra/caddy/Caddyfile.local-https"), true);
  assert.equal(existsSync("scripts/https-local.ps1"), true);
  assert.equal(existsSync("scripts/https-local-bo-tin-cay.ps1"), true);
  const compose = readFileSync("docker-compose.https.yml", "utf8");
  const caddy = readFileSync("infra/caddy/Caddyfile.local-https", "utf8");
  const ps1 = readFileSync("scripts/https-local.ps1", "utf8");
  const pkg = docJson("package.json");
  assert.match(compose, /NEXT_PUBLIC_API_URL:\s*\/api\/v1/);
  assert.match(compose, /127\.0\.0\.1:\$\{WEB_PORT:-3000\}:443/);
  assert.match(caddy, /https:\/\/localhost/);
  assert.match(caddy, /tls internal/);
  assert.match(caddy, /@api path \/api\/\* \/tai-lieu\*/);
  assert.match(caddy, /reverse_proxy @api api:3001/);
  assert.match(caddy, /reverse_proxy web:3000/);
  assert.match(ps1, /certutil\.exe -user -addstore Root/);
  assert.match(ps1, /https:\/\/localhost:/);
  assert.match(pkg.scripts["dev:web:https"], /dev:https/);
});
