...
test("version v3.4.0 dong bo root API va Web", () => {
  assert.equal(readFileSync("VERSION", "utf8").trim(), "3.4.0");
  assert.equal(docJson("package.json").version, "3.4.0");
  assert.equal(docJson("apps/api/package.json").version, "3.4.0");
  assert.equal(docJson("apps/web/package.json").version, "3.4.0");
});
...
test("v2.18.0 cau hinh email ton kho duoc dua vao env va Docker", () => {
  ...
  const readme = readFileSync("README.md", "utf8");
  assert.match(readme, /Phiên bản hiện tại: \*\*v3\.4\.0\*\*/u);
