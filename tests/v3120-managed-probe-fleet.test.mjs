...
test("v3.12.0 dong bo version runtime browser probe fleet va CI", () => {
  ...
  assert.equal(pkg.scripts["probe:agent"], "node scripts/probe-agent-v3120.mjs");
  assert.match(pkg.scripts["probe:fleet"], /probe-fleet-v3120\.ps1/);
  assert.match(pkg.scripts["probe:fleet:once"], /probe-fleet-v3120\.ps1 -Once/);
  assert.equal(existsSync("scripts/probe-fleet-v3120.ps1"), true);
...
test("v3.12.0 managed probe fleet doi chieu keyring profile va PostgreSQL", () => {
  ...
  assert.match(controller, /trang_thai_ops_v3120/);
