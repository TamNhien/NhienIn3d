...
test("v3.12.0 dong bo version runtime browser probe fleet va CI", () => {
  ...
  assert.match(pkg.scripts["probe:fleet"], /probe-fleet-v3240\.ps1/);
  assert.match(pkg.scripts["probe:fleet:once"], /probe-fleet-v3240\.ps1 -Once/);
  assert.equal(existsSync("scripts/probe-fleet-v3140.ps1"), true);
  assert.match(ci, /e2e-runtime-v3240\.ps1/);
  assert.match(ci, /Browser E2E Admin HTTPS v3\.24\.0/);
});
