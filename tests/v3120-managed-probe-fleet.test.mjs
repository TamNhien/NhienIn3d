...
test("v3.12.0 dong bo version runtime browser probe fleet va CI", () => {
  ...
  assert.match(pkg.scripts["probe:fleet:once"], /probe-fleet-v3130\.ps1 -Once/);
  assert.equal(existsSync("scripts/probe-fleet-v3130.ps1"), true);
  assert.match(ci, /e2e-runtime-v3130\.ps1/);
  assert.match(ci, /Browser E2E Admin HTTPS v3\.13\.0/);
});
...
test("v3.12.0 managed probe fleet doi chieu keyring profile va PostgreSQL", () => {
  ...
  assert.match(controller, /trang_thai_ops_v3130/);
