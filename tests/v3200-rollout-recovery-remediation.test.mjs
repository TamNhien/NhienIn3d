...
test("v3.20.0 dong bo version, current scripts, CI va grouped verify", () => {
  ...
  assert.equal(pkg.scripts["probe:agent"], "node scripts/probe-agent-v3230.mjs");
  assert.match(pkg.scripts["recovery:drill"], /recovery-drill-v3230\.ps1/);
  assert.match(pkg.scripts["recovery:pitr"], /recovery-pitr-drill-v3230\.ps1/);
  assert.equal(pkg.scripts["recovery:evidence"], "node scripts/recovery-evidence-v3230.mjs");
  assert.equal(pkg.scripts["recovery:evidence:keygen"], "node scripts/recovery-evidence-keygen-v3230.mjs");
