...
test("v3.10.5 security checker scan installed node_modules thay vi resolve Prisma entrypoint", () => {
...
test("v3.10.5 scanner PASS voi mysql2 3.22.0 va FAIL voi nested mysql2 cu", () => {
  const base = join(tmpdir(), `nhienin3d-v3105-mysql2-${process.pid}-${Date.now()}`);
  try {
...
test("v3.10.5 CI verify va release dung security:mysql2 khong dung npm ls", () => {
...
test("v3.10.5 dong bo Runtime Browser CI Health OpenAPI", () => {
  const pkg = JSON.parse(read("package.json"));
  const ci = read(".github/workflows/ci.yml");
