...
test("v3.6.4 health-version fix duoc giu khi nang patch tiep theo", () => {
  ...
  assert.match(pkg.version, /^(?:3\.6\.[4567]|3\.7\.[012]|3\.8\.0|3\.9\.0)$/);
  assert.match(pkg.scripts["e2e:browser"], /^node scripts\/e2e-browser-v(?:36[4567]|37[012]|380|390)\.mjs$/);
  assert.match(ci, /e2e-runtime-v(?:36[4567]|37[012]|380|390)\.ps1/);
  assert.match(ci, /Browser E2E Admin HTTPS v(?:3\.6\.[4567]|3\.7\.[012]|3\.8\.0|3\.9\.0)/);
