...
test("v3.23.0 API receipt chain va recovery revoked-key fail-closed", () => {
  ...
  assert.match(controller, /ops_runtime\(\) \{ return this\.service\.trang_thai_ops_v3230\(\); \}/);
  assert.match(controller, /cap_nhat_probe_desired_state_v3230/);
  assert.match(controller, /approve_probe_rollout_v3230/);
  assert.match(controller, /reject_probe_rollout_v3230/);
  assert.match(controller, /cancel_probe_rollout_v3230/);
  ...
  assert.match(health, /phien_ban: "v3\.23\.0"/);
  assert.match(main, /setVersion\("3\.23\.0"\)/);
