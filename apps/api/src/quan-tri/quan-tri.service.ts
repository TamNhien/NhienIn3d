...
import { TrangThaiDonHang, TrangThaiGioHang, TrangThaiNhanVien, TrangThaiNguon, TrangThaiPhanCa, TrangThaiSanPham, TrangThaiThanhToan, TrangThaiYeuCauDoiTra, XuLyDoiTra, VaiTro, type Prisma } from "../generated/prisma/client.js";
...
export class QuanTriService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit() {
    ...
    this.logger.log(`Ops v3.29.0 schedulers: DLQ ${dlqPolicy.chu_ky_phut}m, metrics ${opsPolicy.refresh_phut}m, retention ${opsPolicy.retention_days}d.`);
  ...
  private probe_desired_state_env_v3170(): ProbeDesiredStateV3170 {
    ...
    return {
      revision: 0,
      target_version: (process.env.SYSTEM_SLO_PROBE_DESIRED_TARGET_VERSION || "3.29.0").trim(),
  ...
  async cap_nhat_probe_desired_state_v3240(actor: NguoiDungXacThuc, dto: { target_version: string; interval_seconds: number; rollout_percent: number; canary_agents: string[]; paused: boolean; note?: string }) {
    const result = await this.cap_nhat_probe_desired_state_v3230(actor, dto);
    return { ...result, rollout_approval: await this.lay_probe_rollout_proposal_v3240() };
  }

  async approve_probe_rollout_v3240(actor: NguoiDungXacThuc, proposalIdRaw: string, note?: string) { await this.approve_probe_rollout_v3230(actor, proposalIdRaw, note); return this.lay_probe_rollout_proposal_v3240(); }
  async reject_probe_rollout_v3240(actor: NguoiDungXacThuc, proposalIdRaw: string, note?: string) { await this.reject_probe_rollout_v3230(actor, proposalIdRaw, note); return this.lay_probe_rollout_proposal_v3240(); }
  async cancel_probe_rollout_v3240(actor: NguoiDungXacThuc, proposalIdRaw: string, note?: string) { await this.cancel_probe_rollout_v3230(actor, proposalIdRaw, note); return this.lay_probe_rollout_proposal_v3240(); }

  async lay_probe_rollout_proposal_v3250() {
    const base = await this.lay_probe_rollout_proposal_v3240();
    return { ...base, phien_ban: "3.25.0" };
  }

  async cap_nhat_probe_desired_state_v3250(actor: NguoiDungXacThuc, dto: { target_version: string; interval_seconds: number; rollout_percent: number; canary_agents: string[]; paused: boolean; note?: string }) {
    const result = await this.cap_nhat_probe_desired_state_v3240(actor, dto);
    return { ...result, rollout_approval: await this.lay_probe_rollout_proposal_v3250() };
  }

  async approve_probe_rollout_v3250(actor: NguoiDungXacThuc, proposalIdRaw: string, note?: string) { await this.approve_probe_rollout_v3240(actor, proposalIdRaw, note); return this.lay_probe_rollout_proposal_v3250(); }
  async reject_probe_rollout_v3250(actor: NguoiDungXacThuc, proposalIdRaw: string, note?: string) { await this.reject_probe_rollout_v3240(actor, proposalIdRaw, note); return this.lay_probe_rollout_proposal_v3250(); }
  async cancel_probe_rollout_v3250(actor: NguoiDungXacThuc, proposalIdRaw: string, note?: string) { await this.cancel_probe_rollout_v3240(actor, proposalIdRaw, note); return this.lay_probe_rollout_proposal_v3250(); }

  async lay_probe_rollout_proposal_v3260() { const base = await this.lay_probe_rollout_proposal_v3250(); return { ...base, phien_ban: "3.26.0" }; }
  async cap_nhat_probe_desired_state_v3260(actor: NguoiDungXacThuc, dto: { target_version: string; interval_seconds: number; rollout_percent: number; canary_agents: string[]; paused: boolean; note?: string }) { const result = await this.cap_nhat_probe_desired_state_v3250(actor, dto); return { ...result, rollout_approval: await this.lay_probe_rollout_proposal_v3260() }; }
  async approve_probe_rollout_v3260(actor: NguoiDungXacThuc, proposalIdRaw: string, note?: string) { await this.approve_probe_rollout_v3250(actor, proposalIdRaw, note); return this.lay_probe_rollout_proposal_v3260(); }
  ...
  private async recovery_readiness_v3240() {
    const base = await this.recovery_readiness_v3230();
    const backupDir = process.env.SYSTEM_BACKUP_DIR?.trim() || join(process.cwd(), "..", "..", "backups");
    let bundle: Record<string, unknown> | null = null;
    let source = "recovery-evidence-bundle-v3240.json";
    try { bundle = JSON.parse(await readFile(join(backupDir, source), "utf8")) as Record<string, unknown>; }
  ...
  private async recovery_readiness_v3250() {
    const base = await this.recovery_readiness_v3240();
    const backupDir = process.env.SYSTEM_BACKUP_DIR?.trim() || join(process.cwd(), "..", "..", "backups");
    let bundle: Record<string, unknown> | null = null;
    let source = "recovery-evidence-bundle-v3250.json";
    try { bundle = JSON.parse(await readFile(join(backupDir, source), "utf8")) as Record<string, unknown>; }
    catch { source = "recovery-evidence-bundle-v3240.json"; try { bundle = JSON.parse(await readFile(join(backupDir, source), "utf8")) as Record<string, unknown>; } catch {} }
    const verification = bundle ? this.verify_recovery_evidence_bundle_v3230(bundle) : null;
    const manifest = bundle?.manifest && typeof bundle.manifest === "object" && !Array.isArray(bundle.manifest) ? bundle.manifest as Record<string, unknown> : {};
    return { ...base, evidence_bundle_file: `backups/${source}`, evidence_bundle_version: typeof manifest.version === "string" ? manifest.version : base.evidence_bundle_version, evidence_current_version: manifest.version === "3.25.0", evidence_verification: verification || base.evidence_verification, evidence_key_revoked: verification?.key_revoked ?? base.evidence_key_revoked, evidence_revocation_configured: verification?.revocation_configured ?? base.evidence_revocation_configured, evidence_key_trusted: verification?.key_trusted ?? base.evidence_key_trusted, evidence_trust_source: verification?.trust_source ?? base.evidence_trust_source, audit_bundle_ready: !!bundle && verification?.overall_verified === true, audit_bundle_revocation_fail_closed: true as const, private_key_exposed: false as const, secret_values_exposed: false as const };
  }

  async verify_recovery_evidence_v3250() {
    const backupDir = process.env.SYSTEM_BACKUP_DIR?.trim() || join(process.cwd(), "..", "..", "backups");
    const path = join(backupDir, "recovery-evidence-bundle-v3250.json");
    let parsed: Record<string, unknown>;
    try { parsed = JSON.parse(await readFile(path, "utf8")) as Record<string, unknown>; } catch { throw new NotFoundException("Chưa có recovery evidence bundle v3.25.0; chạy npm run recovery:evidence trước"); }
    const verification = this.verify_recovery_evidence_bundle_v3230(parsed);
    return { phien_ban: "3.25.0", file: "backups/recovery-evidence-bundle-v3250.json", ...verification, secret_values_exposed: false as const };
  }

  async xuat_recovery_evidence_bundle_v3250() {
    const backupDir = process.env.SYSTEM_BACKUP_DIR?.trim() || join(process.cwd(), "..", "..", "backups");
    const path = join(backupDir, "recovery-evidence-bundle-v3250.json");
    let raw: string; let parsed: Record<string, unknown>;
    try { raw = await readFile(path, "utf8"); parsed = JSON.parse(raw) as Record<string, unknown>; } catch { throw new NotFoundException("Chưa có recovery evidence bundle v3.25.0; chạy npm run recovery:evidence trước"); }
    const verification = this.verify_recovery_evidence_bundle_v3230(parsed);
    if (!verification.overall_verified) throw new ConflictException(`Recovery evidence bundle không qua trusted + revocation verification (${verification.reason}); không cho phép export audit bundle`);
    return { ten_file: `recovery-evidence-audit-bundle-v3.25.0-${new Date().toISOString().slice(0, 10)}.json`, mime_type: "application/json", base64: Buffer.from(raw, "utf8").toString("base64"), manifest: parsed.manifest || {}, integrity: parsed.integrity || {}, signature: parsed.signature || {}, verification, secret_values_exposed: false as const };
  }

  private async recovery_readiness_v3260() {
    const base = await this.recovery_readiness_v3250();
    const backupDir = process.env.SYSTEM_BACKUP_DIR?.trim() || join(process.cwd(), "..", "..", "backups");
    let bundle: Record<string, unknown> | null = null;
    let source = "recovery-evidence-bundle-v3260.json";
    try { bundle = JSON.parse(await readFile(join(backupDir, source), "utf8")) as Record<string, unknown>; }
    catch { source = "recovery-evidence-bundle-v3250.json"; try { bundle = JSON.parse(await readFile(join(backupDir, source), "utf8")) as Record<string, unknown>; } catch {} }
    const verification = bundle ? this.verify_recovery_evidence_bundle_v3230(bundle) : null;
    const manifest = bundle?.manifest && typeof bundle.manifest === "object" && !Array.isArray(bundle.manifest) ? bundle.manifest as Record<string, unknown> : {};
    return { ...base, evidence_bundle_file: `backups/${source}`, evidence_bundle_version: typeof manifest.version === "string" ? manifest.version : base.evidence_bundle_version, evidence_current_version: manifest.version === "3.26.0", evidence_verification: verification || base.evidence_verification, evidence_key_revoked: verification?.key_revoked ?? base.evidence_key_revoked, evidence_revocation_configured: verification?.revocation_configured ?? base.evidence_revocation_configured, evidence_key_trusted: verification?.key_trusted ?? base.evidence_key_trusted, evidence_trust_source: verification?.trust_source ?? base.evidence_trust_source, audit_bundle_ready: !!bundle && verification?.overall_verified === true, audit_bundle_revocation_fail_closed: true as const, private_key_exposed: false as const, secret_values_exposed: false as const };
  }

  async verify_recovery_evidence_v3260() {
    const backupDir = process.env.SYSTEM_BACKUP_DIR?.trim() || join(process.cwd(), "..", "..", "backups");
    const path = join(backupDir, "recovery-evidence-bundle-v3260.json");
    let parsed: Record<string, unknown>;
    try { parsed = JSON.parse(await readFile(path, "utf8")) as Record<string, unknown>; } catch { throw new NotFoundException("Chưa có recovery evidence bundle v3.26.0; chạy npm run recovery:evidence trước"); }
    const verification = this.verify_recovery_evidence_bundle_v3230(parsed);
    return { phien_ban: "3.26.0", file: "backups/recovery-evidence-bundle-v3260.json", ...verification, secret_values_exposed: false as const };
  }

  async xuat_recovery_evidence_bundle_v3260() {
    const backupDir = process.env.SYSTEM_BACKUP_DIR?.trim() || join(process.cwd(), "..", "..", "backups");
    const path = join(backupDir, "recovery-evidence-bundle-v3260.json");
    let raw: string; let parsed: Record<string, unknown>;
    try { raw = await readFile(path, "utf8"); parsed = JSON.parse(raw) as Record<string, unknown>; } catch { throw new NotFoundException("Chưa có recovery evidence bundle v3.26.0; chạy npm run recovery:evidence trước"); }
    const verification = this.verify_recovery_evidence_bundle_v3230(parsed);
    if (!verification.overall_verified) throw new ConflictException(`Recovery evidence bundle không qua trusted + revocation verification (${verification.reason}); không cho phép export audit bundle`);
  ...
  private async recovery_readiness_v3290() {
    const base = await this.recovery_readiness_v3280();
    const backupDir = process.env.SYSTEM_BACKUP_DIR?.trim() || join(process.cwd(), "..", "..", "backups");
    let bundle: Record<string, unknown> | null = null;
    let source = "recovery-evidence-bundle-v3290.json";
    try { bundle = JSON.parse(await readFile(join(backupDir, source), "utf8")) as Record<string, unknown>; }
    catch { source = "recovery-evidence-bundle-v3280.json"; try { bundle = JSON.parse(await readFile(join(backupDir, source), "utf8")) as Record<string, unknown>; } catch {} }
    const verification = bundle ? this.verify_recovery_evidence_bundle_v3230(bundle) : null;
    const manifest = bundle?.manifest && typeof bundle.manifest === "object" && !Array.isArray(bundle.manifest) ? bundle.manifest as Record<string, unknown> : {};
    return { ...base, evidence_bundle_file: `backups/${source}`, evidence_bundle_version: typeof manifest.version === "string" ? manifest.version : base.evidence_bundle_version, evidence_current_version: manifest.version === "3.29.0", evidence_verification: verification || base.evidence_verification, audit_bundle_ready: !!bundle && verification?.overall_verified === true, private_key_exposed: false as const, secret_values_exposed: false as const };
  }

  async verify_recovery_evidence_v3290() {
    const backupDir = process.env.SYSTEM_BACKUP_DIR?.trim() || join(process.cwd(), "..", "..", "backups");
    const path = join(backupDir, "recovery-evidence-bundle-v3290.json");
    let parsed: Record<string, unknown>;
    try { parsed = JSON.parse(await readFile(path, "utf8")) as Record<string, unknown>; } catch { throw new NotFoundException("Chưa có recovery evidence bundle v3.29.0; chạy npm run recovery:evidence trước"); }
    const verification = this.verify_recovery_evidence_bundle_v3230(parsed);
    return { phien_ban: "3.29.0", file: "backups/recovery-evidence-bundle-v3290.json", ...verification, secret_values_exposed: false as const };
  }

  async xuat_recovery_evidence_bundle_v3290() {
    const backupDir = process.env.SYSTEM_BACKUP_DIR?.trim() || join(process.cwd(), "..", "..", "backups");
    const path = join(backupDir, "recovery-evidence-bundle-v3290.json");
    let raw: string; let parsed: Record<string, unknown>;
    try { raw = await readFile(path, "utf8"); parsed = JSON.parse(raw) as Record<string, unknown>; } catch { throw new NotFoundException("Chưa có recovery evidence bundle v3.29.0; chạy npm run recovery:evidence trước"); }
    const verification = this.verify_recovery_evidence_bundle_v3230(parsed);
    if (!verification.overall_verified) throw new ConflictException(`Recovery evidence bundle không qua trusted + revocation verification (${verification.reason}); không cho phép export audit bundle`);
    return { ten_file: `recovery-evidence-audit-bundle-v3.29.0-${new Date().toISOString().slice(0, 10)}.json`, mime_type: "application/json", base64: Buffer.from(raw, "utf8").toString("base64"), manifest: parsed.manifest || {}, integrity: parsed.integrity || {}, signature: parsed.signature || {}, verification, secret_values_exposed: false as const };
  }
  ...
  async trang_thai_ops_v3250() {
    const [base, rolloutApproval, recovery, remediation] = await Promise.all([this.trang_thai_ops_v3240(), this.lay_probe_rollout_proposal_v3250(), this.recovery_readiness_v3250(), this.postmortem_remediation_v3210()]);
    const fleet = base.probe_fleet as Record<string, unknown> | undefined;
    return {
      ...base,
      phien_ban: "3.25.0",
      probe_fleet: fleet ? { ...fleet, phien_ban: "3.25.0" } : base.probe_fleet,
      multi_region_quorum: { ...base.multi_region_quorum, phien_ban: "3.25.0" },
      ...
      admin_business_safety: {
        ...
        inventory_demand_forecast: true,
        active_cart_pressure_read_only: true,
        cart_demand_is_reservation: false,
        prepaid_cancel_refund_queue: true,
        refund_gateway_automation: false,
        manual_refund_confirmation_required: true,
        no_database_migration: true,
      },
    };
  }

  async trang_thai_ops_v3260() {
    const [base, rolloutApproval, recovery, remediation] = await Promise.all([this.trang_thai_ops_v3250(), this.lay_probe_rollout_proposal_v3260(), this.recovery_readiness_v3260(), this.postmortem_remediation_v3210()]);
    const fleet = base.probe_fleet as Record<string, unknown> | undefined;
    const refundSlaRaw = Number(process.env.SYSTEM_REFUND_RECONCILIATION_SLA_HOURS || 24);
    const refund_sla_hours = Math.max(1, Math.min(168, Number.isFinite(refundSlaRaw) ? Math.round(refundSlaRaw) : 24));
    return {
      ...base,
      phien_ban: "3.26.0",
      probe_fleet: fleet ? { ...fleet, phien_ban: "3.26.0" } : base.probe_fleet,
      multi_region_quorum: { ...base.multi_region_quorum, phien_ban: "3.26.0" },
      rollout_approval: rolloutApproval,
      database_recovery: recovery,
      remediation_backlog: remediation,
      admin_business_safety: {
        ...base.admin_business_safety,
        inventory_cycle_count_preview: true,
        inventory_cycle_count_optimistic_lock: true,
        supplier_grouped_replenishment: true,
  ...
  async trang_thai_ops_v3290() {
    const base = await this.trang_thai_ops_v3280();
    return { ...base, phien_ban: "3.29.0", probe_fleet: base.probe_fleet ? { ...(base.probe_fleet as Record<string, unknown>), phien_ban: "3.29.0" } : base.probe_fleet, multi_region_quorum: { ...base.multi_region_quorum, phien_ban: "3.29.0" }, rollout_approval: await this.lay_probe_rollout_proposal_v3290(), database_recovery: await this.recovery_readiness_v3290(), admin_business_safety: { ...base.admin_business_safety, return_rma_workflow: true, partial_refund_ledger: true, return_stock_atomic_reconciliation: true, gross_margin_dashboard: true, sale_time_cost_snapshot: true, refund_gateway_automation: false, database_migration_count: 25 } };
  }
  ...
  async goi_y_nhap_kho_v3240() {
    const ds = await this.db.bienTheSanPham.findMany({
      include: {
        san_pham: { select: { ma_san_pham: true, ten_san_pham: true, trang_thai: true, nguon_tham_khao: true } },
        vat_lieu: { select: { ten_vat_lieu: true } },
        mau_sac: { select: { ten_mau: true } },
        chi_tiet_nhap_kho: {
          orderBy: { ngay_tao: "desc" },
          take: 1,
          include: { phieu_nhap: { include: { nha_cung_cap_ref: { select: { id: true, ma_nha_cung_cap: true, ten_nha_cung_cap: true, dang_hoat_dong: true } } } } },
        },
      },
      orderBy: { ma_bien_the: "asc" },
    });
    const items = ds
      .filter(x => x.dang_hien_thi && x.ton_toi_thieu > 0 && x.so_luong_ton < x.ton_toi_thieu && !x.san_pham.nguon_tham_khao?.startsWith("__ADMIN_DELETED__:") && x.san_pham.trang_thai !== TrangThaiSanPham.NGUNG_BAN)
      .map(x => {
        const muc_tieu = x.ton_toi_da > x.ton_toi_thieu ? x.ton_toi_da : x.ton_toi_thieu;
        const so_luong_de_xuat = Math.max(0, muc_tieu - x.so_luong_ton);
        const gan_nhat = x.chi_tiet_nhap_kho[0]?.phieu_nhap;
        const ref = gan_nhat?.nha_cung_cap_ref || null;
        const ten_legacy = !ref && gan_nhat?.nha_cung_cap ? gan_nhat.nha_cung_cap : null;
        return {
          bien_the_id: x.id,
          ma_bien_the: x.ma_bien_the,
          ma_san_pham: x.san_pham.ma_san_pham,
          ten_san_pham: x.san_pham.ten_san_pham,
          vat_lieu: x.vat_lieu?.ten_vat_lieu || "Mặc định",
          mau_sac: x.mau_sac?.ten_mau || "Mặc định",
          ton_hien_tai: x.so_luong_ton,
          ton_toi_thieu: x.ton_toi_thieu,
          ton_toi_da: x.ton_toi_da,
          muc_tieu_sau_nhap: muc_tieu,
          so_luong_de_xuat,
          muc_do: x.so_luong_ton <= 0 ? "HET_HANG" : "DUOI_DINH_MUC",
          nha_cung_cap: ref ? { id: ref.id, ma_nha_cung_cap: ref.ma_nha_cung_cap, ten_nha_cung_cap: ref.ten_nha_cung_cap, dang_hoat_dong: ref.dang_hoat_dong } : ten_legacy ? { id: null, ma_nha_cung_cap: null, ten_nha_cung_cap: ten_legacy, dang_hoat_dong: null } : null,
          nguon_nha_cung_cap: ref ? "PHIEU_NHAP_GAN_NHAT" : ten_legacy ? "PHIEU_NHAP_LEGACY" : "CHUA_CO_LICH_SU_NHAP",
          phieu_nhap_gan_nhat: gan_nhat ? { ma_phieu: gan_nhat.ma_phieu, ngay_tao: gan_nhat.ngay_tao.toISOString() } : null,
        };
      })
      .sort((a, b) => (a.ton_hien_tai - b.ton_hien_tai) || a.ma_bien_the.localeCompare(b.ma_bien_the, "vi"));

    const nhom = new Map<string, { nha_cung_cap: (typeof items)[number]["nha_cung_cap"]; so_bien_the: number; so_luong_de_xuat: number }>();
    for (const item of items) {
      const key = item.nha_cung_cap?.id || item.nha_cung_cap?.ten_nha_cung_cap || "__KHONG_GAN__";
      const hien = nhom.get(key) || { nha_cung_cap: item.nha_cung_cap, so_bien_the: 0, so_luong_de_xuat: 0 };
      hien.so_bien_the += 1;
      hien.so_luong_de_xuat += item.so_luong_de_xuat;
      nhom.set(key, hien);
    }
    ...
    return {
      ...
      het_hang: items.filter(x => x.muc_do === "HET_HANG").length,
      khong_co_nha_cung_cap: items.filter(x => !x.nha_cung_cap).length,
      nha_cung_cap_ngung_hoat_dong: items.filter(x => x.nha_cung_cap?.dang_hoat_dong === false).length,
      theo_nha_cung_cap,
      items,
      supplier_inference: "LATEST_RECEIPT" as const,
      write_operation: false as const,
    };
  }

  async xuat_goi_y_nhap_kho_excel_v3240() {
    const plan = await this.goi_y_nhap_kho_v3240();
    const rows: unknown[][] = [["Mã sản phẩm", "Sản phẩm", "Mã biến thể", "Vật liệu", "Màu", "Tồn hiện tại", "Tồn tối thiểu", "Tồn tối đa", "Mục tiêu sau nhập", "SL đề xuất", "Mức độ", "Mã NCC", "Nhà cung cấp", "NCC hoạt động", "Nguồn NCC", "Phiếu nhập gần nhất"]];
  ...
  async goi_y_nhap_kho_v3250() {
    const history_days = 30;
    const parsedForecast = Number(process.env.SYSTEM_INVENTORY_FORECAST_DAYS || 14);
    const forecast_days = Math.max(1, Math.min(90, Number.isFinite(parsedForecast) ? Math.round(parsedForecast) : 14));
    const now = new Date();
    const since = new Date(now.getTime() - history_days * 86_400_000);
    const [ds, orderLines, cartLines] = await Promise.all([
      this.db.bienTheSanPham.findMany({
        include: {
          san_pham: { select: { ma_san_pham: true, ten_san_pham: true, trang_thai: true, nguon_tham_khao: true } },
          vat_lieu: { select: { ten_vat_lieu: true } },
          mau_sac: { select: { ten_mau: true } },
          chi_tiet_nhap_kho: { orderBy: { ngay_tao: "desc" }, take: 1, include: { phieu_nhap: { include: { nha_cung_cap_ref: { select: { id: true, ma_nha_cung_cap: true, ten_nha_cung_cap: true, dang_hoat_dong: true } } } } } },
        },
        orderBy: { ma_bien_the: "asc" },
      }),
      this.db.chiTietDonHang.findMany({
        where: { don_hang: { ngay_tao: { gte: since }, trang_thai: { not: TrangThaiDonHang.DA_HUY } } },
        select: { so_luong: true, tuy_chon: true },
        take: 20_000,
      }),
      this.db.chiTietGioHang.findMany({
        where: { gio_hang: { trang_thai: TrangThaiGioHang.DANG_MO, ngay_het_han: { gt: now } } },
        select: { bien_the_id: true, so_luong: true },
        take: 20_000,
      }),
    ]);
    const soldByVariant = new Map<string, number>();
    let legacy_unmapped_sales_lines = 0;
    for (const line of orderLines) {
      const opt = line.tuy_chon && typeof line.tuy_chon === "object" && !Array.isArray(line.tuy_chon) ? line.tuy_chon as Record<string, unknown> : {};
      const code = typeof opt.ma_bien_the === "string" ? opt.ma_bien_the.trim() : "";
      if (!code) { legacy_unmapped_sales_lines += 1; continue; }
      soldByVariant.set(code, (soldByVariant.get(code) || 0) + line.so_luong);
    }
    const cartByVariant = new Map<string, number>();
    for (const line of cartLines) cartByVariant.set(line.bien_the_id, (cartByVariant.get(line.bien_the_id) || 0) + line.so_luong);

    const items = ds
      .filter(x => x.dang_hien_thi && !x.san_pham.nguon_tham_khao?.startsWith("__ADMIN_DELETED__:") && x.san_pham.trang_thai !== TrangThaiSanPham.NGUNG_BAN)
      .map(x => {
        const sold_30d = soldByVariant.get(x.ma_bien_the) || 0;
        const avg_daily_sales = sold_30d / history_days;
        const forecast_demand = Math.ceil(avg_daily_sales * forecast_days);
        const active_cart_demand = cartByVariant.get(x.id) || 0;
        const muc_tieu_dinh_muc = x.ton_toi_da > x.ton_toi_thieu ? x.ton_toi_da : x.ton_toi_thieu;
        const de_xuat_theo_dinh_muc = x.ton_toi_thieu > 0 && x.so_luong_ton < x.ton_toi_thieu ? Math.max(0, muc_tieu_dinh_muc - x.so_luong_ton) : 0;
        const muc_tieu_du_bao = Math.max(muc_tieu_dinh_muc, forecast_demand);
        const de_xuat_theo_du_bao = forecast_demand > x.so_luong_ton ? Math.max(0, muc_tieu_du_bao - x.so_luong_ton) : 0;
        const so_luong_de_xuat = Math.max(de_xuat_theo_dinh_muc, de_xuat_theo_du_bao);
        const days_of_cover = avg_daily_sales > 0 ? Math.round((x.so_luong_ton / avg_daily_sales) * 10) / 10 : null;
        const cart_pressure = active_cart_demand >= Math.max(1, x.so_luong_ton);
        const muc_do = x.so_luong_ton <= 0 ? "HET_HANG" : de_xuat_theo_dinh_muc > 0 ? "DUOI_DINH_MUC" : de_xuat_theo_du_bao > 0 ? "RUI_RO_DU_BAO" : cart_pressure ? "AP_LUC_GIO_HANG" : "ON_DINH";
        const gan_nhat = x.chi_tiet_nhap_kho[0]?.phieu_nhap;
        const ref = gan_nhat?.nha_cung_cap_ref || null;
        const ten_legacy = !ref && gan_nhat?.nha_cung_cap ? gan_nhat.nha_cung_cap : null;
        return {
          bien_the_id: x.id, ma_bien_the: x.ma_bien_the, ma_san_pham: x.san_pham.ma_san_pham, ten_san_pham: x.san_pham.ten_san_pham,
          vat_lieu: x.vat_lieu?.ten_vat_lieu || "Mặc định", mau_sac: x.mau_sac?.ten_mau || "Mặc định",
          ton_hien_tai: x.so_luong_ton, ton_toi_thieu: x.ton_toi_thieu, ton_toi_da: x.ton_toi_da,
          muc_tieu_sau_nhap: Math.max(muc_tieu_dinh_muc, forecast_demand), so_luong_de_xuat, de_xuat_theo_dinh_muc, de_xuat_theo_du_bao,
          ban_30_ngay: sold_30d, ban_trung_binh_ngay: Math.round(avg_daily_sales * 100) / 100, forecast_days, forecast_demand, days_of_cover,
          active_cart_demand, cart_pressure, cart_demand_is_reservation: false as const, muc_do,
          nha_cung_cap: ref ? { id: ref.id, ma_nha_cung_cap: ref.ma_nha_cung_cap, ten_nha_cung_cap: ref.ten_nha_cung_cap, dang_hoat_dong: ref.dang_hoat_dong } : ten_legacy ? { id: null, ma_nha_cung_cap: null, ten_nha_cung_cap: ten_legacy, dang_hoat_dong: null } : null,
          nguon_nha_cung_cap: ref ? "PHIEU_NHAP_GAN_NHAT" : ten_legacy ? "PHIEU_NHAP_LEGACY" : "CHUA_CO_LICH_SU_NHAP",
          phieu_nhap_gan_nhat: gan_nhat ? { ma_phieu: gan_nhat.ma_phieu, ngay_tao: gan_nhat.ngay_tao.toISOString() } : null,
        };
      })
      .filter(x => x.so_luong_de_xuat > 0 || x.cart_pressure)
      .sort((a, b) => {
        const rank = (v: string) => ({ HET_HANG: 0, DUOI_DINH_MUC: 1, RUI_RO_DU_BAO: 2, AP_LUC_GIO_HANG: 3, ON_DINH: 4 } as Record<string, number>)[v] ?? 9;
        return rank(a.muc_do) - rank(b.muc_do) || b.so_luong_de_xuat - a.so_luong_de_xuat || a.ma_bien_the.localeCompare(b.ma_bien_the, "vi");
      });
    const nhom = new Map<string, { nha_cung_cap: (typeof items)[number]["nha_cung_cap"]; so_bien_the: number; so_luong_de_xuat: number }>();
    for (const item of items.filter(x => x.so_luong_de_xuat > 0)) {
      const key = item.nha_cung_cap?.id || item.nha_cung_cap?.ten_nha_cung_cap || "__KHONG_GAN__";
      const hien = nhom.get(key) || { nha_cung_cap: item.nha_cung_cap, so_bien_the: 0, so_luong_de_xuat: 0 };
      hien.so_bien_the += 1; hien.so_luong_de_xuat += item.so_luong_de_xuat; nhom.set(key, hien);
    }
    return {
      phien_ban: "3.25.0", tao_luc: now.toISOString(), history_days, forecast_days, forecast_basis: "NON_CANCELLED_ORDER_LINES_30D" as const,
      tong_bien_the_can_nhap: items.filter(x => x.so_luong_de_xuat > 0).length, tong_so_luong_de_xuat: items.reduce((sum, x) => sum + x.so_luong_de_xuat, 0),
      het_hang: items.filter(x => x.muc_do === "HET_HANG").length, rui_ro_du_bao: items.filter(x => x.muc_do === "RUI_RO_DU_BAO").length,
      ap_luc_gio_hang: items.filter(x => x.cart_pressure).length, tong_nhu_cau_gio_hang_mo: cartLines.reduce((sum, x) => sum + x.so_luong, 0),
      legacy_unmapped_sales_lines, khong_co_nha_cung_cap: items.filter(x => x.so_luong_de_xuat > 0 && !x.nha_cung_cap).length,
      nha_cung_cap_ngung_hoat_dong: items.filter(x => x.so_luong_de_xuat > 0 && x.nha_cung_cap?.dang_hoat_dong === false).length,
      theo_nha_cung_cap: [...nhom.values()].sort((a, b) => b.so_luong_de_xuat - a.so_luong_de_xuat), items, supplier_inference: "LATEST_RECEIPT" as const,
      write_operation: false as const, auto_purchase_order: false as const, cart_demand_is_reservation: false as const,
    };
  }

  async xuat_goi_y_nhap_kho_excel_v3250() {
    const plan = await this.goi_y_nhap_kho_v3250();
    const rows: unknown[][] = [["Mã SP", "Sản phẩm", "Mã biến thể", "Vật liệu", "Màu", "Tồn hiện tại", "Tồn min", "Tồn max", "Bán 30 ngày", "Bán TB/ngày", `Nhu cầu dự báo ${plan.forecast_days} ngày`, "Ngày phủ tồn", "Nhu cầu giỏ mở (không giữ chỗ)", "Đề xuất theo định mức", "Đề xuất theo dự báo", "SL đề xuất", "Mức độ", "Mã NCC", "Nhà cung cấp", "Nguồn NCC", "Phiếu nhập gần nhất"]];
    for (const item of plan.items) rows.push([item.ma_san_pham, item.ten_san_pham, item.ma_bien_the, item.vat_lieu, item.mau_sac, item.ton_hien_tai, item.ton_toi_thieu, item.ton_toi_da, item.ban_30_ngay, item.ban_trung_binh_ngay, item.forecast_demand, item.days_of_cover ?? "", item.active_cart_demand, item.de_xuat_theo_dinh_muc, item.de_xuat_theo_du_bao, item.so_luong_de_xuat, item.muc_do, item.nha_cung_cap?.ma_nha_cung_cap || "", item.nha_cung_cap?.ten_nha_cung_cap || "", item.nguon_nha_cung_cap, item.phieu_nhap_gan_nhat?.ma_phieu || ""]);
    const buffer = this.tao_xlsx(rows, "Kế hoạch nhập v3.25");
    const ngay = new Date(Date.now() + 7 * 60 * 60 * 1000).toISOString().slice(0, 10);
    return { ten_file: `ke-hoach-nhap-kho-v3.25.0_${ngay}.xlsx`, mime_type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", base64: buffer.toString("base64"), tong_bien_the_can_nhap: plan.tong_bien_the_can_nhap, tong_so_luong_de_xuat: plan.tong_so_luong_de_xuat, forecast_days: plan.forecast_days };
  }

  async goi_y_nhap_kho_v3260() {
    const base = await this.goi_y_nhap_kho_v3250();
    const groups = base.theo_nha_cung_cap.map(group => {
      const active = group.nha_cung_cap?.dang_hoat_dong === true;
      const missing = !group.nha_cung_cap;
      return {
        ...group,
        trang_thai_nhom: missing ? "CHUA_GAN_NCC" as const : active ? "SAN_SANG" as const : "NCC_NGUNG_HOAT_DONG" as const,
        ready_to_send: active,
        manual_supplier_resolution_required: missing || !active,
      };
    });
    return {
      ...base,
      phien_ban: "3.26.0",
      theo_nha_cung_cap: groups,
      tong_nhom_nha_cung_cap: groups.length,
      nhom_san_sang: groups.filter(x => x.ready_to_send).length,
      nhom_can_xu_ly_thu_cong: groups.filter(x => x.manual_supplier_resolution_required).length,
      grouped_supplier_procurement_plan: true as const,
      write_operation: false as const,
      auto_purchase_order: false as const,
    };
  }

  async xuat_goi_y_nhap_kho_excel_v3260() {
    const plan = await this.goi_y_nhap_kho_v3260();
    const rows: unknown[][] = [["Mã SP", "Sản phẩm", "Mã biến thể", "Vật liệu", "Màu", "Tồn hiện tại", "Tồn min", "Tồn max", "Bán 30 ngày", "Bán TB/ngày", `Nhu cầu dự báo ${plan.forecast_days} ngày`, "Ngày phủ tồn", "Nhu cầu giỏ mở (không giữ chỗ)", "Đề xuất theo định mức", "Đề xuất theo dự báo", "SL đề xuất", "Mức độ", "Mã NCC", "Nhà cung cấp", "Trạng thái NCC", "Nguồn NCC", "Phiếu nhập gần nhất"]];
    for (const item of plan.items) rows.push([item.ma_san_pham, item.ten_san_pham, item.ma_bien_the, item.vat_lieu, item.mau_sac, item.ton_hien_tai, item.ton_toi_thieu, item.ton_toi_da, item.ban_30_ngay, item.ban_trung_binh_ngay, item.forecast_demand, item.days_of_cover ?? "", item.active_cart_demand, item.de_xuat_theo_dinh_muc, item.de_xuat_theo_du_bao, item.so_luong_de_xuat, item.muc_do, item.nha_cung_cap?.ma_nha_cung_cap || "", item.nha_cung_cap?.ten_nha_cung_cap || "", item.nha_cung_cap?.dang_hoat_dong === true ? "SAN_SANG" : item.nha_cung_cap ? "NCC_NGUNG_HOAT_DONG" : "CHUA_GAN_NCC", item.nguon_nha_cung_cap, item.phieu_nhap_gan_nhat?.ma_phieu || ""]);
    const buffer = this.tao_xlsx(rows, "Kế hoạch nhập v3.26");
    const ngay = new Date(Date.now() + 7 * 60 * 60 * 1000).toISOString().slice(0, 10);
  ...
  async kiem_tra_kiem_ke_kho_v3260(dto: { dong: Array<{ bien_the_id: string; ton_he_thong: number; ton_thuc_te: number; ly_do?: string }> }) {
    ...
    for (const item of dto.dong) {
      if (seen.has(item.bien_the_id)) throw new BadRequestException(`Biến thể ${item.bien_the_id} bị lặp trong danh sách kiểm kê`);
      seen.add(item.bien_the_id);
    }
    const ids = dto.dong.map(x => x.bien_the_id);
    ...
    const rows = dto.dong.map(item => {
      const v = map.get(item.bien_the_id);
      if (!v) return { ...item, hop_le: false, loi: ["Không tìm thấy biến thể"], ton_he_thong_hien_tai: null, chenh_lech: null, stale_snapshot: false, ma_bien_the: "", ma_san_pham: "", ten_san_pham: "", vat_lieu: "", mau_sac: "" };
      const stale = v.so_luong_ton !== item.ton_he_thong;
      return {
        ...item,
        hop_le: !stale,
        loi: stale ? [`Tồn hệ thống đã thay đổi từ ${item.ton_he_thong} thành ${v.so_luong_ton}; hãy tải lại trước khi áp dụng`] : [],
        ton_he_thong_hien_tai: v.so_luong_ton,
        chenh_lech: item.ton_thuc_te - v.so_luong_ton,
    ...
    return {
      ...
      hop_le: rows.filter(x => x.hop_le).length,
      khong_hop_le: rows.filter(x => !x.hop_le).length,
      co_chenh_lech: rows.filter(x => typeof x.chenh_lech === "number" && x.chenh_lech !== 0).length,
      tong_chenh_lech_tuyet_doi: rows.reduce((sum, x) => sum + (typeof x.chenh_lech === "number" ? Math.abs(x.chenh_lech) : 0), 0),
      co_the_ap_dung: rows.length > 0 && rows.every(x => x.hop_le),
  ...
  async ap_dung_kiem_ke_kho_v3260(actor: NguoiDungXacThuc, dto: { dong: Array<{ bien_the_id: string; ton_he_thong: number; ton_thuc_te: number; ly_do?: string }> }) {
    const preview = await this.kiem_tra_kiem_ke_kho_v3260(dto);
    if (!preview.co_the_ap_dung) throw new ConflictException("Dữ liệu kiểm kê không còn khớp tồn hệ thống; hãy tải lại và kiểm tra lại trước khi áp dụng");
    const ket_qua = await this.db.$transaction(async tx => {
      let changed = 0;
      let tong_abs = 0;
      const items: Array<{ bien_the_id: string; ma_bien_the: string; ton_cu: number; ton_moi: number; chenh_lech: number }> = [];
      for (const input of dto.dong) {
        const current = await tx.bienTheSanPham.findUnique({ where: { id: input.bien_the_id }, include: { san_pham: { select: { ma_san_pham: true, ten_san_pham: true } } } });
        if (!current) throw new NotFoundException(`Không tìm thấy biến thể ${input.bien_the_id}`);
        if (current.so_luong_ton !== input.ton_he_thong) throw new ConflictException(`Tồn ${current.ma_bien_the} đã thay đổi từ ${input.ton_he_thong} thành ${current.so_luong_ton}; transaction kiểm kê đã hủy toàn bộ`);
        const diff = input.ton_thuc_te - current.so_luong_ton;
        if (diff !== 0) {
          const updated = await tx.bienTheSanPham.updateMany({ where: { id: current.id, so_luong_ton: input.ton_he_thong }, data: { so_luong_ton: input.ton_thuc_te } });
          if (updated.count !== 1) throw new ConflictException(`Tồn ${current.ma_bien_the} vừa thay đổi; transaction kiểm kê đã hủy toàn bộ`);
          changed += 1;
          tong_abs += Math.abs(diff);
          await tx.nhatKyBaoMat.create({ data: { loai_su_kien: "ADMIN_KIEM_KE_TON_KHO", nguoi_dung_id: actor.id, chi_tiet: { bien_the_id: current.id, ma_bien_the: current.ma_bien_the, ma_san_pham: current.san_pham.ma_san_pham, ton_cu: current.so_luong_ton, ton_moi: input.ton_thuc_te, chenh_lech: diff, loai_bien_dong: "KIEM_KE", ly_do: input.ly_do?.trim() || "Kiểm kê tồn kho thực tế", optimistic_lock: true } } });
        }
        items.push({ bien_the_id: current.id, ma_bien_the: current.ma_bien_the, ton_cu: current.so_luong_ton, ton_moi: input.ton_thuc_te, chenh_lech: diff });
      }
      await tx.nhatKyBaoMat.create({ data: { loai_su_kien: "ADMIN_AP_DUNG_KIEM_KE_KHO", nguoi_dung_id: actor.id, chi_tiet: { tong_dong: dto.dong.length, da_dieu_chinh: changed, tong_chenh_lech_tuyet_doi: tong_abs, optimistic_lock: true, atomic_transaction: true } } });
  ...
  async tong_quan() {
    ...
    const [nguoi_dung, khach_hang, nhan_vien, ca_lam_viec, phan_ca, don_hang, san_pham, don_30_ngay, don_doanh_thu_30_ngay, trang_thai_don, khach_hang_30_ngay, ton_kho_thap, sap_het_count, het_hang_count, don_gan_day] = await Promise.all([
      ...
      this.db.donHang.findMany({
        ...
        select: {
          ...
          chi_tiet: { select: { so_luong: true, gia_von_snapshot: true } },
          hoan_tien_chi_tiet: { select: { so_tien: true, ngay_tao: true } },
          doi_tra: { where: { trang_thai: { in: [TrangThaiYeuCauDoiTra.DA_NHAN_HANG, TrangThaiYeuCauDoiTra.HOAN_TAT] } }, select: { chi_tiet: { select: { so_luong_nhan: true, nhap_lai_ton: true, chi_tiet_don_hang: { select: { gia_von_snapshot: true } } } } } }
        }
      }),
    ...
    const doanhThuDaGhiNhan = don_doanh_thu_30_ngay.map(d => {
      // v3.29.0: gross -> partial/full refunds -> net revenue, and COGS from sale-time snapshot.
      const tt = d.thanh_toan.find(x => x.trang_thai === TrangThaiThanhToan.DA_THANH_TOAN || x.trang_thai === TrangThaiThanhToan.DA_HOAN_TIEN);
      ...
      const doanh_thu_gop = tt ? Number(tt.so_tien) : Number(d.tong_tien);
      const hoan_tien_ledger = d.hoan_tien_chi_tiet.reduce((sum, x) => sum + Number(x.so_tien), 0);
      // Refunds created before v3.29 only changed payment status, so treat that as a full legacy refund
      // only when there is no granular refund ledger to avoid double subtraction.
      const hoan_tien_legacy = tt?.trang_thai === TrangThaiThanhToan.DA_HOAN_TIEN && hoan_tien_ledger === 0 ? doanh_thu_gop : 0;
      const hoan_tien = Math.min(doanh_thu_gop, hoan_tien_ledger + hoan_tien_legacy);
      const so_tien = Math.max(0, doanh_thu_gop - hoan_tien);
      const gia_von_gop = d.chi_tiet.reduce((sum, line) => sum + (line.gia_von_snapshot == null ? 0 : Number(line.gia_von_snapshot) * line.so_luong), 0);
      const gia_von_thieu = d.chi_tiet.filter(line => line.gia_von_snapshot == null).length;
      const gia_von_nhap_lai = d.doi_tra.reduce((sum, rma) => sum + rma.chi_tiet.reduce((sub, line) => sub + (line.nhap_lai_ton && line.chi_tiet_don_hang.gia_von_snapshot != null ? Number(line.chi_tiet_don_hang.gia_von_snapshot) * line.so_luong_nhan : 0), 0), 0);
      const gia_von = Math.max(0, gia_von_gop - gia_von_nhap_lai);
      return { id: d.id, ngay_ghi_nhan, so_tien, doanh_thu_gop, hoan_tien, gia_von, loi_nhuan: so_tien - gia_von, gia_von_thieu };
    }).filter((x): x is { id: string; ngay_ghi_nhan: Date; so_tien: number; doanh_thu_gop: number; hoan_tien: number; gia_von: number; loi_nhuan: number; gia_von_thieu: number } => Boolean(x))
    ...
    const tongTheo = (ds: typeof doanhThuDaGhiNhan, key: "doanh_thu_gop" | "hoan_tien" | "gia_von" | "loi_nhuan") => ds.reduce((sum, item) => sum + item[key], 0);
    const loiNhuanKy = (batDau: Date) => {
      const ds = doanhThuDaGhiNhan.filter(d => trongKhoang(d.ngay_ghi_nhan, batDau));
      const doanh_thu_thuan = tongTienDoanhThu(ds);
      const gia_von = tongTheo(ds, "gia_von");
      const loi_nhuan_gop = tongTheo(ds, "loi_nhuan");
      return { doanh_thu_gop: tongTheo(ds, "doanh_thu_gop"), hoan_tien: tongTheo(ds, "hoan_tien"), doanh_thu_thuan, gia_von, loi_nhuan_gop, bien_loi_nhuan_percent: doanh_thu_thuan > 0 ? Number(((loi_nhuan_gop / doanh_thu_thuan) * 100).toFixed(2)) : null, dong_thieu_gia_von: ds.reduce((sum, x) => sum + x.gia_von_thieu, 0) };
    };
    const loi_nhuan_hom_nay = loiNhuanKy(bat_dau_hom_nay);
    const loi_nhuan_7_ngay = loiNhuanKy(bat_dau_7_ngay);
    const loi_nhuan_30_ngay = loiNhuanKy(bat_dau_30_ngay);
    ...
    return {
      ...
      loi_nhuan: {
        phien_ban: "3.29.0",
        co_so_gia_von: "SALE_TIME_COST_SNAPSHOT_WITH_MIGRATION_BACKFILL",
        hom_nay: loi_nhuan_hom_nay,
        bay_ngay: loi_nhuan_7_ngay,
        ba_muoi_ngay: loi_nhuan_30_ngay
      },
  ...
  private ca_chong_gio_v3240(a_bat_dau: string, a_ket_thuc: string, b_bat_dau: string, b_ket_thuc: string) {
    // Ca làm hiện tại được giới hạn trong cùng ngày, vì vậy chuỗi HH:mm có thể so sánh trực tiếp.
    // Hai ca chỉ chạm biên (VD 06:00-14:00 và 14:00-22:00) không được xem là chồng giờ.
    return a_bat_dau < b_ket_thuc && a_ket_thuc > b_bat_dau;
  }

  private async tim_xung_dot_phan_ca_v3240(nhan_vien_id: string, ngay_lam: Date, ca: { gio_bat_dau: string; gio_ket_thuc: string }, bo_qua_phan_ca_id?: string) {
    const ds = await this.db.phanCa.findMany({
      where: {
        nhan_vien_id,
        ngay_lam,
        ...(bo_qua_phan_ca_id ? { id: { not: bo_qua_phan_ca_id } } : {}),
      },
      include: {
        ca_lam_viec: true,
        nhan_vien: { include: { nguoi_dung: { select: { ho_ten: true } } } },
      },
      orderBy: { ngay_tao: "asc" },
    });
    return ds.find(x => this.ca_chong_gio_v3240(ca.gio_bat_dau, ca.gio_ket_thuc, x.ca_lam_viec.gio_bat_dau, x.ca_lam_viec.gio_ket_thuc)) || null;
  }

  async danh_sach_xung_dot_phan_ca_v3240(tu_ngay_raw?: string, den_ngay_raw?: string) {
    const hop_le = (value?: string) => !value || /^\d{4}-\d{2}-\d{2}$/.test(value.trim());
    if (!hop_le(tu_ngay_raw) || !hop_le(den_ngay_raw)) throw new BadRequestException("Ngày lọc phân ca phải có định dạng YYYY-MM-DD");
    const hom_nay = new Date();
    const mac_dinh_tu = new Date(Date.UTC(hom_nay.getUTCFullYear(), hom_nay.getUTCMonth(), hom_nay.getUTCDate()));
    const mac_dinh_den = new Date(mac_dinh_tu.getTime() + 30 * 86_400_000);
    const tu_ngay = tu_ngay_raw?.trim() ? new Date(`${tu_ngay_raw.trim()}T00:00:00.000Z`) : mac_dinh_tu;
    const den_ngay = den_ngay_raw?.trim() ? new Date(`${den_ngay_raw.trim()}T00:00:00.000Z`) : mac_dinh_den;
    if (Number.isNaN(tu_ngay.getTime()) || Number.isNaN(den_ngay.getTime())) throw new BadRequestException("Khoảng ngày phân ca không hợp lệ");
    if (tu_ngay > den_ngay) throw new BadRequestException("Từ ngày phân ca không được lớn hơn đến ngày");
    if (den_ngay.getTime() - tu_ngay.getTime() > 366 * 86_400_000) throw new BadRequestException("Chỉ được rà soát tối đa 366 ngày phân ca mỗi lần");
    const ds = await this.db.phanCa.findMany({
      where: { ngay_lam: { gte: tu_ngay, lte: den_ngay } },
      include: {
        ca_lam_viec: true,
        nhan_vien: { include: { nguoi_dung: { select: { ho_ten: true } } } },
      },
      orderBy: [{ ngay_lam: "asc" }, { nhan_vien_id: "asc" }, { ngay_tao: "asc" }],
      take: 3000,
    });
    const xung_dot: Array<Record<string, unknown>> = [];
    for (let i = 0; i < ds.length; i += 1) {
      const a = ds[i];
      for (let j = i + 1; j < ds.length; j += 1) {
        const b = ds[j];
        if (a.ngay_lam.getTime() !== b.ngay_lam.getTime()) break;
        if (a.nhan_vien_id !== b.nhan_vien_id) continue;
        if (!this.ca_chong_gio_v3240(a.ca_lam_viec.gio_bat_dau, a.ca_lam_viec.gio_ket_thuc, b.ca_lam_viec.gio_bat_dau, b.ca_lam_viec.gio_ket_thuc)) continue;
        xung_dot.push({
          ngay_lam: a.ngay_lam.toISOString().slice(0, 10),
          nhan_vien_id: a.nhan_vien_id,
          ma_nhan_vien: a.nhan_vien.ma_nhan_vien,
          ho_ten: a.nhan_vien.nguoi_dung.ho_ten,
          phan_ca_a: { id: a.id, ma_ca: a.ca_lam_viec.ma_ca, ten_ca: a.ca_lam_viec.ten_ca, gio_bat_dau: a.ca_lam_viec.gio_bat_dau, gio_ket_thuc: a.ca_lam_viec.gio_ket_thuc },
          phan_ca_b: { id: b.id, ma_ca: b.ca_lam_viec.ma_ca, ten_ca: b.ca_lam_viec.ten_ca, gio_bat_dau: b.ca_lam_viec.gio_bat_dau, gio_ket_thuc: b.ca_lam_viec.gio_ket_thuc },
        });
      }
    }
    return {
      ...
      tong_xung_dot: xung_dot.length,
      nhan_vien_bi_xung_dot: new Set(xung_dot.map(x => String(x.nhan_vien_id))).size,
      xung_dot,
      overlap_guard_enabled: true as const,
    };
  }

  async danh_sach_xung_dot_phan_ca_v3250(tu_ngay_raw?: string, den_ngay_raw?: string) {
    const base = await this.danh_sach_xung_dot_phan_ca_v3240(tu_ngay_raw, den_ngay_raw);
    return { ...base, phien_ban: "3.25.0" };
  }

  async danh_sach_xung_dot_phan_ca_v3260(tu_ngay_raw?: string, den_ngay_raw?: string) {
    const base = await this.danh_sach_xung_dot_phan_ca_v3250(tu_ngay_raw, den_ngay_raw);
  ...
  async cap_nhat_ca(actor: NguoiDungXacThuc, id: string, dto: CapNhatCaLamDto) {
    ...
    if (gio_bat_dau !== hien_tai.gio_bat_dau || gio_ket_thuc !== hien_tai.gio_ket_thuc) {
      const phan_cong = await this.db.phanCa.findMany({
        where: { ca_lam_viec_id: id },
        select: { id: true, nhan_vien_id: true, ngay_lam: true, nhan_vien: { select: { ma_nhan_vien: true, nguoi_dung: { select: { ho_ten: true } } } } },
        orderBy: { ngay_lam: "asc" },
      });
      for (const item of phan_cong) {
        const xung_dot = await this.tim_xung_dot_phan_ca_v3240(item.nhan_vien_id, item.ngay_lam, { gio_bat_dau, gio_ket_thuc }, item.id);
        if (xung_dot) {
          throw new ConflictException(`Khung giờ mới ${gio_bat_dau}-${gio_ket_thuc} làm trùng lịch ${xung_dot.ca_lam_viec.ma_ca} ${xung_dot.ca_lam_viec.gio_bat_dau}-${xung_dot.ca_lam_viec.gio_ket_thuc} của ${item.nhan_vien.ma_nhan_vien} · ${item.nhan_vien.nguoi_dung.ho_ten} ngày ${item.ngay_lam.toISOString().slice(0, 10)}. Hãy xử lý phân ca xung đột trước khi sửa mẫu ca.`);
        }
      }
    }

    // Không chặn chỉnh sửa mẫu ca khi đã có phân công nếu khung giờ mới vẫn an toàn. PhanCa giữ khóa ngoại tới
  ...
  async tao_phan_ca(actor: NguoiDungXacThuc, dto: TaoPhanCaDto) {
    ...
    const ngay_lam = new Date(`${dto.ngay_lam}T00:00:00Z`);
    const xung_dot = await this.tim_xung_dot_phan_ca_v3240(dto.nhan_vien_id, ngay_lam, ca);
    if (xung_dot) throw new ConflictException(`Nhân viên đã có ${xung_dot.ca_lam_viec.ma_ca} · ${xung_dot.ca_lam_viec.ten_ca} (${xung_dot.ca_lam_viec.gio_bat_dau}-${xung_dot.ca_lam_viec.gio_ket_thuc}) chồng giờ trong ngày đã chọn`);

    try {
      const da_tao = await this.db.phanCa.create({
        data: { nhan_vien_id: dto.nhan_vien_id, ca_lam_viec_id: dto.ca_lam_viec_id, ngay_lam, ghi_chu: dto.ghi_chu?.trim() || null }
      });
  ...
  async cap_nhat_phan_ca(actor: NguoiDungXacThuc, id: string, dto: CapNhatPhanCaDto) {
    ...
    const xung_dot = await this.tim_xung_dot_phan_ca_v3240(nhan_vien_id, ngay_lam, ca, id);
    if (xung_dot) throw new ConflictException(`Phân ca mới chồng giờ với ${xung_dot.ca_lam_viec.ma_ca} · ${xung_dot.ca_lam_viec.ten_ca} (${xung_dot.ca_lam_viec.gio_bat_dau}-${xung_dot.ca_lam_viec.gio_ket_thuc})`);
  ...
  async chi_tiet_don_hang(id: string) {
    const item = await this.db.donHang.findUnique({
      ...
      include: {
        ...
        doi_tra: { include: { chi_tiet: { include: { chi_tiet_don_hang: { select: { ten_san_pham: true, ma_san_pham: true, don_gia: true, gia_von_snapshot: true } } } }, hoan_tien: true }, orderBy: { ngay_tao: "desc" } },
        hoan_tien_chi_tiet: { orderBy: { ngay_tao: "desc" } },
    ...
    const giao_dich_can_hoan = item.thanh_toan.filter(tt => tt.trang_thai === TrangThaiThanhToan.DA_THANH_TOAN);
    const giao_dich_da_hoan = item.thanh_toan.filter(tt => tt.trang_thai === TrangThaiThanhToan.DA_HOAN_TIEN);
    return {
      ...
      chi_tiet: item.chi_tiet.map(ct => ({ ...ct, don_gia: Number(ct.don_gia), thanh_tien: Number(ct.thanh_tien), gia_von_snapshot: ct.gia_von_snapshot == null ? null : Number(ct.gia_von_snapshot) })),
      thanh_toan: item.thanh_toan.map(tt => ({ ...tt, so_tien: Number(tt.so_tien) })),
      doi_tra: item.doi_tra.map(rma => ({
        ...rma,
        tong_tien_hoan_duyet: Number(rma.tong_tien_hoan_duyet),
        tong_tien_da_hoan: rma.hoan_tien.reduce((sum, x) => sum + Number(x.so_tien), 0),
        chi_tiet: rma.chi_tiet.map(line => ({ ...line, so_tien_hoan_duyet: Number(line.so_tien_hoan_duyet), chi_tiet_don_hang: { ...line.chi_tiet_don_hang, don_gia: Number(line.chi_tiet_don_hang.don_gia), gia_von_snapshot: line.chi_tiet_don_hang.gia_von_snapshot == null ? null : Number(line.chi_tiet_don_hang.gia_von_snapshot) } })),
        hoan_tien: rma.hoan_tien.map(x => ({ ...x, so_tien: Number(x.so_tien) })),
      })),
      hoan_tien_chi_tiet: item.hoan_tien_chi_tiet.map(x => ({ ...x, so_tien: Number(x.so_tien) })),
      hoan_tien: {
        can_xu_ly: item.trang_thai === TrangThaiDonHang.DA_HUY && giao_dich_can_hoan.length > 0,
        da_hoan_tien: giao_dich_da_hoan.length > 0 && giao_dich_can_hoan.length === 0,
        so_giao_dich_can_hoan: giao_dich_can_hoan.length,
        so_giao_dich_da_hoan: giao_dich_da_hoan.length,
        so_tien_can_hoan: giao_dich_can_hoan.reduce((sum, tt) => sum + Number(tt.so_tien), 0),
        so_tien_da_hoan: giao_dich_da_hoan.reduce((sum, tt) => sum + Number(tt.so_tien), 0) + item.hoan_tien_chi_tiet.reduce((sum, x) => sum + Number(x.so_tien), 0),
        so_tien_hoan_mot_phan: item.hoan_tien_chi_tiet.reduce((sum, x) => sum + Number(x.so_tien), 0),
        gateway_auto_refund: false as const,
        manual_confirmation_required: true as const,
      },
      lich_su: item.lich_su.map(ls => ({ ...ls, id: ls.id.toString() }))
    };
  }

  async danh_sach_hoan_tien_can_xu_ly_v3250() {
    const ds = await this.db.donHang.findMany({
      where: {
        trang_thai: TrangThaiDonHang.DA_HUY,
        thanh_toan: { some: { trang_thai: TrangThaiThanhToan.DA_THANH_TOAN } },
      },
      include: {
        nguoi_dung: { select: { ho_ten: true, thu_dien_tu: true } },
        thanh_toan: { include: { phuong_thuc: true }, orderBy: { ngay_tao: "desc" } },
      },
      orderBy: { ngay_cap_nhat: "desc" },
      take: 500,
    });
    const items = ds.map(don => {
      const giao_dich = don.thanh_toan.filter(tt => tt.trang_thai === TrangThaiThanhToan.DA_THANH_TOAN);
      return {
        id: don.id,
        ma_don_hang: don.ma_don_hang,
        ho_ten_nguoi_nhan: don.ho_ten_nguoi_nhan,
        thu_dien_tu: don.nguoi_dung?.thu_dien_tu || null,
        ngay_huy_cap_nhat: don.ngay_cap_nhat.toISOString(),
        so_giao_dich: giao_dich.length,
        so_tien_can_hoan: giao_dich.reduce((sum, tt) => sum + Number(tt.so_tien), 0),
        giao_dich: giao_dich.map(tt => ({ id: tt.id, ma_giao_dich: tt.ma_giao_dich, so_tien: Number(tt.so_tien), phuong_thuc: tt.phuong_thuc.ma_phuong_thuc, ngay_thanh_toan: tt.ngay_thanh_toan?.toISOString() || null })),
      };
    });
    return {
      phien_ban: "3.25.0",
      tong_don_can_hoan: items.length,
      tong_tien_can_hoan: items.reduce((sum, item) => sum + item.so_tien_can_hoan, 0),
      items,
      gateway_auto_refund: false as const,
      manual_confirmation_required: true as const,
    };
  ...
  async xac_nhan_hoan_tien_don_hang_v3250(actor: NguoiDungXacThuc, id: string, ghi_chu?: string) {
    const don = await this.db.donHang.findUnique({
      where: { id },
      include: { thanh_toan: { include: { phuong_thuc: true }, orderBy: { ngay_tao: "desc" } } },
    });
    if (!don) throw new NotFoundException("Không tìm thấy đơn hàng");
    if (don.trang_thai !== TrangThaiDonHang.DA_HUY) throw new BadRequestException("Chỉ xác nhận hoàn tiền cho đơn đã hủy");
    const can_hoan = don.thanh_toan.filter(tt => tt.trang_thai === TrangThaiThanhToan.DA_THANH_TOAN);
    const da_hoan = don.thanh_toan.filter(tt => tt.trang_thai === TrangThaiThanhToan.DA_HOAN_TIEN);
    if (!can_hoan.length) {
      if (da_hoan.length) {
        const chi_tiet = await this.chi_tiet_don_hang(id);
        return { ...chi_tiet, xac_nhan_hoan_tien: { da_cap_nhat_moi: false, da_hoan_tien_tu_truoc: true, so_giao_dich: da_hoan.length, so_tien: da_hoan.reduce((sum, tt) => sum + Number(tt.so_tien), 0) } };
      }
      throw new BadRequestException("Đơn đã hủy nhưng không có giao dịch đã thanh toán cần hoàn tiền");
    }
    const so_tien = can_hoan.reduce((sum, tt) => sum + Number(tt.so_tien), 0);
    const note = ghi_chu?.trim() || "Admin xác nhận đã hoàn tiền cho khách sau khi hủy đơn.";
    await this.db.$transaction(async tx => {
      for (const tt of can_hoan) {
        const cap_nhat = await tx.thanhToan.updateMany({
          where: { id: tt.id, trang_thai: TrangThaiThanhToan.DA_THANH_TOAN },
          data: { trang_thai: TrangThaiThanhToan.DA_HOAN_TIEN, noi_dung: `${tt.noi_dung || don.ma_don_hang} · ${note}` },
        });
        if (cap_nhat.count !== 1) throw new ConflictException(`Giao dịch ${tt.ma_giao_dich} đã thay đổi trạng thái; hãy tải lại trước khi xác nhận hoàn tiền`);
      }
      await tx.lichSuDonHang.create({ data: { don_hang_id: id, nguoi_thuc_hien_id: actor.id, trang_thai_cu: TrangThaiDonHang.DA_HUY, trang_thai_moi: TrangThaiDonHang.DA_HUY, ghi_chu: `Hoàn tiền ${so_tien.toLocaleString("vi-VN")}đ · ${note}` } });
    });
    await this.db.nhatKyBaoMat.create({ data: { loai_su_kien: "ADMIN_XAC_NHAN_HOAN_TIEN", nguoi_dung_id: actor.id, chi_tiet: { don_hang_id: id, ma_don_hang: don.ma_don_hang, so_giao_dich: can_hoan.length, so_tien, giao_dich: can_hoan.map(tt => ({ id: tt.id, ma_giao_dich: tt.ma_giao_dich, phuong_thuc: tt.phuong_thuc.ma_phuong_thuc })), gateway_auto_refund: false, ghi_chu: note } } });
    const chi_tiet = await this.chi_tiet_don_hang(id);
    return { ...chi_tiet, xac_nhan_hoan_tien: { da_cap_nhat_moi: true, da_hoan_tien_tu_truoc: false, so_giao_dich: can_hoan.length, so_tien } };
  }

  async xac_nhan_hoan_tien_don_hang_v3260(actor: NguoiDungXacThuc, id: string, ghi_chu?: string) { return this.xac_nhan_hoan_tien_don_hang_v3250(actor, id, ghi_chu); }

  private refund_sla_hours_v3260() {
    const raw = Number(process.env.SYSTEM_REFUND_RECONCILIATION_SLA_HOURS || 24);
    return Math.max(1, Math.min(168, Number.isFinite(raw) ? Math.round(raw) : 24));
  }

  async danh_sach_hoan_tien_can_xu_ly_v3260() {
    const sla_hours = this.refund_sla_hours_v3260();
    const now = new Date();
    const ds = await this.db.donHang.findMany({
      where: {
        trang_thai: TrangThaiDonHang.DA_HUY,
      ...
      include: {
        nguoi_dung: { select: { ho_ten: true, thu_dien_tu: true } },
        thanh_toan: { include: { phuong_thuc: true }, orderBy: { ngay_tao: "desc" } },
        lich_su: { where: { trang_thai_moi: TrangThaiDonHang.DA_HUY }, orderBy: { ngay_tao: "desc" }, take: 1 },
      },
      orderBy: { ngay_cap_nhat: "asc" },
      take: 500,
    });
    const dueSoonHours = Math.max(1, Math.min(6, Math.round(sla_hours / 4)));
    const items = ds.map(don => {
      const giao_dich = don.thanh_toan.filter(tt => tt.trang_thai === TrangThaiThanhToan.DA_THANH_TOAN);
      const huy_luc = don.lich_su[0]?.ngay_tao || don.ngay_cap_nhat;
      const due_at = new Date(huy_luc.getTime() + sla_hours * 3_600_000);
      const age_hours = Math.max(0, Math.round(((now.getTime() - huy_luc.getTime()) / 3_600_000) * 10) / 10);
      const remaining_hours = Math.round(((due_at.getTime() - now.getTime()) / 3_600_000) * 10) / 10;
      const qua_han = remaining_hours < 0;
      const sap_den_han = !qua_han && remaining_hours <= dueSoonHours;
      return {
        id: don.id,
        ma_don_hang: don.ma_don_hang,
        ho_ten_nguoi_nhan: don.ho_ten_nguoi_nhan,
        thu_dien_tu: don.nguoi_dung?.thu_dien_tu || null,
        ngay_huy_cap_nhat: huy_luc.toISOString(),
        refund_due_at: due_at.toISOString(),
        refund_age_hours: age_hours,
        refund_remaining_hours: remaining_hours,
        refund_sla_status: qua_han ? "QUA_HAN" as const : sap_den_han ? "SAP_DEN_HAN" as const : "TRONG_HAN" as const,
        so_giao_dich: giao_dich.length,
        so_tien_can_hoan: giao_dich.reduce((sum, tt) => sum + Number(tt.so_tien), 0),
        giao_dich: giao_dich.map(tt => ({ id: tt.id, ma_giao_dich: tt.ma_giao_dich, so_tien: Number(tt.so_tien), phuong_thuc: tt.phuong_thuc.ma_phuong_thuc, ngay_thanh_toan: tt.ngay_thanh_toan?.toISOString() || null })),
      };
    }).sort((a, b) => {
      const rank = (v: string) => v === "QUA_HAN" ? 0 : v === "SAP_DEN_HAN" ? 1 : 2;
      return rank(a.refund_sla_status) - rank(b.refund_sla_status) || b.refund_age_hours - a.refund_age_hours || a.ma_don_hang.localeCompare(b.ma_don_hang, "vi");
    });
    return {
      phien_ban: "3.26.0",
      refund_sla_hours: sla_hours,
      refund_due_soon_hours: dueSoonHours,
      tong_don_can_hoan: items.length,
      tong_tien_can_hoan: items.reduce((sum, item) => sum + item.so_tien_can_hoan, 0),
      tong_qua_han: items.filter(x => x.refund_sla_status === "QUA_HAN").length,
      tong_sap_den_han: items.filter(x => x.refund_sla_status === "SAP_DEN_HAN").length,
      items,
      gateway_auto_refund: false as const,
      manual_confirmation_required: true as const,
      sla_is_operational_target_only: true as const,
    };
  }

  async xuat_hoan_tien_can_xu_ly_excel_v3260() {
    const queue = await this.danh_sach_hoan_tien_can_xu_ly_v3260();
    const rows: unknown[][] = [["Mã đơn", "Khách hàng", "Email", "Hủy lúc", "Hạn hoàn tiền", "Tuổi hàng đợi (giờ)", "Còn lại (giờ)", "SLA", "Số giao dịch", "Số tiền cần hoàn", "Mã giao dịch", "Phương thức", "Thanh toán lúc"]];
    for (const item of queue.items) {
      if (!item.giao_dich.length) rows.push([item.ma_don_hang, item.ho_ten_nguoi_nhan, item.thu_dien_tu || "", item.ngay_huy_cap_nhat, item.refund_due_at, item.refund_age_hours, item.refund_remaining_hours, item.refund_sla_status, item.so_giao_dich, item.so_tien_can_hoan, "", "", ""]);
      for (const gd of item.giao_dich) rows.push([item.ma_don_hang, item.ho_ten_nguoi_nhan, item.thu_dien_tu || "", item.ngay_huy_cap_nhat, item.refund_due_at, item.refund_age_hours, item.refund_remaining_hours, item.refund_sla_status, item.so_giao_dich, item.so_tien_can_hoan, gd.ma_giao_dich, gd.phuong_thuc, gd.ngay_thanh_toan || ""]);
    }
    const buffer = this.tao_xlsx(rows, "Hoàn tiền cần xử lý");
    const ngay = new Date(Date.now() + 7 * 60 * 60 * 1000).toISOString().slice(0, 10);
  ...
  async lay_probe_rollout_proposal_v3290() { const base = await this.lay_probe_rollout_proposal_v3280(); return { ...base, phien_ban: "3.29.0" }; }
  async cap_nhat_probe_desired_state_v3290(actor: NguoiDungXacThuc, dto: { target_version: string; interval_seconds: number; rollout_percent: number; canary_agents: string[]; paused: boolean; note?: string }) { const result = await this.cap_nhat_probe_desired_state_v3280(actor, dto); return { ...result, rollout_approval: await this.lay_probe_rollout_proposal_v3290() }; }
  async approve_probe_rollout_v3290(actor: NguoiDungXacThuc, proposalIdRaw: string, note?: string) { await this.approve_probe_rollout_v3280(actor, proposalIdRaw, note); return this.lay_probe_rollout_proposal_v3290(); }
  async reject_probe_rollout_v3290(actor: NguoiDungXacThuc, proposalIdRaw: string, note?: string) { await this.reject_probe_rollout_v3280(actor, proposalIdRaw, note); return this.lay_probe_rollout_proposal_v3290(); }
  async cancel_probe_rollout_v3290(actor: NguoiDungXacThuc, proposalIdRaw: string, note?: string) { await this.cancel_probe_rollout_v3280(actor, proposalIdRaw, note); return this.lay_probe_rollout_proposal_v3290(); }
  async danh_sach_xung_dot_phan_ca_v3290(tu_ngay_raw?: string, den_ngay_raw?: string) { const base = await this.danh_sach_xung_dot_phan_ca_v3280(tu_ngay_raw, den_ngay_raw); return { ...base, phien_ban: "3.29.0" }; }
  async danh_sach_hoan_tien_can_xu_ly_v3290() { const base = await this.danh_sach_hoan_tien_can_xu_ly_v3280(); return { ...base, phien_ban: "3.29.0" }; }
  async xuat_hoan_tien_can_xu_ly_excel_v3290() { const base = await this.xuat_hoan_tien_can_xu_ly_excel_v3280(); return { ...base, ten_file: base.ten_file.replace("v3.28.0", "v3.29.0") }; }
  async xac_nhan_hoan_tien_don_hang_v3290(actor: NguoiDungXacThuc, id: string, ghi_chu?: string) { return this.xac_nhan_hoan_tien_don_hang_v3280(actor, id, ghi_chu); }
  async kiem_tra_tep_kiem_ke_kho_v3290(dto: KiemTraTepNhapKhoDto) { const base = await this.kiem_tra_tep_kiem_ke_kho_v3280(dto); return { ...base, phien_ban: "3.29.0" }; }
  async kiem_tra_kiem_ke_kho_v3290(dto: { dong: Array<{ bien_the_id: string; ton_he_thong: number; ton_thuc_te: number; ly_do?: string }> }) { const base = await this.kiem_tra_kiem_ke_kho_v3280(dto); return { ...base, phien_ban: "3.29.0" }; }
  async ap_dung_kiem_ke_kho_v3290(actor: NguoiDungXacThuc, dto: { dong: Array<{ bien_the_id: string; ton_he_thong: number; ton_thuc_te: number; ly_do?: string }> }) { const base = await this.ap_dung_kiem_ke_kho_v3280(actor, dto); return { ...base, phien_ban: "3.29.0" }; }
  async xuat_kiem_ke_kho_excel_v3290(dto: { dong: Array<{ bien_the_id: string; ton_he_thong: number; ton_thuc_te: number; ly_do?: string }> }) { const base = await this.xuat_kiem_ke_kho_excel_v3280(dto); return { ...base, ten_file: base.ten_file.replace("v3.28.0", "v3.29.0") }; }
  async goi_y_nhap_kho_v3290() { const base = await this.goi_y_nhap_kho_v3280(); return { ...base, phien_ban: "3.29.0" }; }
  async xuat_goi_y_nhap_kho_excel_v3290() { const base = await this.xuat_goi_y_nhap_kho_excel_v3280(); return { ...base, ten_file: base.ten_file.replace("v3.28.0", "v3.29.0") }; }

  private chuan_hoa_yeu_cau_doi_tra_v3290(item: any) {
    const tongDaHoan = (item.hoan_tien || []).reduce((sum: number, x: any) => sum + Number(x.so_tien), 0);
    return {
      ...item,
      tong_tien_hoan_duyet: Number(item.tong_tien_hoan_duyet),
      tong_tien_da_hoan: tongDaHoan,
      tong_tien_con_lai: Math.max(0, Number(item.tong_tien_hoan_duyet) - tongDaHoan),
      chi_tiet: (item.chi_tiet || []).map((line: any) => ({
        ...line,
        so_tien_hoan_duyet: Number(line.so_tien_hoan_duyet),
        chi_tiet_don_hang: line.chi_tiet_don_hang ? { ...line.chi_tiet_don_hang, don_gia: Number(line.chi_tiet_don_hang.don_gia), thanh_tien: Number(line.chi_tiet_don_hang.thanh_tien), gia_von_snapshot: line.chi_tiet_don_hang.gia_von_snapshot == null ? null : Number(line.chi_tiet_don_hang.gia_von_snapshot) } : undefined,
      })),
      hoan_tien: (item.hoan_tien || []).map((x: any) => ({ ...x, so_tien: Number(x.so_tien) })),
    };
  }

  async danh_sach_doi_tra_v3290(don_hang_id?: string) {
    const ds = await this.db.yeuCauDoiTra.findMany({
      where: don_hang_id ? { don_hang_id } : undefined,
      include: {
        don_hang: { select: { id: true, ma_don_hang: true, ho_ten_nguoi_nhan: true, tong_tien: true, trang_thai: true } },
        chi_tiet: { include: { chi_tiet_don_hang: true, bien_the: { select: { id: true, ma_bien_the: true, so_luong_ton: true } } } },
        hoan_tien: { orderBy: { ngay_tao: "desc" } },
      },
      orderBy: { ngay_tao: "desc" },
      take: 300,
    });
    return { phien_ban: "3.29.0", items: ds.map(x => this.chuan_hoa_yeu_cau_doi_tra_v3290(x)), partial_refund_ledger: true as const, auto_gateway_refund: false as const };
  }

  async tao_yeu_cau_doi_tra_v3290(actor: NguoiDungXacThuc, don_hang_id: string, dto: { ly_do: string; ghi_chu?: string; dong: Array<{ chi_tiet_don_hang_id: string; so_luong: number; xu_ly: "HOAN_TIEN" | "DOI_HANG"; nhap_lai_ton?: boolean; so_tien_hoan_duyet?: number; ly_do?: string }> }) {
    const don = await this.db.donHang.findUnique({ where: { id: don_hang_id }, include: { chi_tiet: true } });
    if (!don) throw new NotFoundException("Không tìm thấy đơn hàng");
    if (don.trang_thai !== TrangThaiDonHang.HOAN_TAT) throw new ConflictException("Chỉ tạo yêu cầu đổi/trả cho đơn đã giao / hoàn tất");
    const ids = dto.dong.map(x => x.chi_tiet_don_hang_id);
    if (new Set(ids).size !== ids.length) throw new BadRequestException("Một dòng đơn hàng không được lặp trong cùng yêu cầu đổi/trả");
    const lineMap = new Map(don.chi_tiet.map(x => [x.id, x]));
    if (ids.some(id => !lineMap.has(id))) throw new BadRequestException("Có dòng đổi/trả không thuộc đơn hàng này");
    const daYeuCau = await this.db.chiTietDoiTra.findMany({ where: { chi_tiet_don_hang_id: { in: ids }, yeu_cau: { is: { trang_thai: { not: TrangThaiYeuCauDoiTra.HUY } } } }, select: { chi_tiet_don_hang_id: true, so_luong_y_cau: true } });
    const used = new Map<string, number>();
    for (const x of daYeuCau) used.set(x.chi_tiet_don_hang_id, (used.get(x.chi_tiet_don_hang_id) || 0) + x.so_luong_y_cau);
    const variantCodes = [...new Set(dto.dong.map(x => String((lineMap.get(x.chi_tiet_don_hang_id)?.tuy_chon as any)?.ma_bien_the || "")).filter(Boolean))];
    const variants = variantCodes.length ? await this.db.bienTheSanPham.findMany({ where: { ma_bien_the: { in: variantCodes } }, select: { id: true, ma_bien_the: true } }) : [];
    const variantMap = new Map(variants.map(x => [x.ma_bien_the, x]));
    let tongHoan = 0;
    const normalized = dto.dong.map(x => {
      const line = lineMap.get(x.chi_tiet_don_hang_id)!;
      const conLai = line.so_luong - (used.get(line.id) || 0);
      if (x.so_luong > conLai) throw new ConflictException(`Dòng ${line.ma_san_pham} chỉ còn ${conLai} sản phẩm có thể đổi/trả`);
      const maBienThe = String((line.tuy_chon as any)?.ma_bien_the || "") || null;
      const variant = maBienThe ? variantMap.get(maBienThe) : undefined;
      if (x.nhap_lai_ton && !variant) throw new BadRequestException(`Không tìm thấy biến thể ${maBienThe || "của dòng hàng"} để nhập lại tồn`);
      const maxRefund = Number(line.don_gia) * x.so_luong;
      const approved = x.xu_ly === "HOAN_TIEN" ? (x.so_tien_hoan_duyet == null ? maxRefund : x.so_tien_hoan_duyet) : 0;
      if (approved > maxRefund) throw new BadRequestException(`Tiền hoàn của ${line.ma_san_pham} không được vượt giá trị dòng trả`);
      tongHoan += approved;
      return { line, x, maBienThe, variant, approved };
    });
    const ma = `RMA-${new Date().toISOString().replace(/\D/g, "").slice(0, 14)}-${randomUUID().slice(0, 6).toUpperCase()}`;
    const created = await this.db.yeuCauDoiTra.create({
      data: {
        ma_yeu_cau: ma, don_hang_id, ly_do: dto.ly_do.trim(), ghi_chu: dto.ghi_chu?.trim() || null, nguoi_tao_id: actor.id, tong_tien_hoan_duyet: tongHoan,
        chi_tiet: { create: normalized.map(({ line, x, maBienThe, variant, approved }) => ({ chi_tiet_don_hang_id: line.id, bien_the_id: variant?.id || null, ma_bien_the: maBienThe, so_luong_y_cau: x.so_luong, xu_ly: x.xu_ly as XuLyDoiTra, nhap_lai_ton: Boolean(x.nhap_lai_ton), so_tien_hoan_duyet: approved, ly_do: x.ly_do?.trim() || null })) },
      },
      include: { don_hang: { select: { id: true, ma_don_hang: true, ho_ten_nguoi_nhan: true, tong_tien: true, trang_thai: true } }, chi_tiet: { include: { chi_tiet_don_hang: true, bien_the: { select: { id: true, ma_bien_the: true, so_luong_ton: true } } } }, hoan_tien: true },
    });
    await this.db.nhatKyBaoMat.create({ data: { loai_su_kien: "ADMIN_TAO_YEU_CAU_DOI_TRA", nguoi_dung_id: actor.id, chi_tiet: { yeu_cau_id: created.id, ma_yeu_cau: ma, don_hang_id, tong_dong: normalized.length, tong_tien_hoan_duyet: tongHoan, auto_gateway_refund: false } } });
    return this.chuan_hoa_yeu_cau_doi_tra_v3290(created);
  }

  async cap_nhat_trang_thai_doi_tra_v3290(actor: NguoiDungXacThuc, id: string, trang_thai: "DA_DUYET" | "HUY" | "HOAN_TAT", ghi_chu?: string) {
    const current = await this.db.yeuCauDoiTra.findUnique({ where: { id }, include: { hoan_tien: true } });
    if (!current) throw new NotFoundException("Không tìm thấy yêu cầu đổi/trả");
    if (trang_thai === "DA_DUYET" && current.trang_thai !== TrangThaiYeuCauDoiTra.CHO_DUYET) throw new ConflictException("Chỉ yêu cầu CHO_DUYET mới được duyệt");
    if (trang_thai === "HUY" && (current.trang_thai === TrangThaiYeuCauDoiTra.DA_NHAN_HANG || current.trang_thai === TrangThaiYeuCauDoiTra.HOAN_TAT)) throw new ConflictException("Không thể hủy yêu cầu đã nhận hàng hoặc hoàn tất");
    if (trang_thai === "HOAN_TAT") {
      if (current.trang_thai !== TrangThaiYeuCauDoiTra.DA_NHAN_HANG) throw new ConflictException("Chỉ yêu cầu đã nhận hàng mới được hoàn tất");
      const daHoan = current.hoan_tien.reduce((sum, x) => sum + Number(x.so_tien), 0);
      if (daHoan + 0.0001 < Number(current.tong_tien_hoan_duyet)) throw new ConflictException("Yêu cầu vẫn còn số tiền hoàn đã duyệt chưa được xác nhận thực tế");
    }
    const next = trang_thai as TrangThaiYeuCauDoiTra;
    const updated = await this.db.yeuCauDoiTra.update({ where: { id }, data: { trang_thai: next, nguoi_duyet_id: next === TrangThaiYeuCauDoiTra.DA_DUYET ? actor.id : current.nguoi_duyet_id, ngay_duyet: next === TrangThaiYeuCauDoiTra.DA_DUYET ? new Date() : current.ngay_duyet, ngay_hoan_tat: next === TrangThaiYeuCauDoiTra.HOAN_TAT ? new Date() : current.ngay_hoan_tat, ghi_chu: ghi_chu?.trim() ? `${current.ghi_chu || ""}${current.ghi_chu ? "\n" : ""}${ghi_chu.trim()}` : current.ghi_chu }, include: { don_hang: { select: { id: true, ma_don_hang: true, ho_ten_nguoi_nhan: true, tong_tien: true, trang_thai: true } }, chi_tiet: { include: { chi_tiet_don_hang: true, bien_the: { select: { id: true, ma_bien_the: true, so_luong_ton: true } } } }, hoan_tien: true } });
    await this.db.nhatKyBaoMat.create({ data: { loai_su_kien: "ADMIN_CAP_NHAT_DOI_TRA", nguoi_dung_id: actor.id, chi_tiet: { yeu_cau_id: id, ma_yeu_cau: current.ma_yeu_cau, truoc: current.trang_thai, sau: next } } });
    return this.chuan_hoa_yeu_cau_doi_tra_v3290(updated);
  }

  async nhan_hang_doi_tra_v3290(actor: NguoiDungXacThuc, id: string, dto: { dong: Array<{ chi_tiet_doi_tra_id: string; so_luong_nhan: number }>; ghi_chu?: string }) {
    const current = await this.db.yeuCauDoiTra.findUnique({ where: { id }, include: { chi_tiet: true } });
    if (!current) throw new NotFoundException("Không tìm thấy yêu cầu đổi/trả");
    if (current.trang_thai !== TrangThaiYeuCauDoiTra.DA_DUYET) throw new ConflictException("Chỉ yêu cầu đã duyệt mới được xác nhận nhận hàng");
    const ids = dto.dong.map(x => x.chi_tiet_doi_tra_id);
    if (new Set(ids).size !== ids.length || ids.length !== current.chi_tiet.length) throw new BadRequestException("Phải gửi đúng một dòng nhận hàng cho mỗi dòng đổi/trả");
    const input = new Map(dto.dong.map(x => [x.chi_tiet_doi_tra_id, x.so_luong_nhan]));
    for (const line of current.chi_tiet) {
      const qty = input.get(line.id);
      if (qty == null) throw new BadRequestException("Thiếu dòng nhận hàng đổi/trả");
      if (qty > line.so_luong_y_cau) throw new BadRequestException(`Số lượng nhận của ${line.ma_bien_the || line.id} vượt số lượng đã duyệt`);
      if (line.nhap_lai_ton && qty > 0 && !line.bien_the_id) throw new ConflictException(`Dòng ${line.ma_bien_the || line.id} không còn liên kết biến thể để nhập lại tồn`);
    }
    await this.db.$transaction(async tx => {
      for (const line of current.chi_tiet) {
        const qty = input.get(line.id)!;
        await tx.chiTietDoiTra.update({ where: { id: line.id }, data: { so_luong_nhan: qty } });
        if (line.nhap_lai_ton && qty > 0 && line.bien_the_id) {
          const before = await tx.bienTheSanPham.findUnique({ where: { id: line.bien_the_id }, select: { so_luong_ton: true, ma_bien_the: true } });
          if (!before) throw new ConflictException(`Biến thể ${line.ma_bien_the || line.bien_the_id} không còn tồn tại`);
          await tx.bienTheSanPham.update({ where: { id: line.bien_the_id }, data: { so_luong_ton: { increment: qty } } });
          await tx.nhatKyBaoMat.create({ data: { loai_su_kien: "ADMIN_NHAP_LAI_TON_DOI_TRA", nguoi_dung_id: actor.id, chi_tiet: { yeu_cau_id: id, ma_yeu_cau: current.ma_yeu_cau, bien_the_id: line.bien_the_id, ma_bien_the: before.ma_bien_the, ton_cu: before.so_luong_ton, ton_moi: before.so_luong_ton + qty, chenh_lech: qty, loai_bien_dong: "NHAP_KHO", ly_do: dto.ghi_chu?.trim() || `Nhập lại tồn từ ${current.ma_yeu_cau}` } } });
        }
      }
      await tx.yeuCauDoiTra.update({ where: { id }, data: { trang_thai: TrangThaiYeuCauDoiTra.DA_NHAN_HANG, ngay_nhan_hang: new Date(), ghi_chu: dto.ghi_chu?.trim() ? `${current.ghi_chu || ""}${current.ghi_chu ? "\n" : ""}${dto.ghi_chu.trim()}` : current.ghi_chu } });
    });
    await this.db.nhatKyBaoMat.create({ data: { loai_su_kien: "ADMIN_NHAN_HANG_DOI_TRA", nguoi_dung_id: actor.id, chi_tiet: { yeu_cau_id: id, ma_yeu_cau: current.ma_yeu_cau, tong_nhan: dto.dong.reduce((sum, x) => sum + x.so_luong_nhan, 0), atomic_transaction: true } } });
    const list = await this.danh_sach_doi_tra_v3290();
    return list.items.find((x: any) => x.id === id);
  }

  async xac_nhan_hoan_tien_mot_phan_v3290(actor: NguoiDungXacThuc, id: string, so_tien: number, ghi_chu?: string) {
    const current = await this.db.yeuCauDoiTra.findUnique({ where: { id }, include: { hoan_tien: true, don_hang: { include: { thanh_toan: { orderBy: { ngay_tao: "desc" } } } } } });
    if (!current) throw new NotFoundException("Không tìm thấy yêu cầu đổi/trả");
    if (current.trang_thai !== TrangThaiYeuCauDoiTra.DA_NHAN_HANG) throw new ConflictException("Chỉ xác nhận hoàn tiền sau khi đã nhận hàng trả");
    const daHoanRma = current.hoan_tien.reduce((sum, x) => sum + Number(x.so_tien), 0);
    const conRma = Math.max(0, Number(current.tong_tien_hoan_duyet) - daHoanRma);
    if (so_tien > conRma + 0.0001) throw new BadRequestException(`Số tiền hoàn vượt phần còn lại của yêu cầu (${conRma.toLocaleString("vi-VN")}đ)`);
    const payment = current.don_hang.thanh_toan.find(x => x.trang_thai === TrangThaiThanhToan.DA_THANH_TOAN) || current.don_hang.thanh_toan.find(x => x.trang_thai === TrangThaiThanhToan.DA_HOAN_TIEN);
    if (!payment) throw new ConflictException("Đơn hàng không có giao dịch đã thanh toán để đối soát hoàn tiền");
    const orderRefunds = await this.db.hoanTienDonHang.aggregate({ where: { don_hang_id: current.don_hang_id }, _sum: { so_tien: true } });
    const daHoanDon = Number(orderRefunds._sum.so_tien || 0);
    const paymentAmount = Number(payment.so_tien);
    if (daHoanDon + so_tien > paymentAmount + 0.0001) throw new ConflictException("Tổng tiền hoàn sẽ vượt số tiền đã thanh toán của đơn");
    const note = ghi_chu?.trim() || `Admin xác nhận hoàn tiền thực tế cho ${current.ma_yeu_cau}`;
    const result = await this.db.$transaction(async tx => {
      const refund = await tx.hoanTienDonHang.create({ data: { don_hang_id: current.don_hang_id, yeu_cau_doi_tra_id: id, thanh_toan_id: payment.id, so_tien, ghi_chu: note, nguoi_xac_nhan_id: actor.id } });
      const newOrderRefund = daHoanDon + so_tien;
      if (payment.trang_thai === TrangThaiThanhToan.DA_THANH_TOAN && newOrderRefund + 0.0001 >= paymentAmount) await tx.thanhToan.update({ where: { id: payment.id }, data: { trang_thai: TrangThaiThanhToan.DA_HOAN_TIEN, noi_dung: `${payment.noi_dung || current.don_hang.ma_don_hang} · đã hoàn đủ qua đối soát v3.29` } });
      const newRmaRefund = daHoanRma + so_tien;
      if (newRmaRefund + 0.0001 >= Number(current.tong_tien_hoan_duyet)) await tx.yeuCauDoiTra.update({ where: { id }, data: { trang_thai: TrangThaiYeuCauDoiTra.HOAN_TAT, ngay_hoan_tat: new Date() } });
      return refund;
    });
    await this.db.nhatKyBaoMat.create({ data: { loai_su_kien: "ADMIN_HOAN_TIEN_MOT_PHAN", nguoi_dung_id: actor.id, chi_tiet: { yeu_cau_id: id, ma_yeu_cau: current.ma_yeu_cau, don_hang_id: current.don_hang_id, so_tien, tong_da_hoan_rma: daHoanRma + so_tien, tong_duyet_rma: Number(current.tong_tien_hoan_duyet), gateway_auto_refund: false, refund_ledger_id: result.id } } });
    const list = await this.danh_sach_doi_tra_v3290();
    return list.items.find((x: any) => x.id === id);
  }

  async xuat_doi_tra_excel_v3290() {
    const data = await this.danh_sach_doi_tra_v3290();
    const rows: unknown[][] = [["Mã RMA", "Mã đơn", "Trạng thái", "Lý do", "Sản phẩm", "Mã biến thể", "SL yêu cầu", "SL nhận", "Xử lý", "Nhập lại tồn", "Hoàn duyệt", "Đã hoàn", "Ngày tạo"]];
    for (const rma of data.items as any[]) for (const line of rma.chi_tiet) rows.push([rma.ma_yeu_cau, rma.don_hang.ma_don_hang, rma.trang_thai, rma.ly_do, line.chi_tiet_don_hang.ten_san_pham, line.ma_bien_the || "", line.so_luong_y_cau, line.so_luong_nhan, line.xu_ly, line.nhap_lai_ton ? "Có" : "Không", line.so_tien_hoan_duyet, rma.tong_tien_da_hoan, new Date(rma.ngay_tao).toISOString()]);
    const buffer = this.tao_xlsx(rows, "Đổi trả v3.29");
    return { ten_file: `doi-tra-rma-v3.29.0_${new Date().toISOString().slice(0,10)}.xlsx`, mime_type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", base64: buffer.toString("base64"), tong_yeu_cau: data.items.length };
  }

  async xuat_loi_nhuan_excel_v3290() {
    const tq = await this.tong_quan();
    const rows: unknown[][] = [["Kỳ", "Doanh thu gộp", "Hoàn tiền", "Doanh thu thuần", "Giá vốn", "Lợi nhuận gộp", "Biên lợi nhuận %", "Dòng thiếu giá vốn"]];
    const add = (name: string, x: any) => rows.push([name, x.doanh_thu_gop, x.hoan_tien, x.doanh_thu_thuan, x.gia_von, x.loi_nhuan_gop, x.bien_loi_nhuan_percent ?? "", x.dong_thieu_gia_von]);
    add("Hôm nay", tq.loi_nhuan.hom_nay); add("7 ngày", tq.loi_nhuan.bay_ngay); add("30 ngày", tq.loi_nhuan.ba_muoi_ngay);
    const buffer = this.tao_xlsx(rows, "Lợi nhuận v3.29");
    return { ten_file: `loi-nhuan-gop-v3.29.0_${new Date().toISOString().slice(0,10)}.xlsx`, mime_type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", base64: buffer.toString("base64") };
  }
