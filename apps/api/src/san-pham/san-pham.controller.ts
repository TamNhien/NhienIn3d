...
export class SanPhamController {
  ...
  private kemDiemDanhGia<T extends { danh_gia?: { so_sao: number }[] }>(item: T) {
    const danh_gia = item.danh_gia ?? [];
    const so_luong_danh_gia = danh_gia.length;
    const diem_danh_gia = so_luong_danh_gia ? Number((danh_gia.reduce((tong, x) => tong + x.so_sao, 0) / so_luong_danh_gia).toFixed(1)) : 0;
    const { danh_gia: _bo, ...san_pham } = item;
    return { ...san_pham, diem_danh_gia, so_luong_danh_gia };
  }
  ...
  @ApiQuery({ name: "sap_xep", required: false, enum: ["ma_tang", "moi_nhat", "gia_tang", "gia_giam", "ten_az"] })
  ...
  async danh_sach(
    ...
    if (!["ma_tang", "moi_nhat", "gia_tang", "gia_giam", "ten_az"].includes(sap_xep)) throw new BadRequestException("sap_xep không hợp lệ");
    ...
    const orderBy: Prisma.SanPhamOrderByWithRelationInput[] = sap_xep === "gia_tang"
      ...
      : sap_xep === "gia_giam" ? [{ gia_ban: "desc" }, { ten_san_pham: "asc" }]
      : sap_xep === "ten_az" ? [{ ten_san_pham: "asc" }]
      : sap_xep === "moi_nhat" ? [{ ngay_tao: "desc" }]
      : [{ ngay_tao: "asc" }];

    const raw = await this.db.sanPham.findMany({
      where,
      include: {
        danh_muc: true,
        hinh_anh: { orderBy: { thu_tu: "asc" } },
        bien_the: { where: { dang_hien_thi: true }, include: { vat_lieu: true, mau_sac: true }, orderBy: { ma_bien_the: "asc" } },
        danh_gia: { where: { da_duyet: true }, select: { so_sao: true } }
      },
      orderBy,
      take: gioi_han_so
    });
    const ket_qua = raw.map(item => this.kemDiemDanhGia(item));
    if (sap_xep !== "ma_tang") return ket_qua;
    const soThuTu = (ma: string) => Number(ma.match(/-(\d+)$/)?.[1] ?? Number.MAX_SAFE_INTEGER);
    return ket_qua.sort((a, b) => soThuTu(a.ma_san_pham) - soThuTu(b.ma_san_pham) || a.ma_san_pham.localeCompare(b.ma_san_pham));
  }

  @Get(":duong_dan/lien-quan")
  async lien_quan(@Param("duong_dan") duong_dan: string) {
    const hien_tai = await this.db.sanPham.findUnique({ where: { duong_dan }, select: { id: true, danh_muc_id: true } });
    if (!hien_tai) throw new NotFoundException("Không tìm thấy sản phẩm");
    const raw = await this.db.sanPham.findMany({
      where: { id: { not: hien_tai.id }, trang_thai: "DANG_BAN", danh_muc_id: hien_tai.danh_muc_id },
      include: { hinh_anh: { orderBy: { thu_tu: "asc" } }, bien_the: { where: { dang_hien_thi: true }, include: { vat_lieu: true, mau_sac: true } }, danh_gia: { where: { da_duyet: true }, select: { so_sao: true } } },
      orderBy: { ngay_tao: "asc" }, take: 4
    });
    return raw.map(item => this.kemDiemDanhGia(item));
  }

  @Get(":duong_dan")
  async chi_tiet(@Param("duong_dan") duong_dan: string) {
    const raw = await this.db.sanPham.findUnique({
      where: { duong_dan },
      include: {
        danh_muc: true,
        hinh_anh: { orderBy: { thu_tu: "asc" } },
    ...
    if (!raw || raw.trang_thai !== "DANG_BAN") throw new NotFoundException("Không tìm thấy sản phẩm");
    return this.kemDiemDanhGia(raw);
  }
}
