...
export default function OpsDashboardPage() {
  ...
  async function ackDeadLetter(item: WebhookDeadLetterAdmin) {
    ...
    try { await acknowledgeWebhookDeadLetterAdmin(item.id, "Đã xác nhận xử lý từ Ops Dashboard v3.18.0"); setThongBao(`Đã acknowledge dead-letter #${item.id}.`); await taiDuLieu(); }
    catch (error) { setLoi(error instanceof Error ? error.message : String(error)); }
    finally { setDangXuLy(""); }
  }
  ...
  return <main className={styles.page}>
    <header className={styles.header}>
      <div><span className={styles.kicker}>NHIENIN3D · OPS v3.18.0</span><h1>Ops Dashboard</h1><p>Target-time PITR rehearsal, health-gated canary auto-rollback và postmortem approval/action reminders; giữ archive portability, on-call, quorum, managed fleet và DLQ replay.</p>{access && <small className={styles.accessBadge}>{access.admin ? "ADMIN · SERVICE OWNER" : `${access.vai_tro_ops} · ${access.dich_vu.join(", ")}`}</small>}</div>
    ...
    <section className={`${styles.panel} ${styles.compactPanel}`}><div className={styles.panelHead}><div><h2>Probe desired-state · canary rollout</h2><p>Ed25519-signed desired-state + health gate; canary tự rollback khi fleet/quorum xấu sau grace window. Remote code execution luôn OFF.</p></div><small className={`${styles.accessBadge} ${styles.fleetStateBadge} ${desiredState?.current.paused || !desiredState?.signed_delivery ? styles.fleetWarn : styles.fleetReady}`}>{desiredState?.current.paused ? "PAUSED" : desiredState?.signed_delivery ? "ACTIVE" : "SIGNING REQUIRED"}</small></div><div className={styles.fleetStats}><div><span>Revision</span><b>{desiredState?.current.revision ?? 0}</b></div><div><span>Target</span><b>{desiredState?.current.target_version ?? "3.18.0"}</b></div><div><span>Rollout</span><b>{desiredState?.current.rollout_percent ?? 0}%</b></div><div><span>Interval</span><b>{desiredState?.current.interval_seconds ?? 300}s</b></div></div>{access?.admin && <div className={styles.assignmentForm}><input aria-label="Desired target version" value={desiredForm.target_version} onChange={e => setDesiredForm(x => ({ ...x, target_version: e.target.value }))}/><input aria-label="Desired interval seconds" type="number" min={30} max={3600} value={desiredForm.interval_seconds} onChange={e => setDesiredForm(x => ({ ...x, interval_seconds: Number(e.target.value) }))}/><input aria-label="Desired rollout percent" type="number" min={0} max={100} value={desiredForm.rollout_percent} onChange={e => setDesiredForm(x => ({ ...x, rollout_percent: Number(e.target.value) }))}/><input aria-label="Desired canary agents" placeholder="agent-hcm-01,agent-local" value={desiredForm.canary_agents} onChange={e => setDesiredForm(x => ({ ...x, canary_agents: e.target.value }))}/><label className={styles.quorumHint}><input type="checkbox" checked={desiredForm.paused} onChange={e => setDesiredForm(x => ({ ...x, paused: e.target.checked }))}/> Paused</label><button onClick={() => void saveDesiredState()} disabled={dangXuLy === "desired-state-save"}>Lưu rollout</button><button className={styles.secondary} onClick={() => void rollbackDesiredState()} disabled={!desiredState?.previous || dangXuLy === "desired-state-rollback"}>Rollback</button></div>}<p className={styles.quorumHint}>Canary: {desiredState?.current.canary_agents.join(", ") || "—"} · signed delivery {desiredState?.signed_delivery ? "ON" : "OFF"} · health gate {runtime?.probe_health_gate?.status || "—"} ({runtime?.probe_health_gate?.online_percent ?? 0}% online / {runtime?.probe_health_gate?.quorum_failures ?? 0} quorum fail) · auto rollback {runtime?.probe_health_gate?.auto_rollback ? "ON" : "OFF"} · remote code execution: OFF.</p></section>
    <section className={`${styles.panel} ${styles.compactPanel}`}><div className={styles.panelHead}><div><h2>Recovery readiness · RPO/RTO</h2><p>Đọc trạng thái PostgreSQL/WAL + logical drill; v3.18 thêm target-time PITR rehearsal opt-in trên cluster restore cô lập.</p></div><small className={`${styles.accessBadge} ${styles.fleetStateBadge} ${runtime?.database_recovery?.pitr_ready && runtime?.database_recovery?.drill_fresh ? styles.fleetReady : styles.fleetWarn}`}>{runtime?.database_recovery?.pitr_ready ? "PITR READY" : "LOGICAL READY"}</small></div><div className={styles.fleetStats}><div><span>RPO</span><b>{runtime?.database_recovery?.observed_rpo_minutes ?? "—"}m</b><small>target {runtime?.database_recovery?.rpo_target_minutes ?? 60}m</small></div><div><span>RTO drill</span><b>{runtime?.database_recovery?.observed_rto_seconds ?? "—"}s</b><small>target {runtime?.database_recovery?.rto_target_minutes ?? 30}m</small></div><div><span>WAL archive</span><b>{runtime?.database_recovery?.wal_archive.archive_mode ?? "unknown"}</b><small>{runtime?.database_recovery?.wal_archive.wal_level ?? "—"}</small></div><div><span>Drill</span><b>{runtime?.database_recovery?.drill_fresh ? "FRESH" : "DUE"}</b><small>{runtime?.database_recovery?.drill_age_days ?? "—"} day(s)</small></div></div><p className={styles.quorumHint}>Method: {runtime?.database_recovery?.method ?? "—"} · RPO {runtime?.database_recovery?.rpo_met === true ? "PASS" : runtime?.database_recovery?.rpo_met === false ? "MISS" : "N/A"} · RTO {runtime?.database_recovery?.rto_met === true ? "PASS" : runtime?.database_recovery?.rto_met === false ? "MISS" : "N/A"} · target-time PITR exercised: {runtime?.database_recovery?.pitr_restore_exercised ? "YES" : "NO"} · opt-in drill {runtime?.database_recovery?.pitr_drill_opt_in ? "SUPPORTED" : "OFF"}.</p></section>
