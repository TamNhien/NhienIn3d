...
import {
  ...
  rollbackProbeDesiredStateAdmin, approveProbeRollbackAdmin, approveProbeRolloutAdmin, rejectProbeRolloutAdmin, cancelProbeRolloutAdmin,
  xuatRecoveryEvidenceBundleAdmin, verifyRecoveryEvidenceAdmin, xuatRemediationBacklogExcelAdmin, escalateRemediationBacklogAdmin, acknowledgeRemediationBacklogAdmin,
  layIncidentPostmortemAdmin,
  luuIncidentPostmortemAdmin, duyetIncidentPostmortemAdmin,
  layDlqKeyringAdmin,
...
export default function OpsDashboardPage() {
  ...
  const [desiredForm, setDesiredForm] = useState({ target_version: "3.25.0", interval_seconds: 300, rollout_percent: 0, canary_agents: "", paused: true, note: "" });
  const [postmortem, setPostmortem] = useState<IncidentPostmortemAdmin | null>(null);
  const [postmortemForm, setPostmortemForm] = useState({ summary: "", impact: "", root_cause: "", detection: "", resolution: "", runbook_url: "", lessons: "", action_item: "", action_severity: "P3" as "P1" | "P2" | "P3" | "P4", action_service: "api" });
  ...
  async function exportRecoveryEvidence() {
    setDangXuLy("recovery-evidence"); setLoi("");
    try { const kq = await xuatRecoveryEvidenceBundleAdmin(); taiTep(kq); setThongBao(`Đã tải ${kq.ten_file}.`); }
  ...
  async function verifyRecoveryEvidence() {
    setDangXuLy("recovery-verify"); setLoi("");
    try { const kq = await verifyRecoveryEvidenceAdmin(); setThongBao(`Recovery evidence: ${kq.overall_verified ? "VERIFIED" : "FAILED"} · SHA-256 ${kq.sha256_valid ? "PASS" : "FAIL"} · Ed25519 ${kq.signature_configured ? (kq.signature_valid && kq.fingerprint_valid ? "VERIFIED" : "FAIL") : "OPTIONAL"}.`); await taiDuLieu(); }
    catch (error) { setLoi(error instanceof Error ? error.message : String(error)); } finally { setDangXuLy(""); }
  }

  async function exportRemediation() {
    setDangXuLy("remediation-export"); setLoi("");
    try { const kq = await xuatRemediationBacklogExcelAdmin(); taiTep(kq); setThongBao(`Đã xuất ${kq.ten_file} · SLA breach ${kq.sla_breached}.`); }
    catch (error) { setLoi(error instanceof Error ? error.message : String(error)); } finally { setDangXuLy(""); }
  }

  async function escalateRemediation() {
    if (!confirm("Gửi escalation remediation SLA đang breach tới on-call hiện tại?")) return;
    setDangXuLy("remediation-escalate"); setLoi("");
    try { const kq = await escalateRemediationBacklogAdmin(); setThongBao(`Remediation escalation: ${kq.sent} action đã route, ${kq.skipped} bỏ qua.`); await taiDuLieu(); }
    catch (error) { setLoi(error instanceof Error ? error.message : String(error)); } finally { setDangXuLy(""); }
  }

  async function acknowledgeRemediation(service: string) {
    if (!confirm(`Acknowledge remediation SLA breach của ${service} trong 24 giờ? Nếu có breach mới, ack sẽ tự mất hiệu lực.`)) return;
    setDangXuLy(`remediation-ack-${service}`); setLoi("");
    try { const kq = await acknowledgeRemediationBacklogAdmin(service, "Acknowledged from Ops Dashboard v3.25.0", 24); setThongBao(`Đã acknowledge ${service} tới ${fmt(kq.snooze_until)}.`); await taiDuLieu(); }
    catch (error) { setLoi(error instanceof Error ? error.message : String(error)); } finally { setDangXuLy(""); }
  }
  ...
  async function ackDeadLetter(item: WebhookDeadLetterAdmin) {
    ...
    try { await acknowledgeWebhookDeadLetterAdmin(item.id, "Đã xác nhận xử lý từ Ops Dashboard v3.25.0"); setThongBao(`Đã acknowledge dead-letter #${item.id}.`); await taiDuLieu(); }
    catch (error) { setLoi(error instanceof Error ? error.message : String(error)); }
    finally { setDangXuLy(""); }
  }
  ...
  async function saveDesiredState() {
    ...
    try {
      ...
      if (kq.pending_approval) { setThongBao("Đã tạo production rollout proposal; cần người thứ hai duyệt trước TTL."); await taiDuLieu(); return; }
  ...
  async function loadPostmortem(chuKy: string) {
    ...
    try { const kq = await layIncidentPostmortemAdmin(chuKy); const incidentService = incidents.find(x => x.chu_ky === chuKy)?.dich_vu || "api"; const suggestedRunbook = runtime?.service_runbooks?.runbooks?.[incidentService] || ""; setPostmortem(kq); const openAction = kq.action_items.find(x => x.status !== "DONE"); setPostmortemForm({ summary: kq.summary || "", impact: kq.impact || "", root_cause: kq.root_cause || "", detection: kq.detection || "", resolution: kq.resolution || "", runbook_url: kq.runbook_url || suggestedRunbook, lessons: kq.lessons || "", action_item: openAction?.title || "", action_severity: openAction?.severity || "P3", action_service: openAction?.service || incidentService }); }
    catch (error) { setLoi(error instanceof Error ? error.message : String(error)); }
  }
  ...
  async function savePostmortem() {
    ...
    try { const existing = postmortem?.action_items || []; const actions = postmortemForm.action_item.trim() ? [...existing.filter(x => x.title !== postmortemForm.action_item.trim()), { id: "ui-action", title: postmortemForm.action_item.trim(), owner: "", status: "OPEN" as const, due_date: null, severity: postmortemForm.action_severity, service: postmortemForm.action_service.trim() || "api" }] : existing; const kq = await luuIncidentPostmortemAdmin(selectedIncident, { summary: postmortemForm.summary, impact: postmortemForm.impact, root_cause: postmortemForm.root_cause, detection: postmortemForm.detection, resolution: postmortemForm.resolution, runbook_url: postmortemForm.runbook_url, lessons: postmortemForm.lessons, action_items: actions }); setPostmortem(kq); setThongBao(`Đã lưu postmortem ${kq.status}.`); await taiDuLieu(); }
    catch (error) { setLoi(error instanceof Error ? error.message : String(error)); } finally { setDangXuLy(""); }
  }

  async function approveProbeRollback() {
    setDangXuLy("probe-rollback-approve"); setLoi("");
    try { await approveProbeRollbackAdmin("Approved from Ops Dashboard v3.25.0"); setThongBao("Đã duyệt auto rollback cho revision hiện tại."); await taiDuLieu(); }
    catch (error) { setLoi(error instanceof Error ? error.message : String(error)); }
    finally { setDangXuLy(""); }
  }

  async function approveProbeRollout() {
    const proposal = runtime?.rollout_approval?.proposal; if (!proposal || proposal.status !== "PENDING") return;
    setDangXuLy("probe-rollout-approve"); setLoi("");
    try { const kq = await approveProbeRolloutAdmin(proposal.id, "Approved from Ops Dashboard v3.25.0"); setDesiredState(kq); setThongBao("Đã duyệt production rollout theo TTL + two-person rule."); await taiDuLieu(); }
    catch (error) { setLoi(error instanceof Error ? error.message : String(error)); } finally { setDangXuLy(""); }
  }

  async function rejectProbeRollout() {
    const proposal = runtime?.rollout_approval?.proposal; if (!proposal || proposal.status !== "PENDING") return;
    if (!confirm("Từ chối production rollout proposal đang chờ?")) return;
    setDangXuLy("probe-rollout-reject"); setLoi("");
    try { await rejectProbeRolloutAdmin(proposal.id, "Rejected from Ops Dashboard v3.25.0"); setThongBao("Đã từ chối production rollout proposal."); await taiDuLieu(); }
    catch (error) { setLoi(error instanceof Error ? error.message : String(error)); } finally { setDangXuLy(""); }
  }

  async function cancelProbeRollout() {
    const proposal = runtime?.rollout_approval?.proposal; if (!proposal || proposal.status !== "PENDING") return;
    if (!confirm("Hủy production rollout proposal do bạn đã tạo?")) return;
    setDangXuLy("probe-rollout-cancel"); setLoi("");
    try { await cancelProbeRolloutAdmin(proposal.id, "Cancelled from Ops Dashboard v3.25.0"); setThongBao("Đã hủy production rollout proposal."); await taiDuLieu(); }
    catch (error) { setLoi(error instanceof Error ? error.message : String(error)); } finally { setDangXuLy(""); }
  }

  async function approvePostmortem(decision: "APPROVED" | "CHANGES_REQUESTED") {
    if (!selectedIncident || !postmortem || postmortem.status !== "COMPLETE") return;
    setDangXuLy("postmortem-approval"); setLoi("");
    try { const kq = await duyetIncidentPostmortemAdmin(selectedIncident, decision); setPostmortem(kq); setThongBao(decision === "APPROVED" ? "Đã duyệt postmortem." : "Đã yêu cầu chỉnh sửa postmortem."); await taiDuLieu(); }
    catch (error) { setLoi(error instanceof Error ? error.message : String(error)); } finally { setDangXuLy(""); }
  }
  ...
  return <main className={styles.page}>
    <header className={styles.header}>
      <div><span className={styles.kicker}>NHIENIN3D · OPS v3.25.0</span><h1>Ops Dashboard</h1><p>Rollout approval có proposal/envelope/decision receipt SHA-256 + receipt hash chain; recovery evidence có trusted-key pinning, key rotation và revoked-key fail-closed; remediation ack/retry/escalation tiếp tục được giữ.</p>{access && <small className={styles.accessBadge}>{access.admin ? "ADMIN · SERVICE OWNER" : `${access.vai_tro_ops} · ${access.dich_vu.join(", ")}`}</small>}</div>
    ...
    <section className={`${styles.panel} ${styles.compactPanel}`}><div className={styles.panelHead}><div><h2>Probe desired-state · canary rollout</h2><p>Ed25519-signed desired-state + health gate; production rollout có TTL, two-person, SHA-256 proposal + immutable envelope + decision receipt + tamper-evident receipt chain, health preflight và approve/reject/cancel. Remote code execution luôn OFF.</p></div><small className={`${styles.accessBadge} ${styles.fleetStateBadge} ${desiredState?.current.paused || !desiredState?.signed_delivery ? styles.fleetWarn : styles.fleetReady}`}>{desiredState?.current.paused ? "PAUSED" : desiredState?.signed_delivery ? "ACTIVE" : "SIGNING REQUIRED"}</small></div><div className={styles.fleetStats}><div><span>Revision</span><b>{desiredState?.current.revision ?? 0}</b></div><div><span>Target</span><b>{desiredState?.current.target_version ?? "3.25.0"}</b></div><div><span>Rollout</span><b>{desiredState?.current.rollout_percent ?? 0}%</b></div><div><span>Interval</span><b>{desiredState?.current.interval_seconds ?? 300}s</b></div></div>{access?.admin && <div className={styles.assignmentForm}><input aria-label="Desired target version" value={desiredForm.target_version} onChange={e => setDesiredForm(x => ({ ...x, target_version: e.target.value }))}/><input aria-label="Desired interval seconds" type="number" min={30} max={3600} value={desiredForm.interval_seconds} onChange={e => setDesiredForm(x => ({ ...x, interval_seconds: Number(e.target.value) }))}/><input aria-label="Desired rollout percent" type="number" min={0} max={100} value={desiredForm.rollout_percent} onChange={e => setDesiredForm(x => ({ ...x, rollout_percent: Number(e.target.value) }))}/><input aria-label="Desired canary agents" placeholder="agent-hcm-01,agent-local" value={desiredForm.canary_agents} onChange={e => setDesiredForm(x => ({ ...x, canary_agents: e.target.value }))}/><label className={styles.quorumHint}><input type="checkbox" checked={desiredForm.paused} onChange={e => setDesiredForm(x => ({ ...x, paused: e.target.checked }))}/> Paused</label><button onClick={() => void saveDesiredState()} disabled={dangXuLy === "desired-state-save"}>Lưu rollout</button><button className={styles.secondary} onClick={() => void rollbackDesiredState()} disabled={!desiredState?.previous || dangXuLy === "desired-state-rollback"}>Rollback</button>{runtime?.rollout_approval?.proposal?.status === "PENDING" && <><button onClick={() => void approveProbeRollout()} disabled={dangXuLy === "probe-rollout-approve" || (runtime?.rollout_approval?.proposal_sha256_valid === false || runtime?.rollout_approval?.proposal_envelope_sha256_valid === false)}>Duyệt production rollout</button><button className={styles.secondary} onClick={() => void rejectProbeRollout()} disabled={dangXuLy === "probe-rollout-reject"}>Từ chối</button><button className={styles.danger} onClick={() => void cancelProbeRollout()} disabled={dangXuLy === "probe-rollout-cancel"}>Hủy proposal</button></>}{runtime?.probe_health_gate?.rollback_approval_required && !runtime?.probe_health_gate?.rollback_approved_for_revision && <button className={styles.secondary} onClick={() => void approveProbeRollback()} disabled={dangXuLy === "probe-rollback-approve"}>Duyệt auto rollback</button>}</div>}<p className={styles.quorumHint}>Canary: {desiredState?.current.canary_agents.join(", ") || "—"} · signed delivery {desiredState?.signed_delivery ? "ON" : "OFF"} · health gate {runtime?.probe_health_gate?.status || "—"} ({runtime?.probe_health_gate?.online_percent ?? 0}% online / {runtime?.probe_health_gate?.quorum_failures ?? 0} quorum fail / burn {runtime?.probe_health_gate?.burn_rate_max_observed ?? "—"}x) · cooldown {runtime?.probe_health_gate?.cooldown_remaining_minutes ?? 0}m · rollback approval {runtime?.probe_health_gate?.rollback_approval_required ? (runtime?.probe_health_gate?.rollback_approved_for_revision ? "APPROVED" : "REQUIRED") : "N/A"} · rollout proposal {runtime?.rollout_approval?.proposal?.status || "NONE"} / TTL {runtime?.rollout_approval?.ttl_minutes ?? 30}m / two-person {runtime?.rollout_approval?.two_person_rule ? "ON" : "OFF"} / SHA-256 {runtime?.rollout_approval?.proposal_sha256_valid === false ? "FAIL" : runtime?.rollout_approval?.proposal ? "PASS" : "N/A"} / envelope {runtime?.rollout_approval?.proposal_envelope_sha256_valid === false ? "FAIL" : runtime?.rollout_approval?.proposal ? "PASS" : "N/A"} / receipt {runtime?.rollout_approval?.decision_receipt_sha256_valid === false ? "FAIL" : runtime?.rollout_approval?.proposal?.decision_receipt_sha256 ? "PASS" : "PENDING"} / chain {runtime?.rollout_approval?.receipt_chain_valid === false ? `FAIL:${runtime?.rollout_approval?.receipt_chain_reason || "INVALID"}` : `PASS:${runtime?.rollout_approval?.receipt_chain_length ?? 0}`} / health preflight {runtime?.rollout_approval?.require_healthy_preflight ? "ON" : "OFF"} · auto rollback {runtime?.probe_health_gate?.auto_rollback ? "ON" : "OFF"} · remote code execution: OFF.</p></section>
    <section className={`${styles.panel} ${styles.compactPanel}`}><div className={styles.panelHead}><div><h2>Recovery readiness · RPO/RTO</h2><p>Đọc PostgreSQL/WAL + PITR drill; v3.23 pin trust anchor cho Ed25519, hỗ trợ rotate key và revoked-key denylist fail-closed; private key không vào source.</p></div><small className={`${styles.accessBadge} ${styles.fleetStateBadge} ${runtime?.database_recovery?.pitr_ready && runtime?.database_recovery?.drill_fresh ? styles.fleetReady : styles.fleetWarn}`}>{runtime?.database_recovery?.pitr_ready ? "PITR READY" : "LOGICAL READY"}</small></div><div className={styles.fleetStats}><div><span>RPO</span><b>{runtime?.database_recovery?.observed_rpo_minutes ?? "—"}m</b><small>target {runtime?.database_recovery?.rpo_target_minutes ?? 60}m</small></div><div><span>RTO drill</span><b>{runtime?.database_recovery?.observed_rto_seconds ?? "—"}s</b><small>target {runtime?.database_recovery?.rto_target_minutes ?? 30}m</small></div><div><span>WAL archive</span><b>{runtime?.database_recovery?.wal_archive.archive_mode ?? "unknown"}</b><small>{runtime?.database_recovery?.wal_archive.wal_level ?? "—"}</small></div><div><span>Drill</span><b>{runtime?.database_recovery?.drill_fresh ? "FRESH" : "DUE"}</b><small>{runtime?.database_recovery?.drill_age_days ?? "—"} day(s)</small></div></div>{access?.admin && <div className={styles.inlineActions}><button className={styles.secondary} onClick={() => void verifyRecoveryEvidence()} disabled={dangXuLy === "recovery-verify"}>Verify evidence</button><button onClick={() => void exportRecoveryEvidence()} disabled={dangXuLy === "recovery-evidence" || !runtime?.database_recovery?.audit_bundle_ready}>Tải recovery audit bundle</button></div>}<p className={styles.quorumHint}>Method: {runtime?.database_recovery?.method ?? "—"} · RPO {runtime?.database_recovery?.rpo_met === true ? "PASS" : runtime?.database_recovery?.rpo_met === false ? "MISS" : "N/A"} · RTO {runtime?.database_recovery?.rto_met === true ? "PASS" : runtime?.database_recovery?.rto_met === false ? "MISS" : "N/A"} · target-time PITR exercised: {runtime?.database_recovery?.pitr_restore_exercised ? "YES" : "NO"} · evidence history {runtime?.database_recovery?.evidence_history_count ?? 0}/{runtime?.database_recovery?.evidence_retention_runs ?? 30} · bundle {runtime?.database_recovery?.evidence_current_version ? "v3.23 CURRENT" : runtime?.database_recovery?.evidence_bundle_version || "MISSING"} · SHA-256 {runtime?.database_recovery?.evidence_sha256_verified ? "VERIFIED" : runtime?.database_recovery?.evidence_sha256 ? "FAIL" : "MISSING"} · Ed25519 {runtime?.database_recovery?.evidence_signature_verified ? "VERIFIED" : runtime?.database_recovery?.evidence_ed25519_configured ? "FAIL" : "OPTIONAL"} · trust anchor {runtime?.database_recovery?.evidence_ed25519_configured ? (runtime?.database_recovery?.evidence_key_revoked ? "REVOKED" : runtime?.database_recovery?.evidence_key_trusted ? `${runtime?.database_recovery?.evidence_trust_source || "TRUSTED"}` : "UNTRUSTED") : "N/A"} · revoked-key policy {runtime?.database_recovery?.evidence_revocation_configured ? `ACTIVE:${runtime?.database_recovery?.evidence_revoked_fingerprints ?? 0}` : "EMPTY"} · opt-in drill {runtime?.database_recovery?.pitr_drill_opt_in ? "SUPPORTED" : "OFF"}.</p></section>
    <section className={`${styles.panel} ${styles.compactPanel}`}><div className={styles.panelHead}><div><h2>Incident postmortem · runbook</h2><p>Draft/complete, approval workflow, action reminder; remediation v3.21 có SLA theo P1-P4, Excel và on-call escalation thực thi.</p></div><small className={`${styles.accessBadge} ${styles.postmortemStatusBadge}`}>{runtime?.incident_postmortem?.complete ?? 0} COMPLETE · {runtime?.incident_postmortem?.draft ?? 0} DRAFT</small></div>{selectedIncident && access?.admin ? <><div className={styles.assignmentForm}><input aria-label="Postmortem summary" placeholder="Summary" value={postmortemForm.summary} onChange={e => setPostmortemForm(x => ({ ...x, summary: e.target.value }))}/><input aria-label="Postmortem root cause" placeholder="Root cause" value={postmortemForm.root_cause} onChange={e => setPostmortemForm(x => ({ ...x, root_cause: e.target.value }))}/><input aria-label="Postmortem resolution" placeholder="Resolution" value={postmortemForm.resolution} onChange={e => setPostmortemForm(x => ({ ...x, resolution: e.target.value }))}/><input aria-label="Postmortem runbook" placeholder="https://..." value={postmortemForm.runbook_url} onChange={e => setPostmortemForm(x => ({ ...x, runbook_url: e.target.value }))}/><input aria-label="Postmortem action item" placeholder="Action item" value={postmortemForm.action_item} onChange={e => setPostmortemForm(x => ({ ...x, action_item: e.target.value }))}/><select aria-label="Postmortem action severity" value={postmortemForm.action_severity} onChange={e => setPostmortemForm(x => ({ ...x, action_severity: e.target.value as "P1" | "P2" | "P3" | "P4" }))}><option value="P1">P1 · 4h</option><option value="P2">P2 · 24h</option><option value="P3">P3 · 72h</option><option value="P4">P4 · 168h</option></select><input aria-label="Postmortem action service" placeholder="api" value={postmortemForm.action_service} onChange={e => setPostmortemForm(x => ({ ...x, action_service: e.target.value }))}/><button onClick={() => void savePostmortem()} disabled={dangXuLy === "postmortem-save"}>Lưu postmortem</button></div>{postmortem?.status === "COMPLETE" && <div className={styles.inlineActions}><small className={`${styles.accessBadge} ${styles.postmortemStatusBadge}`}>{postmortem.approval_status || "PENDING"}</small><button onClick={() => void approvePostmortem("APPROVED")} disabled={dangXuLy === "postmortem-approval"}>Duyệt</button><button className={styles.secondary} onClick={() => void approvePostmortem("CHANGES_REQUESTED")} disabled={dangXuLy === "postmortem-approval"}>Yêu cầu sửa</button></div>}</> : <p>Chọn một incident trong danh sách phía dưới để mở postmortem.</p>}<p className={styles.quorumHint}>Timeline {postmortem?.timeline_snapshot?.events ?? "—"} event(s) · open {runtime?.incident_postmortem?.open_actions ?? 0} · overdue {runtime?.incident_postmortem?.overdue_actions ?? 0} · pending approval {runtime?.incident_postmortem?.pending_approval ?? 0} · remediation open {runtime?.remediation_backlog?.open_actions ?? 0} / SLA breach {runtime?.remediation_backlog?.sla_breached_actions ?? 0} / due soon {runtime?.remediation_backlog?.sla_due_soon_actions ?? 0} / unowned {runtime?.remediation_backlog?.unowned_actions ?? 0} / ack {runtime?.remediation_backlog?.acknowledged_services ?? 0} service / retry {runtime?.remediation_backlog?.retry_pending_services ?? 0} · runbooks {runtime?.service_runbooks?.configured ? "READY" : "OPTIONAL"}.</p>{access?.admin && <><div className={styles.inlineActions}><button className={styles.secondary} onClick={() => void exportRemediation()} disabled={dangXuLy === "remediation-export"}>Xuất remediation Excel</button><button onClick={() => void escalateRemediation()} disabled={dangXuLy === "remediation-escalate" || !(runtime?.remediation_backlog?.sla_breached_actions ?? 0)}>Escalate SLA breach</button></div><div className={styles.inlineActions}>{Object.entries(runtime?.remediation_backlog?.service_status || {}).filter(([, st]) => (st.breached_actions || 0) > 0).map(([service, st]) => <button key={service} className={styles.secondary} onClick={() => void acknowledgeRemediation(service)} disabled={dangXuLy === `remediation-ack-${service}` || st.acknowledged}>Ack {service} 24h{st.escalation_level ? ` · L${st.escalation_level}` : ""}</button>)}</div></>}</section>
