...
import {
  ...
  rollbackProbeDesiredStateAdmin, approveProbeRollbackAdmin,
...
export default function OpsDashboardPage() {
  ...
  const [desiredForm, setDesiredForm] = useState({ target_version: "3.18.0", interval_seconds: 300, rollout_percent: 0, canary_agents: "", paused: true, note: "" });
  ...
  async function ackDeadLetter(item: WebhookDeadLetterAdmin) {
    ...
    try { await acknowledgeWebhookDeadLetterAdmin(item.id, "Đã xác nhận xử lý từ Ops Dashboard v3.19.0"); setThongBao(`Đã acknowledge dead-letter #${item.id}.`); await taiDuLieu(); }
    catch (error) { setLoi(error instanceof Error ? error.message : String(error)); }
    finally { setDangXuLy(""); }
  }
  ...
  async function loadPostmortem(chuKy: string) {
    ...
    try { const kq = await layIncidentPostmortemAdmin(chuKy); const incidentService = incidents.find(x => x.chu_ky === chuKy)?.dich_vu || "api"; const suggestedRunbook = runtime?.service_runbooks?.runbooks?.[incidentService] || ""; setPostmortem(kq); setPostmortemForm({ summary: kq.summary || "", impact: kq.impact || "", root_cause: kq.root_cause || "", detection: kq.detection || "", resolution: kq.resolution || "", runbook_url: kq.runbook_url || suggestedRunbook, lessons: kq.lessons || "", action_item: kq.action_items.find(x => x.status !== "DONE")?.title || "" }); }
    catch (error) { setLoi(error instanceof Error ? error.message : String(error)); }
  }
  ...
  async function approveProbeRollback() {
    ...
    try { await approveProbeRollbackAdmin("Approved from Ops Dashboard v3.19.0"); setThongBao("Đã duyệt auto rollback cho revision hiện tại."); await taiDuLieu(); }
    catch (error) { setLoi(error instanceof Error ? error.message : String(error)); }
    finally { setDangXuLy(""); }
  }
  ...
  return <main className={styles.page}>
    ...
    <section className={`${styles.panel} ${styles.compactPanel}`}><div className={styles.panelHead}><div><h2>Probe desired-state · canary rollout</h2><p>Ed25519-signed desired-state + health gate; canary tự rollback khi fleet/quorum/error-budget burn-rate xấu, có cooldown + production approval policy. Remote code execution luôn OFF.</p></div><small className={`${styles.accessBadge} ${styles.fleetStateBadge} ${desiredState?.current.paused || !desiredState?.signed_delivery ? styles.fleetWarn : styles.fleetReady}`}>{desiredState?.current.paused ? "PAUSED" : desiredState?.signed_delivery ? "ACTIVE" : "SIGNING REQUIRED"}</small></div><div className={styles.fleetStats}><div><span>Revision</span><b>{desiredState?.current.revision ?? 0}</b></div><div><span>Target</span><b>{desiredState?.current.target_version ?? "3.19.0"}</b></div><div><span>Rollout</span><b>{desiredState?.current.rollout_percent ?? 0}%</b></div><div><span>Interval</span><b>{desiredState?.current.interval_seconds ?? 300}s</b></div></div>{access?.admin && <div className={styles.assignmentForm}><input aria-label="Desired target version" value={desiredForm.target_version} onChange={e => setDesiredForm(x => ({ ...x, target_version: e.target.value }))}/><input aria-label="Desired interval seconds" type="number" min={30} max={3600} value={desiredForm.interval_seconds} onChange={e => setDesiredForm(x => ({ ...x, interval_seconds: Number(e.target.value) }))}/><input aria-label="Desired rollout percent" type="number" min={0} max={100} value={desiredForm.rollout_percent} onChange={e => setDesiredForm(x => ({ ...x, rollout_percent: Number(e.target.value) }))}/><input aria-label="Desired canary agents" placeholder="agent-hcm-01,agent-local" value={desiredForm.canary_agents} onChange={e => setDesiredForm(x => ({ ...x, canary_agents: e.target.value }))}/><label className={styles.quorumHint}><input type="checkbox" checked={desiredForm.paused} onChange={e => setDesiredForm(x => ({ ...x, paused: e.target.checked }))}/> Paused</label><button onClick={() => void saveDesiredState()} disabled={dangXuLy === "desired-state-save"}>Lưu rollout</button><button className={styles.secondary} onClick={() => void rollbackDesiredState()} disabled={!desiredState?.previous || dangXuLy === "desired-state-rollback"}>Rollback</button>{runtime?.probe_health_gate?.rollback_approval_required && !runtime?.probe_health_gate?.rollback_approved_for_revision && <button className={styles.secondary} onClick={() => void approveProbeRollback()} disabled={dangXuLy === "probe-rollback-approve"}>Duyệt auto rollback</button>}</div>}<p className={styles.quorumHint}>Canary: {desiredState?.current.canary_agents.join(", ") || "—"} · signed delivery {desiredState?.signed_delivery ? "ON" : "OFF"} · health gate {runtime?.probe_health_gate?.status || "—"} ({runtime?.probe_health_gate?.online_percent ?? 0}% online / {runtime?.probe_health_gate?.quorum_failures ?? 0} quorum fail / burn {runtime?.probe_health_gate?.burn_rate_max_observed ?? "—"}x) · cooldown {runtime?.probe_health_gate?.cooldown_remaining_minutes ?? 0}m · approval {runtime?.probe_health_gate?.rollback_approval_required ? (runtime?.probe_health_gate?.rollback_approved_for_revision ? "APPROVED" : "REQUIRED") : "N/A"} · auto rollback {runtime?.probe_health_gate?.auto_rollback ? "ON" : "OFF"} · remote code execution: OFF.</p></section>
    <section className={`${styles.panel} ${styles.compactPanel}`}><div className={styles.panelHead}><div><h2>Recovery readiness · RPO/RTO</h2><p>Đọc trạng thái PostgreSQL/WAL + logical drill; v3.18 thêm target-time PITR rehearsal opt-in trên cluster restore cô lập.</p></div><small className={`${styles.accessBadge} ${styles.fleetStateBadge} ${runtime?.database_recovery?.pitr_ready && runtime?.database_recovery?.drill_fresh ? styles.fleetReady : styles.fleetWarn}`}>{runtime?.database_recovery?.pitr_ready ? "PITR READY" : "LOGICAL READY"}</small></div><div className={styles.fleetStats}><div><span>RPO</span><b>{runtime?.database_recovery?.observed_rpo_minutes ?? "—"}m</b><small>target {runtime?.database_recovery?.rpo_target_minutes ?? 60}m</small></div><div><span>RTO drill</span><b>{runtime?.database_recovery?.observed_rto_seconds ?? "—"}s</b><small>target {runtime?.database_recovery?.rto_target_minutes ?? 30}m</small></div><div><span>WAL archive</span><b>{runtime?.database_recovery?.wal_archive.archive_mode ?? "unknown"}</b><small>{runtime?.database_recovery?.wal_archive.wal_level ?? "—"}</small></div><div><span>Drill</span><b>{runtime?.database_recovery?.drill_fresh ? "FRESH" : "DUE"}</b><small>{runtime?.database_recovery?.drill_age_days ?? "—"} day(s)</small></div></div><p className={styles.quorumHint}>Method: {runtime?.database_recovery?.method ?? "—"} · RPO {runtime?.database_recovery?.rpo_met === true ? "PASS" : runtime?.database_recovery?.rpo_met === false ? "MISS" : "N/A"} · RTO {runtime?.database_recovery?.rto_met === true ? "PASS" : runtime?.database_recovery?.rto_met === false ? "MISS" : "N/A"} · target-time PITR exercised: {runtime?.database_recovery?.pitr_restore_exercised ? "YES" : "NO"} · evidence history {runtime?.database_recovery?.evidence_history_count ?? 0}/{runtime?.database_recovery?.evidence_retention_runs ?? 30} · opt-in drill {runtime?.database_recovery?.pitr_drill_opt_in ? "SUPPORTED" : "OFF"}.</p></section>
    <section className={`${styles.panel} ${styles.compactPanel}`}><div className={styles.panelHead}><div><h2>Incident postmortem · runbook</h2><p>Draft/complete, approval workflow, action reminder và HTTPS runbook theo service dependency.</p></div><small className={`${styles.accessBadge} ${styles.postmortemStatusBadge}`}>{runtime?.incident_postmortem?.complete ?? 0} COMPLETE · {runtime?.incident_postmortem?.draft ?? 0} DRAFT</small></div>{selectedIncident && access?.admin ? <><div className={styles.assignmentForm}><input aria-label="Postmortem summary" placeholder="Summary" value={postmortemForm.summary} onChange={e => setPostmortemForm(x => ({ ...x, summary: e.target.value }))}/><input aria-label="Postmortem root cause" placeholder="Root cause" value={postmortemForm.root_cause} onChange={e => setPostmortemForm(x => ({ ...x, root_cause: e.target.value }))}/><input aria-label="Postmortem resolution" placeholder="Resolution" value={postmortemForm.resolution} onChange={e => setPostmortemForm(x => ({ ...x, resolution: e.target.value }))}/><input aria-label="Postmortem runbook" placeholder="https://..." value={postmortemForm.runbook_url} onChange={e => setPostmortemForm(x => ({ ...x, runbook_url: e.target.value }))}/><input aria-label="Postmortem action item" placeholder="Action item" value={postmortemForm.action_item} onChange={e => setPostmortemForm(x => ({ ...x, action_item: e.target.value }))}/><button onClick={() => void savePostmortem()} disabled={dangXuLy === "postmortem-save"}>Lưu postmortem</button></div>{postmortem?.status === "COMPLETE" && <div className={styles.inlineActions}><small className={`${styles.accessBadge} ${styles.postmortemStatusBadge}`}>{postmortem.approval_status || "PENDING"}</small><button onClick={() => void approvePostmortem("APPROVED")} disabled={dangXuLy === "postmortem-approval"}>Duyệt</button><button className={styles.secondary} onClick={() => void approvePostmortem("CHANGES_REQUESTED")} disabled={dangXuLy === "postmortem-approval"}>Yêu cầu sửa</button></div>}</> : <p>Chọn một incident trong danh sách phía dưới để mở postmortem.</p>}<p className={styles.quorumHint}>Timeline {postmortem?.timeline_snapshot?.events ?? "—"} event(s) · open {runtime?.incident_postmortem?.open_actions ?? 0} · overdue {runtime?.incident_postmortem?.overdue_actions ?? 0} · pending approval {runtime?.incident_postmortem?.pending_approval ?? 0} · remediation open {runtime?.remediation_backlog?.open_actions ?? 0} / overdue {runtime?.remediation_backlog?.overdue_actions ?? 0} / unowned {runtime?.remediation_backlog?.unowned_actions ?? 0} · runbooks {runtime?.service_runbooks?.configured ? "READY" : "OPTIONAL"}.</p></section>
