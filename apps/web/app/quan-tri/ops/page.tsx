...
import {
  ...
  rollbackProbeDesiredStateAdmin, approveProbeRollbackAdmin, approveProbeRolloutAdmin,
  xuatRecoveryEvidenceBundleAdmin, xuatRemediationBacklogExcelAdmin, escalateRemediationBacklogAdmin,
...
export default function OpsDashboardPage() {
  ...
  const [desiredForm, setDesiredForm] = useState({ target_version: "3.20.0", interval_seconds: 300, rollout_percent: 0, canary_agents: "", paused: true, note: "" });
  ...
  async function ackDeadLetter(item: WebhookDeadLetterAdmin) {
    ...
    try { await acknowledgeWebhookDeadLetterAdmin(item.id, "Đã xác nhận xử lý từ Ops Dashboard v3.20.0"); setThongBao(`Đã acknowledge dead-letter #${item.id}.`); await taiDuLieu(); }
    catch (error) { setLoi(error instanceof Error ? error.message : String(error)); }
    finally { setDangXuLy(""); }
  }
  ...
  async function approveProbeRollback() {
    ...
    try { await approveProbeRollbackAdmin("Approved from Ops Dashboard v3.20.0"); setThongBao("Đã duyệt auto rollback cho revision hiện tại."); await taiDuLieu(); }
    catch (error) { setLoi(error instanceof Error ? error.message : String(error)); }
    finally { setDangXuLy(""); }
  }
  ...
  async function approveProbeRollout() {
    ...
    try { const kq = await approveProbeRolloutAdmin(proposal.id, "Approved from Ops Dashboard v3.20.0"); setDesiredState(kq); setThongBao("Đã duyệt production rollout theo TTL + two-person rule."); await taiDuLieu(); }
    catch (error) { setLoi(error instanceof Error ? error.message : String(error)); } finally { setDangXuLy(""); }
  }
  ...
  return <main className={styles.page}>
    <header className={styles.header}>
      <div><span className={styles.kicker}>NHIENIN3D · OPS v3.20.0</span><h1>Ops Dashboard</h1><p>Production rollout approval TTL/two-person, signed recovery audit bundle và remediation SLA/on-call escalation; giữ health gate, archive, quorum, managed fleet và DLQ replay.</p>{access && <small className={styles.accessBadge}>{access.admin ? "ADMIN · SERVICE OWNER" : `${access.vai_tro_ops} · ${access.dich_vu.join(", ")}`}</small>}</div>
