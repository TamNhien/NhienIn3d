...
export default function OpsDashboardPage() {
  ...
  const [desiredForm, setDesiredForm] = useState({ target_version: "3.23.0", interval_seconds: 300, rollout_percent: 0, canary_agents: "", paused: true, note: "" });
  ...
  async function acknowledgeRemediation(service: string) {
    ...
    try { const kq = await acknowledgeRemediationBacklogAdmin(service, "Acknowledged from Ops Dashboard v3.23.0", 24); setThongBao(`Đã acknowledge ${service} tới ${fmt(kq.snooze_until)}.`); await taiDuLieu(); }
    catch (error) { setLoi(error instanceof Error ? error.message : String(error)); } finally { setDangXuLy(""); }
  }
  ...
  async function ackDeadLetter(item: WebhookDeadLetterAdmin) {
    ...
    try { await acknowledgeWebhookDeadLetterAdmin(item.id, "Đã xác nhận xử lý từ Ops Dashboard v3.23.0"); setThongBao(`Đã acknowledge dead-letter #${item.id}.`); await taiDuLieu(); }
    catch (error) { setLoi(error instanceof Error ? error.message : String(error)); }
    finally { setDangXuLy(""); }
  }
  ...
  async function approveProbeRollback() {
    ...
    try { await approveProbeRollbackAdmin("Approved from Ops Dashboard v3.23.0"); setThongBao("Đã duyệt auto rollback cho revision hiện tại."); await taiDuLieu(); }
    catch (error) { setLoi(error instanceof Error ? error.message : String(error)); }
    finally { setDangXuLy(""); }
  }
  ...
  async function approveProbeRollout() {
    ...
    try { const kq = await approveProbeRolloutAdmin(proposal.id, "Approved from Ops Dashboard v3.23.0"); setDesiredState(kq); setThongBao("Đã duyệt production rollout theo TTL + two-person rule."); await taiDuLieu(); }
    catch (error) { setLoi(error instanceof Error ? error.message : String(error)); } finally { setDangXuLy(""); }
  }
  ...
  async function rejectProbeRollout() {
    ...
    try { await rejectProbeRolloutAdmin(proposal.id, "Rejected from Ops Dashboard v3.23.0"); setThongBao("Đã từ chối production rollout proposal."); await taiDuLieu(); }
    catch (error) { setLoi(error instanceof Error ? error.message : String(error)); } finally { setDangXuLy(""); }
  }
  ...
  async function cancelProbeRollout() {
    ...
    try { await cancelProbeRolloutAdmin(proposal.id, "Cancelled from Ops Dashboard v3.23.0"); setThongBao("Đã hủy production rollout proposal."); await taiDuLieu(); }
    catch (error) { setLoi(error instanceof Error ? error.message : String(error)); } finally { setDangXuLy(""); }
  }
