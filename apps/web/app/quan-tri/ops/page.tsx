...
const pct = (value?: number | null) => value == null ? "—" : `${value.toLocaleString("vi-VN", { maximumFractionDigits: 2 })}%`;
...
export default function OpsDashboardPage() {
  ...
  const taiDuLieu = useCallback(async () => {
    ...
    try {
      ...
      const [sloData, incidentData, maintenanceData, policyData, webhookData] = await Promise.all([
      ...
      setSla(sloData); setIncidents(incidentData.du_lieu); setMaintenance(maintenanceData.du_lieu); setPolicy(policyData); setWebhook(webhookData.du_lieu);
  ...
  async function savePolicy() {
    ...
    try { const kq = await capNhatCauHinhSloNangCaoAdmin({ burn_windows: policy.burn_windows, service_targets: policy.service_targets }); setPolicy(kq); setThongBao("Đã lưu multi-window burn-rate và service error-budget policy."); await taiDuLieu(); }
    catch (error) { setLoi(error instanceof Error ? error.message : String(error)); }
    finally { setDangXuLy(""); }
  }
  ...
  return <main className={styles.page}>
    <header className={styles.header}>
      <div><span className={styles.kicker}>NHIENIN3D · OPS v3.7.2</span><h1>Ops Dashboard</h1><p>Incident, SLO, error budget, burn-rate, MTTA/MTTR, maintenance và webhook delivery trên một màn hình vận hành riêng.</p></div>
    ...
    <section className={styles.cards}>
      ...
      <article><span>Maintenance active</span><strong>{activeMaintenance.length}</strong><small>{maintenance.length} window đã cấu hình</small></article>
    </section>

    <section className={styles.grid2}>
      <article className={styles.panel}><h2>Multi-window burn-rate policy</h2><p>Ngưỡng cảnh báo có thể thay đổi mà không cần deploy.</p>{policy && <>
    ...
    <section className={styles.panel}><div className={styles.panelHead}><div><h2>Maintenance windows</h2><p>Hỗ trợ nhiều cửa sổ, lặp hằng ngày/hằng tuần và vẫn suppress cảnh báo tự động khi active.</p></div></div>
    ...
    <section className={styles.grid2}>
      <article className={styles.panel}><h2>Incident theo bộ lọc</h2><div className={styles.incidents}>{incidents.map(item => <div key={item.chu_ky}><span>{item.trang_thai_xu_ly.replaceAll("_", " ")}</span><b>{item.van_de.join(" · ") || "Incident"}</b><small>#{item.chu_ky.slice(0, 12)} · {fmt(item.bat_dau)} · {item.so_su_kien} sự kiện</small></div>)}{!incidents.length && <p>Không có incident phù hợp.</p>}</div></article>
      <article className={styles.panel}><h2>Webhook delivery log</h2><p>Retry/backoff và HMAC được ghi từng delivery attempt trong lịch sử vận hành.</p><div className={styles.incidents}>{webhook.map(item => <div key={item.id}><span>{item.trang_thai}</span><b>{item.mo_ta || "Webhook delivery"}</b><small>{fmt(item.ngay_tao)} · {JSON.stringify(item.chi_tiet)}</small></div>)}{!webhook.length && <p>Chưa có delivery log.</p>}</div></article>
    </section>
  </main>;
}
