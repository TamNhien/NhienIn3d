...
import {
  ...
  layDonMuaHangAdmin,
  taoDonMuaHangAdmin,
  capNhatTrangThaiDonMuaAdmin,
  xuatDonMuaExcelAdmin,
  layPhienKiemKeKhoAdmin,
  taoPhienKiemKeKhoAdmin,
  guiDuyetPhienKiemKeKhoAdmin,
  duyetApDungPhienKiemKeKhoAdmin,
  huyPhienKiemKeKhoAdmin,
  layNhaCungCapAdmin,
  taoNhaCungCapAdmin,
  ...
  layCauHinhSloVanHanhAdmin,
  capNhatCauHinhSloVanHanhAdmin,
  layBaoTriHeThongAdmin,
  capNhatBaoTriHeThongAdmin,
  laySlaVanHanhAdmin,
  layDanhSachSuCoVanHanhAdmin,
  layChiTietSuCoVanHanhAdmin,
  xuatDanhSachSuCoExcelAdmin,
  xuatChiTietSuCoExcelAdmin,
  tiepNhanSuCoVanHanhAdmin,
  khacPhucSuCoVanHanhAdmin,
...
const dinhDangInputNgayGio = (gia_tri?: string | null) => {
  if (!gia_tri) return "";
  const d = new Date(gia_tri);
  if (Number.isNaN(d.getTime())) return gia_tri.slice(0, 16);
  const local = new Date(d.getTime() - d.getTimezoneOffset() * 60_000);
  return local.toISOString().slice(0, 16);
};
const taiTepBase64 = (kq: { base64: string; mime_type: string; ten_file: string }) => {
...
export default function QuanTriPage() {
  ...
  const [hoan_tien_cho, setHoanTienCho] = useState<HoanTienCanXuLyAdmin | null>(null);
  ...
  const [don_mua_qt, setDonMuaQt] = useState<DonMuaHangAdmin[]>([]);
  const [phien_kiem_ke_qt, setPhienKiemKeQt] = useState<PhienKiemKeKhoAdmin[]>([]);
  ...
  const [ncc_moi, setNccMoi] = useState({ ma_nha_cung_cap: "", ten_nha_cung_cap: "", nguoi_lien_he: "", so_dien_thoai: "", thu_dien_tu: "", dia_chi: "", ghi_chu: "", dang_hoat_dong: true, thoi_gian_giao_hang_ngay: 7 });
  ...
  const [cau_hinh_slo, setCauHinhSlo] = useState<CauHinhSloVanHanhAdmin>({ sla_muc_tieu_percent: 99, uptime_muc_tieu_percent: 99.9, canh_bao_xu_huong: true, nguon_cau_hinh: "ENV" });
  const [bao_tri_he_thong, setBaoTriHeThong] = useState<BaoTriHeThongAdmin>({ bat: false, bat_dau: null, ket_thuc: null, ly_do: "", dang_bao_tri: false, sap_bao_tri: false, da_ket_thuc: false, nguon_cau_hinh: "MAC_DINH" });
  ...
  const [su_co_ghi_chu, setSuCoGhiChu] = useState("");
  ...
  const taiDuLieu = useCallback(async () => {
    ...
    const [tq, nd, nvData, caData, pcData, donData, hoanTienData, spData, dmData, vlData, msData, chKhoData, lsKhoData, phieuNhapData, nccData, emailKhoData, keHoachNhapData, donMuaData, phienKiemKeData, dgData, nkPage, heThongData, vanHanhPage, thongKeVanHanh, cauHinhCanhBao, cauHinhSlo, baoTriHeThong, slaVanHanh, suCoVanHanh] = await Promise.all([layTongQuan(), layNguoiDung(), layNhanVien(), layCaLam(), layPhanCa(), layDonHangAdmin(), layHoanTienCanXuLyAdmin(), laySanPhamAdmin(), layDanhMucAdmin(), layVatLieuAdmin(), layMauSacAdmin(), layCauHinhKhoAdmin(), layLichSuKhoAdmin(), layPhieuNhapKhoAdmin(), layNhaCungCapAdmin(), layTrangThaiCanhBaoKhoEmailAdmin(), layGoiYNhapKhoAdmin(), layDonMuaHangAdmin(), layPhienKiemKeKhoAdmin(), layDanhGiaAdmin(), layNhatKyCursorAdmin({ kich_thuoc: 25 }), laySucKhoeHeThongAdmin(), layLichSuVanHanhCursorAdmin({ kich_thuoc: 20 }), layThongKeVanHanhAdmin(), layCauHinhCanhBaoHeThongAdmin(), layCauHinhSloVanHanhAdmin(), layBaoTriHeThongAdmin(), laySlaVanHanhAdmin(90), layDanhSachSuCoVanHanhAdmin(100)]);
    setTongQuan(tq);
    setNguoiDung(nd);
    setNhanVien(nvData);
    setCaLam(caData);
    ...
    setDonMuaQt(donMuaData);
    setPhienKiemKeQt(phienKiemKeData);
    setDanhGiaQt(dgData);
    ...
    setCauHinhSlo(cauHinhSlo);
    setBaoTriHeThong(baoTriHeThong);
  ...
  async function taiDonTheoBoLoc() {
    ...
    setThongBao("");
    try {
      const [ds, queue] = await Promise.all([layDonHangAdmin(don_loc_trang_thai, don_tim_kiem), layHoanTienCanXuLyAdmin()]);
      setDonHang(ds);
      setHoanTienCho(queue);
  ...
  async function xacNhanHoanTienDon() {
    ...
    if (!window.confirm(`Xác nhận bạn đã hoàn ${dinhDangTien(so_tien)} cho khách của đơn ${don_chon.ma_don_hang}? Hệ thống chỉ ghi nhận trạng thái, không tự gọi cổng thanh toán.`)) return;
    setDangXuLy(`hoan-tien-${don_chon.id}`);
    setThongBao("Đang ghi nhận xác nhận hoàn tiền...");
    try {
      const da_luu = await xacNhanHoanTienDonHangAdmin(don_chon.id, don_ghi_chu.trim() || undefined);
      setDonChon(da_luu);
      setDonGhiChu("");
      const [queue, ds, tq] = await Promise.all([layHoanTienCanXuLyAdmin(), layDonHangAdmin(don_loc_trang_thai, don_tim_kiem), layTongQuan()]);
      setHoanTienCho(queue); setDonHang(ds); setTongQuan(tq);
      setThongBao(da_luu.xac_nhan_hoan_tien?.da_cap_nhat_moi ? `Đã xác nhận hoàn ${dinhDangTien(da_luu.xac_nhan_hoan_tien.so_tien)} cho ${da_luu.ma_don_hang}.` : `Đơn ${da_luu.ma_don_hang} đã được ghi nhận hoàn tiền trước đó.`);
    } catch (e) { setThongBao(e instanceof Error ? e.message : "Không thể xác nhận hoàn tiền"); }
    finally { setDangXuLy(null); }
  }
  ...
  async function luuTrangThaiDon() {
    ...
    try {
      ...
      void Promise.allSettled([
        ...
        laySanPhamAdmin(),
        layNhatKyAdmin()
      ]).then(([ds_kq, refund_kq, sp_kq, nk_kq]) => {
        if (ds_kq.status === "fulfilled") setDonHang(ds_kq.value);
        if (refund_kq.status === "fulfilled") setHoanTienCho(refund_kq.value);
  ...
  async function apDungKiemKeKho() {
    ...
    try {
      const kq = await apDungKiemKeKhoAdmin([{ bien_the_id: row.bien_the_id, ton_he_thong: row.ton_he_thong, ton_thuc_te: row.ton_thuc_te, ly_do: row.ly_do || "Kiểm kê tồn kho thực tế v3.28" }]);
  ...
  const dongKiemKeTuTep = () => (kiem_ke_tep?.dong || []).filter(row => row.hop_le && row.bien_the_id && typeof row.ton_he_thong === "number").map(row => ({ bien_the_id: String(row.bien_the_id), ton_he_thong: Number(row.ton_he_thong), ton_thuc_te: Number(row.ton_thuc_te), ly_do: row.ly_do || "Kiểm kê tồn kho theo file v3.28" }));
  ...
  async function taiExcelDoiSoatKiemKe() {
    const source = kiem_ke_tep ? dongKiemKeTuTep() : (kiem_ke_preview?.dong || []).filter(row => row.bien_the_id && typeof row.ton_he_thong === "number").map(row => ({ bien_the_id: row.bien_the_id, ton_he_thong: Number(row.ton_he_thong), ton_thuc_te: Number(row.ton_thuc_te), ly_do: row.ly_do || "Kiểm kê tồn kho thực tế v3.28" }));
  ...
  async function taoPoTuNhaCungCap(nha_cung_cap_id: string) {
    const rows=(ke_hoach_nhap?.items||[]).filter(x=>x.nha_cung_cap?.id===nha_cung_cap_id && x.so_luong_de_xuat>0);
    if(!rows.length){setThongBao("Không có dòng đề xuất hợp lệ cho nhà cung cấp này.");return;}
    const ncc=nha_cung_cap_qt.find(x=>x.id===nha_cung_cap_id); if(!ncc?.dang_hoat_dong){setThongBao("Nhà cung cấp chưa sẵn sàng.");return;}
    if(!window.confirm(`Tạo PO nháp cho ${ncc.ten_nha_cung_cap} với ${rows.length} biến thể? Đơn giá nhập mặc định 0 và cần được rà soát trước khi duyệt.`)) return;
    setDangXuLy(`po-create-${nha_cung_cap_id}`); setThongBao("");
    try { await taoDonMuaHangAdmin({nha_cung_cap_id,ngay_du_kien:sauNgay(ncc.thoi_gian_giao_hang_ngay||7),ghi_chu:"Tạo từ kế hoạch replenishment v3.28",dong:rows.map(x=>({bien_the_id:x.bien_the_id,so_luong_dat:x.so_luong_de_xuat,don_gia_nhap:0}))}); setDonMuaQt(await layDonMuaHangAdmin()); setThongBao(`Đã tạo PO nháp cho ${ncc.ten_nha_cung_cap}.`); }
    catch(e){setThongBao(e instanceof Error?e.message:"Không thể tạo PO nháp");} finally{setDangXuLy(null);}
  }
  async function doiTrangThaiPo(id:string,trang_thai:"DA_DUYET"|"DA_DAT"|"HUY") { setDangXuLy(`po-${id}`); setThongBao(""); try{await capNhatTrangThaiDonMuaAdmin(id,trang_thai);setDonMuaQt(await layDonMuaHangAdmin());setThongBao(`Đã cập nhật đơn mua sang ${trang_thai}.`);}catch(e){setThongBao(e instanceof Error?e.message:"Không thể cập nhật PO");}finally{setDangXuLy(null);} }
  async function taiDonMuaExcel(){ setDangXuLy("po-excel");try{const x=await xuatDonMuaExcelAdmin();taiTepBase64(x);setThongBao(`Đã xuất ${x.ten_file}.`);}catch(e){setThongBao(e instanceof Error?e.message:"Không thể xuất PO Excel");}finally{setDangXuLy(null);} }
  async function luuThanhPhienKiemKe(){ const source=kiem_ke_tep?dongKiemKeTuTep():(kiem_ke_preview?.dong||[]).filter(x=>x.bien_the_id&&typeof x.ton_he_thong==="number").map(x=>({bien_the_id:x.bien_the_id,ton_he_thong:Number(x.ton_he_thong),ton_thuc_te:Number(x.ton_thuc_te),ly_do:x.ly_do||"Phiên kiểm kê v3.28"})); if(!source.length){setThongBao("Hãy xem trước kiểm kê trước khi lưu phiên.");return;} setDangXuLy("kiem-ke-session");try{await taoPhienKiemKeKhoAdmin(source,"Phiên kiểm kê tạo từ màn hình Kho v3.28");setPhienKiemKeQt(await layPhienKiemKeKhoAdmin());setThongBao("Đã lưu phiên kiểm kê nháp.");}catch(e){setThongBao(e instanceof Error?e.message:"Không thể lưu phiên kiểm kê");}finally{setDangXuLy(null);} }
  async function xuLyPhienKiemKe(id:string,action:"gui"|"duyet"|"huy"){setDangXuLy(`kk-${id}`);try{if(action==="gui")await guiDuyetPhienKiemKeKhoAdmin(id);else if(action==="duyet")await duyetApDungPhienKiemKeKhoAdmin(id);else await huyPhienKiemKeKhoAdmin(id);const [sessions,sp,ls,plan]=await Promise.all([layPhienKiemKeKhoAdmin(),laySanPhamAdmin(),layLichSuKhoAdmin(),layGoiYNhapKhoAdmin()]);setPhienKiemKeQt(sessions);setSanPhamQt(sp);setLichSuKho(ls);setKeHoachNhap(plan);setThongBao(action==="duyet"?"Đã duyệt và áp dụng phiên kiểm kê.":"Đã cập nhật phiên kiểm kê.");}catch(e){setThongBao(e instanceof Error?e.message:"Không thể xử lý phiên kiểm kê");}finally{setDangXuLy(null);} }
  ...
  async function xacNhanNhapKhoTheoLo() {
    ...
    try {
      const phieu = await nhapKhoTheoLoAdmin({
        ...
        don_mua_hang_id: nhap_lo_meta.don_mua_hang_id || undefined,
        cho_phep_vuot_don_mua: nhap_lo_meta.cho_phep_vuot_don_mua,
      ...
      setImportKho(null); setNhapLoMeta({ ma_lo: "", nha_cung_cap_id: "", ghi_chu: "", don_mua_hang_id: "", cho_phep_vuot_don_mua: false }); setDonMuaQt(await layDonMuaHangAdmin());
  ...
  async function taoNhaCungCapMoi(e: FormEvent) {
    ...
    try {
      const da_tao = await taoNhaCungCapAdmin({
        ...
        dia_chi: ncc_moi.dia_chi.trim(), ghi_chu: ncc_moi.ghi_chu.trim(), dang_hoat_dong: ncc_moi.dang_hoat_dong, thoi_gian_giao_hang_ngay: ncc_moi.thoi_gian_giao_hang_ngay
      });
      const [ds, nk] = await Promise.all([layNhaCungCapAdmin(ncc_tim_kiem, ncc_loc_hoat_dong), layNhatKyAdmin()]);
      setNhaCungCapQt(ds); setNhatKy(nk); setNccMoi({ ma_nha_cung_cap: "", ten_nha_cung_cap: "", nguoi_lien_he: "", so_dien_thoai: "", thu_dien_tu: "", dia_chi: "", ghi_chu: "", dang_hoat_dong: true, thoi_gian_giao_hang_ngay: 7 });
  ...
  async function luuNhaCungCap(item: NhaCungCapAdmin) {
    ...
    try {
      const da_luu = await capNhatNhaCungCapAdmin(item.id, { ten_nha_cung_cap: item.ten_nha_cung_cap.trim(), nguoi_lien_he: item.nguoi_lien_he?.trim() || "", so_dien_thoai: item.so_dien_thoai?.trim() || "", thu_dien_tu: item.thu_dien_tu?.trim() || "", dia_chi: item.dia_chi?.trim() || "", ghi_chu: item.ghi_chu?.trim() || "", dang_hoat_dong: item.dang_hoat_dong, thoi_gian_giao_hang_ngay: item.thoi_gian_giao_hang_ngay || 7 });
  ...
  async function moChiTietSuCo(chu_ky: string) {
    ...
    try { const chiTiet = await layChiTietSuCoVanHanhAdmin(chu_ky); setSuCoChon(chiTiet); setSuCoGhiChu(chiTiet.ghi_chu || ""); }
    catch (e) { setThongBao(e instanceof Error ? e.message : "Không thể tải chuỗi sự cố"); }
    finally { setDangXuLy(null); }
  }

  async function luuCauHinhSlo() {
    setDangXuLy("luu-cau-hinh-slo"); setThongBao("");
    try {
      const kq = await capNhatCauHinhSloVanHanhAdmin({ sla_muc_tieu_percent: Number(cau_hinh_slo.sla_muc_tieu_percent), uptime_muc_tieu_percent: Number(cau_hinh_slo.uptime_muc_tieu_percent), canh_bao_xu_huong: cau_hinh_slo.canh_bao_xu_huong });
      setCauHinhSlo(kq);
      setSlaVanHanh(await laySlaVanHanhAdmin(sla_so_ngay));
      setThongBao("Đã lưu mục tiêu SLO và đánh giá lại xu hướng 7/30 ngày.");
    } catch (e) { setThongBao(e instanceof Error ? e.message : "Không thể lưu cấu hình SLO"); }
    finally { setDangXuLy(null); }
  }

  async function luuBaoTriHeThong() {
    setDangXuLy("luu-bao-tri-he-thong"); setThongBao("");
    try {
      const bat_dau = bao_tri_he_thong.bat && bao_tri_he_thong.bat_dau ? new Date(bao_tri_he_thong.bat_dau).toISOString() : undefined;
      const ket_thuc = bao_tri_he_thong.bat && bao_tri_he_thong.ket_thuc ? new Date(bao_tri_he_thong.ket_thuc).toISOString() : undefined;
      const kq = await capNhatBaoTriHeThongAdmin({ bat: bao_tri_he_thong.bat, bat_dau, ket_thuc, ly_do: bao_tri_he_thong.ly_do.trim() });
      setBaoTriHeThong(kq);
      setSucKhoeHeThong(await laySucKhoeHeThongAdmin());
      setThongBao(kq.dang_bao_tri ? "Maintenance window đang hoạt động; cảnh báo tự động được tạm ngưng." : kq.sap_bao_tri ? "Đã lên lịch maintenance window." : "Đã lưu cấu hình maintenance window.");
    } catch (e) { setThongBao(e instanceof Error ? e.message : "Không thể lưu maintenance window"); }
    finally { setDangXuLy(null); }
  }

  async function capNhatXuLySuCo(hanh_dong: "tiep-nhan" | "khac-phuc") {
    if (!su_co_chon) return;
    setDangXuLy(`xu-ly-su-co-${hanh_dong}`); setThongBao("");
    try {
  ...
  async function taiDanhSachSuCoExcel() {
    setDangXuLy("xuat-incident-excel"); setThongBao("");
    try { const kq = await xuatDanhSachSuCoExcelAdmin(); taiTepBase64(kq); setThongBao(`Đã xuất Excel incident: ${kq.ten_file}.`); }
    catch (e) { setThongBao(e instanceof Error ? e.message : "Không thể xuất danh sách incident"); }
    finally { setDangXuLy(null); }
  }

  async function taiChiTietSuCoExcel() {
    if (!su_co_chon) return;
    setDangXuLy("xuat-incident-timeline-excel"); setThongBao("");
    try { const kq = await xuatChiTietSuCoExcelAdmin(su_co_chon.chu_ky); taiTepBase64(kq); setThongBao(`Đã xuất timeline incident: ${kq.ten_file}.`); }
    catch (e) { setThongBao(e instanceof Error ? e.message : "Không thể xuất timeline incident"); }
    finally { setDangXuLy(null); }
  }
  ...
  async function taiSucKhoeHeThong() {
    ...
    try {
      ...
      setCauHinhSlo(sloConfig);
      setBaoTriHeThong(baoTri);
      setSlaVanHanh(sla);
  ...
  return <main className="cine-admin-shell page-shell">
    ...
    {tab === "kho" && <section className="cine-admin-operations cine-commerce-admin-v212 cine-inventory-admin-v214 cine-variant-admin-v215">
      ...
      <div className="cine-card cine-replenishment-v324">
        <div className="cine-replenishment-head-v324"><div><h3>Kế hoạch nhập đề xuất</h3><p>v3.28 bổ sung lead time, safety stock và reorder point lên nền tốc độ bán 30 ngày; kế hoạch vẫn gom theo nhà cung cấp để Admin tạo PO nháp có kiểm soát; nhu cầu dự báo {ke_hoach_nhap?.forecast_days || 14} ngày; nhu cầu từ giỏ đang mở chỉ là tín hiệu áp lực, không giữ chỗ tồn kho. Không tự gửi PO cho nhà cung cấp và không tự nhập kho.</p></div><button type="button" className="cine-btn cine-btn-secondary" onClick={taiKeHoachNhapKhoExcel} disabled={dang_xu_ly==="excel-goi-y-nhap"}>{dang_xu_ly==="excel-goi-y-nhap"?"Đang xuất…":"Xuất kế hoạch Excel"}</button></div>
        ...
        {ke_hoach_nhap && ke_hoach_nhap.theo_nha_cung_cap.length>0 && <div className="cine-supplier-plan-v326"><h4>Gom đề xuất theo nhà cung cấp</h4><div>{ke_hoach_nhap.theo_nha_cung_cap.slice(0,8).map((group,index)=><article key={group.nha_cung_cap?.id || group.nha_cung_cap?.ten_nha_cung_cap || `missing-${index}`} data-status={group.trang_thai_nhom || "CHUA_GAN_NCC"}><span><b>{group.nha_cung_cap?.ten_nha_cung_cap || "Chưa gán nhà cung cấp"}</b><small>{group.nha_cung_cap?.ma_nha_cung_cap || "Cần xử lý thủ công"} · lead time {group.lead_time_days || 7} ngày</small></span><strong>+{group.so_luong_de_xuat}</strong><small>{group.so_bien_the} biến thể · {group.ready_to_send ? "sẵn sàng lập đơn mua" : "chưa sẵn sàng"}</small>{group.nha_cung_cap?.id && group.ready_to_send && <button type="button" className="cine-btn cine-btn-secondary" onClick={()=>void taoPoTuNhaCungCap(String(group.nha_cung_cap?.id))} disabled={dang_xu_ly===`po-create-${group.nha_cung_cap.id}`}>Tạo PO nháp</button>}</article>)}</div></div>}
      </div>

      <div className="cine-card cine-purchase-orders-v328"><div className="cine-po-head-v328"><div><h3>Đơn mua hàng · Purchase Order</h3><p>v3.28 nối kế hoạch nhập với PO nháp. Luồng: NHÁP → ĐÃ DUYỆT → ĐÃ ĐẶT; không tự gửi nhà cung cấp và không tự nhập kho.</p></div><button type="button" className="cine-btn cine-btn-secondary" onClick={()=>void taiDonMuaExcel()} disabled={dang_xu_ly==="po-excel"}>Xuất PO Excel</button></div><div className="cine-po-list-v328">{don_mua_qt.slice(0,12).map(po=><article key={po.id}><span><b>{po.ma_don_mua}</b><small>{po.nha_cung_cap.ten_nha_cung_cap} · {po.chi_tiet.length} dòng</small></span><span><b>{po.trang_thai.replaceAll("_"," ")}</b><small>SL {po.tong_so_luong} · {Number(po.tong_gia_tri).toLocaleString("vi-VN")} đ</small></span><span className="cine-po-actions-v328">{po.trang_thai==="NHAP"&&<button type="button" className="cine-btn cine-btn-primary" onClick={()=>void doiTrangThaiPo(po.id,"DA_DUYET")}>Duyệt</button>}{po.trang_thai==="DA_DUYET"&&<button type="button" className="cine-btn cine-btn-primary" onClick={()=>void doiTrangThaiPo(po.id,"DA_DAT")}>Đánh dấu đã đặt</button>}{!["HUY","HOAN_TAT"].includes(po.trang_thai)&&<button type="button" className="cine-btn cine-btn-danger-outline" onClick={()=>void doiTrangThaiPo(po.id,"HUY")}>Hủy</button>}</span></article>)}{don_mua_qt.length===0&&<div className="cine-dashboard-empty">Chưa có PO. Có thể tạo PO nháp từ nhóm nhà cung cấp sẵn sàng phía trên.</div>}</div></div>
    ...
    {tab === "nha-cung-cap" && <section className="cine-admin-operations cine-supplier-admin-v219">
      ...
      <div className="cine-supplier-layout-v219">
        <form className="cine-card cine-supplier-create-v219" onSubmit={taoNhaCungCapMoi}>
          ...
          <div className="cine-supplier-form-grid-v219"><label><span>Mã nhà cung cấp</span><input required value={ncc_moi.ma_nha_cung_cap} onChange={e=>setNccMoi(x=>({...x,ma_nha_cung_cap:e.target.value}))} placeholder="VD: NCC-PLA-01"/></label><label><span>Tên nhà cung cấp</span><input required value={ncc_moi.ten_nha_cung_cap} onChange={e=>setNccMoi(x=>({...x,ten_nha_cung_cap:e.target.value}))}/></label><label><span>Người liên hệ</span><input value={ncc_moi.nguoi_lien_he} onChange={e=>setNccMoi(x=>({...x,nguoi_lien_he:e.target.value}))}/></label><label><span>Số điện thoại</span><input value={ncc_moi.so_dien_thoai} onChange={e=>setNccMoi(x=>({...x,so_dien_thoai:e.target.value}))}/></label><label><span>Email</span><input type="email" value={ncc_moi.thu_dien_tu} onChange={e=>setNccMoi(x=>({...x,thu_dien_tu:e.target.value}))}/></label><label className="wide"><span>Địa chỉ</span><input value={ncc_moi.dia_chi} onChange={e=>setNccMoi(x=>({...x,dia_chi:e.target.value}))}/></label><label className="wide"><span>Ghi chú</span><textarea value={ncc_moi.ghi_chu} onChange={e=>setNccMoi(x=>({...x,ghi_chu:e.target.value}))}/></label><label><span>Lead time (ngày)</span><input type="number" min="1" max="180" value={ncc_moi.thoi_gian_giao_hang_ngay} onChange={e=>setNccMoi(x=>({...x,thoi_gian_giao_hang_ngay:Math.max(1,Math.min(180,Number(e.target.value)||7))}))}/></label><label className="cine-check-v215 cine-supplier-active-check-v320"><input type="checkbox" checked={ncc_moi.dang_hoat_dong} onChange={e=>setNccMoi(x=>({...x,dang_hoat_dong:e.target.checked}))}/><span>Đang hoạt động</span></label></div>
        ...
        <div className="cine-supplier-main-v219">
          ...
          <div className="cine-supplier-list-v219">{nha_cung_cap_qt.map(item=><article className="cine-card cine-supplier-row-v219" key={item.id}><div className="cine-supplier-code-v219"><b>{item.ma_nha_cung_cap}</b><small>{item.so_phieu_nhap} phiếu nhập · {item.so_don_mua || 0} PO</small><span className={`status-badge ${item.dang_hoat_dong?"active":"locked"}`}>{item.dang_hoat_dong?"Hoạt động":"Ngừng"}</span></div><div className="cine-supplier-fields-v219"><label><span>Tên nhà cung cấp</span><input value={item.ten_nha_cung_cap} onChange={e=>suaNhaCungCapLocal(item.id,{ten_nha_cung_cap:e.target.value})}/></label><label><span>Người liên hệ</span><input value={item.nguoi_lien_he||""} onChange={e=>suaNhaCungCapLocal(item.id,{nguoi_lien_he:e.target.value})}/></label><label><span>Điện thoại</span><input value={item.so_dien_thoai||""} onChange={e=>suaNhaCungCapLocal(item.id,{so_dien_thoai:e.target.value})}/></label><label><span>Email</span><input type="email" value={item.thu_dien_tu||""} onChange={e=>suaNhaCungCapLocal(item.id,{thu_dien_tu:e.target.value})}/></label><label className="wide"><span>Địa chỉ</span><input value={item.dia_chi||""} onChange={e=>suaNhaCungCapLocal(item.id,{dia_chi:e.target.value})}/></label><label className="wide"><span>Ghi chú</span><input value={item.ghi_chu||""} onChange={e=>suaNhaCungCapLocal(item.id,{ghi_chu:e.target.value})}/></label><label><span>Lead time (ngày)</span><input type="number" min="1" max="180" value={item.thoi_gian_giao_hang_ngay||7} onChange={e=>suaNhaCungCapLocal(item.id,{thoi_gian_giao_hang_ngay:Math.max(1,Math.min(180,Number(e.target.value)||7))})}/></label><label className="cine-check-v215 cine-supplier-active-check-v320"><input type="checkbox" checked={item.dang_hoat_dong} onChange={e=>suaNhaCungCapLocal(item.id,{dang_hoat_dong:e.target.checked})}/><span>{item.dang_hoat_dong?"Đang hoạt động":"Ngừng hoạt động"}</span></label></div><div className="cine-row-actions-v215"><button type="button" className="cine-btn cine-btn-secondary" onClick={()=>luuNhaCungCap(item)} disabled={dang_xu_ly===`ncc-${item.id}`}>Lưu</button><button type="button" className="cine-btn cine-btn-danger-outline" onClick={()=>xoaNhaCungCap(item)} disabled={dang_xu_ly===`ncc-${item.id}`||item.so_phieu_nhap>0||(item.so_don_mua||0)>0} title={item.so_phieu_nhap>0||(item.so_don_mua||0)>0?"Đã có phiếu nhập/PO: hãy chuyển sang Ngừng hoạt động":"Xóa nhà cung cấp"}>Xóa</button></div></article>)}{nha_cung_cap_qt.length===0&&<div className="cine-card cine-dashboard-empty">Không có nhà cung cấp phù hợp.</div>}</div>
        </div>
      </div>
    </section>}
    ...
    {tab === "he-thong" && <section className="cine-admin-operations cine-system-admin-v310">
      <div className="cine-operations-heading"><div><h2>Sức khỏe hệ thống & vận hành</h2><p>v3.18.0 bổ sung target-time PITR rehearsal opt-in, health-gated canary auto-rollback và postmortem approval/action reminder; các tính năng archive/on-call/device-bound vẫn tương thích.</p></div><div className="cine-system-heading-actions-v310"><Link href="/quan-tri/ops" className="cine-btn cine-btn-secondary">Mở Ops Dashboard</Link><span className={`status-badge ${suc_khoe_he_thong?.trang_thai === "TOT" ? "active" : "locked"}`}>{suc_khoe_he_thong?.trang_thai === "TOT" ? "Hệ thống tốt" : suc_khoe_he_thong?.trang_thai === "LOI" ? "Có lỗi" : "Cần chú ý"}</span><button type="button" className="cine-btn cine-btn-primary" onClick={taiSucKhoeHeThong} disabled={dang_xu_ly === "suc-khoe-he-thong"}>{dang_xu_ly === "suc-khoe-he-thong" ? "Đang kiểm tra…" : "Kiểm tra lại"}</button></div></div>
      {suc_khoe_he_thong ? <>
        <div className="cine-system-grid-v310 cine-system-grid-v320">
          ...
          <article className={`cine-card ${suc_khoe_he_thong.bao_tri.dang_bao_tri ? "cine-maintenance-active-v360" : ""}`}><span>Maintenance window</span><b>{suc_khoe_he_thong.bao_tri.dang_bao_tri ? "Đang bảo trì" : suc_khoe_he_thong.bao_tri.sap_bao_tri ? "Đã lên lịch" : "Không hoạt động"}</b><p>{suc_khoe_he_thong.bao_tri.bat_dau ? `${new Date(suc_khoe_he_thong.bao_tri.bat_dau).toLocaleString("vi-VN")} → ${suc_khoe_he_thong.bao_tri.ket_thuc ? new Date(suc_khoe_he_thong.bao_tri.ket_thuc).toLocaleString("vi-VN") : "—"}` : "Chưa có lịch bảo trì"}</p><small>{suc_khoe_he_thong.bao_tri.ly_do || "Cảnh báo tự động chỉ tạm ngưng khi cửa sổ đang hoạt động."}</small></article>
          <article className="cine-card"><span>Webhook cảnh báo</span><b>{suc_khoe_he_thong.webhook.san_sang ? "Sẵn sàng" : suc_khoe_he_thong.webhook.bat ? "Chưa hợp lệ" : "Đang tắt"}</b><p>{suc_khoe_he_thong.webhook.endpoint || "SYSTEM_ALERT_WEBHOOK_ENABLED=false"}</p><small>Token bearer chỉ đọc từ môi trường và không được trả về UI.</small></article>
        ...
        <div className="cine-card cine-maintenance-config-v360">
          <div className="cine-ops-history-head-v320"><div><h3>Maintenance window</h3><p>Lên lịch bảo trì có chủ đích. Trong khoảng thời gian này job cảnh báo tự động tạm ngưng, nhưng Admin vẫn có thể kiểm tra/gửi thủ công.</p></div><span className={`status-badge ${bao_tri_he_thong.dang_bao_tri ? "locked" : "active"}`}>{bao_tri_he_thong.dang_bao_tri ? "Đang bảo trì" : bao_tri_he_thong.sap_bao_tri ? "Đã lên lịch" : "Không hoạt động"}</span></div>
          <div className="cine-maintenance-grid-v360">
            <label className="cine-toggle-field-v340"><span>Bật maintenance</span><input type="checkbox" checked={bao_tri_he_thong.bat} onChange={e=>setBaoTriHeThong(x=>({...x,bat:e.target.checked}))}/><b>{bao_tri_he_thong.bat ? "Đang bật" : "Đang tắt"}</b></label>
            <label><span>Bắt đầu</span><input aria-label="Bảo trì bắt đầu" type="datetime-local" value={dinhDangInputNgayGio(bao_tri_he_thong.bat_dau)} onChange={e=>setBaoTriHeThong(x=>({...x,bat_dau:e.target.value || null}))}/></label>
            <label><span>Kết thúc</span><input aria-label="Bảo trì kết thúc" type="datetime-local" value={dinhDangInputNgayGio(bao_tri_he_thong.ket_thuc)} onChange={e=>setBaoTriHeThong(x=>({...x,ket_thuc:e.target.value || null}))}/></label>
            <label className="cine-maintenance-reason-v360"><span>Lý do / phạm vi</span><input aria-label="Lý do bảo trì" maxLength={500} value={bao_tri_he_thong.ly_do} onChange={e=>setBaoTriHeThong(x=>({...x,ly_do:e.target.value}))} placeholder="VD: nâng PostgreSQL, bảo trì mạng, thay đổi hạ tầng..."/></label>
          </div>
          <div className="cine-system-config-actions-v340"><small>{bao_tri_he_thong.ngay_cap_nhat ? `Cập nhật gần nhất: ${new Date(bao_tri_he_thong.ngay_cap_nhat).toLocaleString("vi-VN")}` : "Chưa có maintenance window lưu trong database."}</small><button type="button" className="cine-btn cine-btn-primary" onClick={luuBaoTriHeThong} disabled={dang_xu_ly === "luu-bao-tri-he-thong"}>{dang_xu_ly === "luu-bao-tri-he-thong" ? "Đang lưu…" : "Lưu maintenance window"}</button></div>
        </div>

        <div className="cine-card cine-slo-config-v350">
          <div className="cine-ops-history-head-v320"><div><h3>Mục tiêu SLO vận hành</h3><p>Đặt mục tiêu riêng cho SLA/Uptime. Hệ thống tính error budget 30 ngày và burn-rate 1h/6h/24h để phát hiện tốc độ tiêu ngân sách lỗi trước khi SLO dài hạn bị phá vỡ.</p></div><span className="cine-admin-count">{cau_hinh_slo.nguon_cau_hinh}</span></div>
          <div className="cine-slo-config-grid-v350">
            <label><span>SLA mục tiêu (%)</span><input type="number" min="90" max="100" step="0.01" value={cau_hinh_slo.sla_muc_tieu_percent} onChange={e=>setCauHinhSlo(x=>({...x,sla_muc_tieu_percent:Number(e.target.value)}))}/></label>
            <label><span>Uptime mục tiêu (%)</span><input type="number" min="90" max="100" step="0.01" value={cau_hinh_slo.uptime_muc_tieu_percent} onChange={e=>setCauHinhSlo(x=>({...x,uptime_muc_tieu_percent:Number(e.target.value)}))}/></label>
            <label className="cine-toggle-field-v340"><span>Cảnh báo xu hướng</span><input type="checkbox" checked={cau_hinh_slo.canh_bao_xu_huong} onChange={e=>setCauHinhSlo(x=>({...x,canh_bao_xu_huong:e.target.checked}))}/><b>{cau_hinh_slo.canh_bao_xu_huong ? "Đang bật" : "Đang tắt"}</b></label>
          </div>
          <div className="cine-system-config-actions-v340"><small>{cau_hinh_slo.ngay_cap_nhat ? `Cập nhật gần nhất: ${new Date(cau_hinh_slo.ngay_cap_nhat).toLocaleString("vi-VN")}` : "Đang dùng mục tiêu mặc định từ .env."}</small><button type="button" className="cine-btn cine-btn-primary" onClick={luuCauHinhSlo} disabled={dang_xu_ly === "luu-cau-hinh-slo"}>{dang_xu_ly === "luu-cau-hinh-slo" ? "Đang lưu…" : "Lưu mục tiêu SLO"}</button></div>
        </div>
        ...
        {sla_van_hanh && <div className="cine-card cine-sla-v340">
          ...
          <div className="cine-sla-summary-v340"><article className={sla_van_hanh.tong_quan.dat_sla===false?"slo-breach":""}><span>SLA</span><b>{sla_van_hanh.tong_quan.sla_percent == null ? "—" : `${sla_van_hanh.tong_quan.sla_percent}%`}</b><small>Mục tiêu {sla_van_hanh.muc_tieu.sla_muc_tieu_percent}% · {sla_van_hanh.tong_quan.tot}/{sla_van_hanh.tong_quan.tong} mẫu tốt</small></article><article className={sla_van_hanh.tong_quan.dat_uptime===false?"slo-breach":""}><span>Uptime</span><b>{sla_van_hanh.tong_quan.uptime_percent == null ? "—" : `${sla_van_hanh.tong_quan.uptime_percent}%`}</b><small>Mục tiêu {sla_van_hanh.muc_tieu.uptime_muc_tieu_percent}% · {sla_van_hanh.tong_quan.loi} mẫu lỗi</small></article><article><span>Xu hướng 7 / 30 ngày</span><b>{sla_van_hanh.xu_huong.bay_ngay.uptime_percent ?? "—"}% / {sla_van_hanh.xu_huong.ba_muoi_ngay.uptime_percent ?? "—"}%</b><small>{sla_van_hanh.canh_bao.length ? `${sla_van_hanh.canh_bao.length} cảnh báo SLO` : "Đang đạt mục tiêu hoặc chưa đủ mẫu"}</small></article></div>
          <div className="cine-error-budget-v360"><article><span>Error budget SLA · 30 ngày</span><b>{sla_van_hanh.ngan_sach_loi.sla.con_lai_percent == null ? "—" : `${sla_van_hanh.ngan_sach_loi.sla.con_lai_percent}% còn lại`}</b><small>Đã tiêu {sla_van_hanh.ngan_sach_loi.sla.da_tieu_thu_percent ?? "—"}% ngân sách · {sla_van_hanh.ngan_sach_loi.sla.mau_xau}/{sla_van_hanh.ngan_sach_loi.sla.tong_mau} mẫu xấu</small></article><article><span>Error budget Uptime · 30 ngày</span><b>{sla_van_hanh.ngan_sach_loi.uptime.con_lai_percent == null ? "—" : `${sla_van_hanh.ngan_sach_loi.uptime.con_lai_percent}% còn lại`}</b><small>Đã tiêu {sla_van_hanh.ngan_sach_loi.uptime.da_tieu_thu_percent ?? "—"}% ngân sách · {sla_van_hanh.ngan_sach_loi.uptime.mau_xau}/{sla_van_hanh.ngan_sach_loi.uptime.tong_mau} mẫu lỗi</small></article><article><span>Burn-rate SLA · 1h / 6h / 24h</span><b>{sla_van_hanh.burn_rate.sla.mot_gio.burn_rate ?? "—"}x · {sla_van_hanh.burn_rate.sla.sau_gio.burn_rate ?? "—"}x · {sla_van_hanh.burn_rate.sla.hai_muoi_bon_gio.burn_rate ?? "—"}x</b><small>Burn-rate &gt; 1x nghĩa là đang tiêu ngân sách nhanh hơn tốc độ bền vững.</small></article><article><span>Burn-rate Uptime · 1h / 6h / 24h</span><b>{sla_van_hanh.burn_rate.uptime.mot_gio.burn_rate ?? "—"}x · {sla_van_hanh.burn_rate.uptime.sau_gio.burn_rate ?? "—"}x · {sla_van_hanh.burn_rate.uptime.hai_muoi_bon_gio.burn_rate ?? "—"}x</b><small>Mức hiện tại: {sla_van_hanh.burn_rate.uptime.mot_gio.muc_do.replaceAll("_", " ")}</small></article></div>
          {sla_van_hanh.canh_bao.length>0&&<div className="cine-slo-warnings-v350">{sla_van_hanh.canh_bao.map(item=><span key={item}>⚠ {item}</span>)}</div>}
        ...
        <div className="cine-card cine-incidents-v340 cine-incidents-v350">
          <div className="cine-ops-history-head-v320"><div><h3>Incident vận hành</h3><p>Chuỗi sự cố theo chữ ký đọc trực tiếp bảng tổng hợp `su_co_van_hanh`; Admin có thể tiếp nhận, khắc phục và xuất Incident/Timeline ra Excel.</p></div><div className="cine-ops-history-actions-v330"><span>{su_co_van_hanh.length} incident gần nhất</span><button type="button" className="cine-btn cine-btn-secondary" onClick={taiDanhSachSuCoExcel} disabled={dang_xu_ly === "xuat-incident-excel"}>{dang_xu_ly === "xuat-incident-excel" ? "Đang xuất…" : "Xuất Incident Excel"}</button></div></div>
          <div className="cine-incidents-layout-v340">
            <div className="cine-incident-list-v340">{su_co_van_hanh.map(item=><button type="button" key={item.chu_ky} className={`cine-incident-item-v340 ${su_co_chon?.chu_ky===item.chu_ky?"selected":""}`} onClick={()=>void moChiTietSuCo(item.chu_ky)} disabled={dang_xu_ly===`su-co-${item.chu_ky}`}><span className={`status-badge ${item.trang_thai_xu_ly==="DA_KHAC_PHUC"?"active":"locked"}`}>{item.trang_thai_xu_ly.replaceAll("_"," ")}</span><b>{item.van_de.join(" · ") || "Không có mô tả"}</b><small>#{item.chu_ky.slice(0,12)} · {item.so_su_kien} sự kiện · {item.thoi_luong_phut} phút</small><em>{item.nguoi_khac_phuc_ten ? `Khắc phục: ${item.nguoi_khac_phuc_ten}` : item.nguoi_tiep_nhan_ten ? `Xử lý: ${item.nguoi_tiep_nhan_ten}` : new Date(item.gan_nhat).toLocaleString("vi-VN")}</em></button>)}{su_co_van_hanh.length===0&&<div className="cine-dashboard-empty">Chưa ghi nhận incident có chữ ký.</div>}</div>
            <div className="cine-incident-detail-v340">{su_co_chon ? <><div><b>Incident #{su_co_chon.chu_ky.slice(0,16)}</b><span className="cine-incident-head-actions-v360"><button type="button" className="cine-btn cine-btn-secondary" onClick={taiChiTietSuCoExcel} disabled={dang_xu_ly === "xuat-incident-timeline-excel"}>{dang_xu_ly === "xuat-incident-timeline-excel" ? "Đang xuất…" : "Xuất Timeline Excel"}</button><button type="button" className="cine-btn cine-btn-secondary" onClick={()=>{setSuCoChon(null);setSuCoGhiChu("");}}>Đóng</button></span></div><p>{su_co_chon.van_de.join(" · ") || "Không có mô tả vấn đề."}</p><div className="cine-incident-meta-v350"><span className={`status-badge ${su_co_chon.trang_thai_xu_ly==="DA_KHAC_PHUC"?"active":"locked"}`}>{su_co_chon.trang_thai_xu_ly.replaceAll("_"," ")}</span><small>{new Date(su_co_chon.bat_dau).toLocaleString("vi-VN")} → {new Date(su_co_chon.gan_nhat).toLocaleString("vi-VN")} · {su_co_chon.thoi_luong_phut} phút</small>{su_co_chon.nguoi_tiep_nhan_ten&&<small>Tiếp nhận: {su_co_chon.nguoi_tiep_nhan_ten}{su_co_chon.tiep_nhan_luc?` · ${new Date(su_co_chon.tiep_nhan_luc).toLocaleString("vi-VN")}`:""}</small>}{su_co_chon.nguoi_khac_phuc_ten&&<small>Khắc phục: {su_co_chon.nguoi_khac_phuc_ten}{su_co_chon.khac_phuc_luc?` · ${new Date(su_co_chon.khac_phuc_luc).toLocaleString("vi-VN")}`:""}</small>}</div><label className="cine-incident-note-v350"><span>Ghi chú xử lý / khắc phục</span><textarea rows={3} value={su_co_ghi_chu} onChange={e=>setSuCoGhiChu(e.target.value)} maxLength={2000} placeholder="Nguyên nhân, thao tác xử lý, kết quả kiểm tra..."/></label><div className="cine-incident-actions-v350">{su_co_chon.trang_thai_xu_ly==="MOI"&&<button type="button" className="cine-btn cine-btn-primary" onClick={()=>void capNhatXuLySuCo("tiep-nhan")} disabled={dang_xu_ly?.startsWith("xu-ly-su-co")}>Tiếp nhận incident</button>}{su_co_chon.trang_thai_xu_ly!=="DA_KHAC_PHUC"&&<button type="button" className="cine-btn cine-btn-success" onClick={()=>void capNhatXuLySuCo("khac-phuc")} disabled={dang_xu_ly?.startsWith("xu-ly-su-co")}>Đánh dấu đã khắc phục</button>}</div><div className="cine-incident-timeline-v340">{su_co_chon.su_kien.map(item=><article key={item.id}><i/><div><b>{item.loai} · {item.trang_thai.replaceAll("_"," ")}</b><span>{new Date(item.ngay_tao).toLocaleString("vi-VN")}</span><small>{item.mo_ta || JSON.stringify(item.chi_tiet)}</small></div></article>)}</div></> : <div className="cine-dashboard-empty">Chọn một incident để xem timeline và xử lý.</div>}</div>
          </div>
        </div>
