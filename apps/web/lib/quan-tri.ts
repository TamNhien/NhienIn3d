...
export type OpsProbeFleetAgent = { agent_id: string; required: boolean; managed: boolean; source: "PROFILE" | "KEYRING" | "UNMANAGED" | string; key_configured: boolean; per_agent_key: boolean; asymmetric_key?: boolean; registered: boolean; status: "ONLINE" | "STALE" | "OFFLINE" | "MISSING" | string; heartbeat_age_seconds: number | null; expected_region?: string | null; expected_node_name?: string | null; region: string; node_name: string; phien_ban?: string | null; lan_heartbeat?: string | null; lan_mau?: string | null; warnings: string[] };
export type OpsProbeFleet = { phien_ban: string; mode: string; stale_after_seconds: number; offline_after_seconds: number; expected: number; configured_profiles: number; per_agent_keys: number; asymmetric_public_keys?: number; shared_secret_enabled: boolean; registered: number; online: number; stale: number; offline: number; missing: number; unmanaged: number; registration_coverage_percent: number; online_coverage_percent: number; key_coverage_percent: number; ready: boolean; warnings: string[]; agents: OpsProbeFleetAgent[]; secret_values_exposed: false; loi?: string };
...
export type OpsQuorumEndpoint = { endpoint_id: string; expected_regions: number; observed_regions: number; healthy_regions: number; quorum_required: number; quorum_met: boolean; consensus: "QUORUM_OK" | "DEGRADED" | "OUTAGE" | string; disagreement: boolean; anomaly_regions: string[]; regions: OpsQuorumRegion[] };
export type OpsMultiRegionQuorum = { phien_ban: string; config: { window_seconds: number; min_regions: number; healthy_percent: number; anomaly_lookback_minutes: number; anomaly_min_samples: number; anomaly_latency_multiplier: number }; expected_regions: string[]; summary: { endpoints: number; quorum_ok: number; degraded: number; outage: number; disagreements: number; anomalies: number }; ready: boolean; endpoints: OpsQuorumEndpoint[]; loi?: string };
...
export const layOpsOnCallAdmin = () => goi<{ schedules: OpsOnCallScheduleAdmin[]; policies: OpsEscalationPolicyAdmin[]; current: { at: string; service: string; current: OpsOnCallScheduleAdmin[]; policies: OpsEscalationPolicyAdmin[] } }>("/quan-tri/he-thong/ops/on-call");
