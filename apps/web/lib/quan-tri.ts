...
export type SloEndpointCheckAdmin = { id: string; ten: string; path: string; muc_tieu_percent: number; timeout_ms: number };
export type CauHinhSloNangCaoAdmin = { burn_windows: Array<{ gio: number; nguong: number; muc_do: "CANH_BAO" | "CAO" | "NGHIEM_TRONG" | string }>; service_targets: { api: number; postgresql: number; backup: number; smtp: number }; endpoint_checks: SloEndpointCheckAdmin[]; nguon_cau_hinh: string; ngay_cap_nhat?: string | null };
export type WebhookDeliveryAdmin = LichSuVanHanhAdmin;
export type WebhookDeadLetterAdmin = LichSuVanHanhAdmin & { da_replay: boolean };
export type EndpointSloAdmin = SloEndpointCheckAdmin & { tong_mau: number; tong_thoi_gian_phut: number; downtime_phut: number; availability_percent: number | null; dat_muc_tieu: boolean | null; error_budget_da_tieu_percent: number | null; error_budget_con_lai_percent: number | null };
...
export type AdminSucKhoeHeThong = {
  ...
  webhook: { bat: boolean; san_sang: boolean; endpoint: string; timeout_ms: number; max_retries?: number; backoff_ms?: number; co_hmac?: boolean; adapter?: "GENERIC" | "SLACK" | "TEAMS" | "DISCORD" | string };
  van_de?: string[];
...
export const capNhatCauHinhSloNangCaoAdmin = (payload: Pick<CauHinhSloNangCaoAdmin, "burn_windows" | "service_targets" | "endpoint_checks">) => goi<CauHinhSloNangCaoAdmin>("/quan-tri/he-thong/cau-hinh-slo-nang-cao", { method: "POST", body: JSON.stringify(payload) });
...
export const layWebhookDeadLetterAdmin = (gioi_han = 30) => goi<{ du_lieu: WebhookDeadLetterAdmin[]; gioi_han: number; cau_hinh: AdminSucKhoeHeThong["webhook"] }>(`/quan-tri/he-thong/webhook/dead-letter?gioi_han=${gioi_han}`);
export const replayWebhookDeadLetterAdmin = (id: string) => goi<{ dead_letter_id: string; da_gui: boolean; ly_do?: string; http_status?: number; so_lan_thu: number; adapter: string }>(`/quan-tri/he-thong/webhook/dead-letter/${encodeURIComponent(id)}/replay`, { method: "POST" });
