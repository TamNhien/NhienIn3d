...
export type OpsProbeFleetAgent = { agent_id: string; required: boolean; managed: boolean; source: "PROFILE" | "KEYRING" | "UNMANAGED" | string; key_configured: boolean; per_agent_key: boolean; registered: boolean; status: "ONLINE" | "STALE" | "OFFLINE" | "MISSING" | string; heartbeat_age_seconds: number | null; expected_region?: string | null; expected_node_name?: string | null; region: string; node_name: string; phien_ban?: string | null; lan_heartbeat?: string | null; lan_mau?: string | null; warnings: string[] };
export type OpsProbeFleet = { phien_ban: string; mode: string; stale_after_seconds: number; offline_after_seconds: number; expected: number; configured_profiles: number; per_agent_keys: number; shared_secret_enabled: boolean; registered: number; online: number; stale: number; offline: number; missing: number; unmanaged: number; registration_coverage_percent: number; online_coverage_percent: number; key_coverage_percent: number; ready: boolean; warnings: string[]; agents: OpsProbeFleetAgent[]; secret_values_exposed: false; loi?: string };
