...
export type ProbeRolloutProposalAdmin = { id: string; status: "PENDING" | "APPLIED" | "EXPIRED" | "CANCELLED" | "REJECTED"; base_revision: number; proposed_at: string; expires_at: string; proposed_by_id: string; proposed_by: string; approved_at?: string | null; approved_by?: string | null; applied_revision?: number | null; proposal_sha256?: string | null; rejected_at?: string | null; rejected_by?: string | null; cancelled_at?: string | null; cancelled_by?: string | null; decision_note?: string; approval_health_snapshot?: Record<string, unknown> | null; diff: Record<string, { before: unknown; after: unknown }> };
export type ProbeDesiredStateUpdateAdmin = ProbeDesiredStateAdmin & { pending_approval?: boolean; rollout_approval?: { required: boolean; ttl_minutes: number; two_person_rule: boolean; require_healthy_preflight?: boolean; proposal_sha256_valid?: boolean; proposal?: ProbeRolloutProposalAdmin | null; secret_values_exposed?: false } };
...
export const verifyRecoveryEvidenceAdmin = () => goi<{ overall_verified: boolean; sha256_valid: boolean; signature_configured: boolean; signature_valid: boolean; fingerprint_valid: boolean; reason: string }>("/quan-tri/he-thong/ops/recovery/evidence-verify");
