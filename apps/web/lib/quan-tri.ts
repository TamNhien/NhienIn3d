...
export type XungDotPhanCaAdmin = {
  phien_ban: string; tu_ngay: string; den_ngay: string; tong_phan_ca: number; tong_xung_dot: number; nhan_vien_bi_xung_dot: number; overlap_guard_enabled: boolean;
  xung_dot: Array<{
    ngay_lam: string; nhan_vien_id: string; ma_nhan_vien: string; ho_ten: string;
    phan_ca_a: { id: string; ma_ca: string; ten_ca: string; gio_bat_dau: string; gio_ket_thuc: string };
    phan_ca_b: { id: string; ma_ca: string; ten_ca: string; gio_bat_dau: string; gio_ket_thuc: string };
  }>;
};


export type AdminThanhToanTomTat = { trang_thai: string; ma_giao_dich: string };
export type AdminThanhToanChiTiet = { id: string; ma_giao_dich: string; so_tien: number; trang_thai: string; ngay_tao: string; ngay_cap_nhat?: string; ngay_thanh_toan?: string | null; phuong_thuc: { ten_phuong_thuc: string; ma_phuong_thuc: string } };
...
export type HoanTienCanXuLyAdmin = {
  phien_ban: string; tong_don_can_hoan: number; tong_tien_can_hoan: number; tong_qua_han?: number; tong_sap_den_han?: number; refund_sla_hours?: number; refund_due_soon_hours?: number; gateway_auto_refund: boolean; manual_confirmation_required: boolean; sla_is_operational_target_only?: boolean;
  items: Array<{ id: string; ma_don_hang: string; ho_ten_nguoi_nhan: string; thu_dien_tu?: string | null; ngay_huy_cap_nhat: string; refund_due_at?: string; refund_age_hours?: number; refund_remaining_hours?: number; refund_sla_status?: "TRONG_HAN" | "SAP_DEN_HAN" | "QUA_HAN" | string; so_giao_dich: number; so_tien_can_hoan: number; giao_dich: Array<{ id: string; ma_giao_dich: string; so_tien: number; phuong_thuc: string; ngay_thanh_toan?: string | null }> }>;
};
...
export type IncidentPostmortemActionAdmin = { id: string; title: string; owner: string; status: "OPEN" | "IN_PROGRESS" | "DONE"; due_date: string | null; severity?: "P1" | "P2" | "P3" | "P4"; service?: string };
...
export type ProbeRolloutProposalAdmin = { id: string; status: "PENDING" | "APPLIED" | "EXPIRED" | "CANCELLED" | "REJECTED"; base_revision: number; proposed_at: string; expires_at: string; proposed_by_id: string; proposed_by: string; approved_at?: string | null; approved_by?: string | null; applied_revision?: number | null; proposal_sha256?: string | null; proposal_envelope_sha256?: string | null; decision_receipt_sha256?: string | null; rejected_at?: string | null; rejected_by?: string | null; cancelled_at?: string | null; cancelled_by?: string | null; decision_note?: string; approval_health_snapshot?: Record<string, unknown> | null; diff: Record<string, { before: unknown; after: unknown }> };
export type ProbeDesiredStateUpdateAdmin = ProbeDesiredStateAdmin & { pending_approval?: boolean; rollout_approval?: { required: boolean; ttl_minutes: number; two_person_rule: boolean; require_healthy_preflight?: boolean; proposal_sha256_valid?: boolean; proposal_envelope_sha256_valid?: boolean; decision_receipt_sha256_valid?: boolean; receipt_chain_valid?: boolean; receipt_chain_reason?: string; receipt_chain_length?: number; receipt_chain_head_sha256?: string | null; receipt_chain_history_limit?: number; proposal?: ProbeRolloutProposalAdmin | null; secret_values_exposed?: false } };
...
export type OpsRuntimeAdmin = { phien_ban: string; probe_health_gate?: { phien_ban: string; status: string; active: boolean; healthy: boolean; in_grace: boolean; enabled: boolean; auto_rollback: boolean; min_online_percent: number; max_quorum_failures: number; grace_seconds: number; max_burn_rate?: number; burn_rate_max_observed?: number | null; burn_rate_blocked?: boolean; rollback_cooldown_minutes?: number; cooldown_remaining_minutes?: number; rollback_approval_required?: boolean; rollback_approved_for_revision?: boolean; online_percent: number; quorum_failures: number; revision: number; previous_available: boolean; remote_code_execution: false }; service_runbooks?: { configured: boolean; runbooks: Record<string, string>; https_only: boolean; secret_values_exposed: false }; probe_agent: { agent_id: string; region: string; node_name: string }; endpoint_samples: number; dlq: { chu_ky_phut: number; max_attempts: number; payload_encryption_ready: boolean; key_id?: string | null; key_source?: string | null }; ops_metrics: { refresh_phut: number; retention_days: number; cache?: Record<string, unknown> | null }; rbac: { active_assignments: number; roles: string[] }; distributed_probe?: { stale_after_seconds: number; online: number; offline: number; agents: OpsProbeAgentHealth[] }; probe_fleet?: OpsProbeFleet; multi_region_quorum?: OpsMultiRegionQuorum; asymmetric_probe_signing?: { algorithm: string; public_keyring_configured: boolean; hmac_backward_compatible: boolean; key_lifecycle?: { configured_keys: number; active_keys: number; revoked_keys: number; disabled_keys: number; expired_keys: number; not_yet_valid_keys: number; expiring_soon_keys: number; expiry_warn_days: number; secret_values_exposed: false }; secret_values_exposed: false }; quorum_alerting?: { enabled: boolean; active: boolean; level: string; issues: string[] }; service_dependency?: OpsServiceDependency; probe_enrollment?: { configured: boolean; ttl_minutes: number; enrolled_public_keys: number; token_visible_once: boolean; private_key_exposed: false; require_device_id?: boolean; rotation_days?: number; device_bound_agents?: number; rotation_due_agents?: number; raw_device_id_exposed?: false }; archive_portability?: { format: string; archived_batches: number; s3_presigned_upload: boolean; s3_allowed_hosts_configured: number; restore_replay_supported: boolean; retention_class: string; secret_values_exposed: false }; probe_desired_state?: ProbeDesiredStateAdmin; incident_postmortem?: { postmortems: number; complete: number; draft: number; open_actions: number; done_actions: number; approved?: number; pending_approval?: number; changes_requested?: number; overdue_actions?: number; due_soon_actions?: number; approval_required?: boolean; timeline_snapshot: boolean; https_runbook_only: boolean }; database_recovery?: DatabaseRecoveryAdmin & { evidence_export_supported?: boolean; evidence_history_count?: number; evidence_retention_runs?: number; evidence_bundle_file?: string; evidence_bundle_version?: string | null; evidence_current_version?: boolean; evidence_sha256_supported?: boolean; evidence_ed25519_supported?: boolean; evidence_sha256?: string | null; evidence_sha256_verified?: boolean; evidence_ed25519_configured?: boolean; evidence_signature_verified?: boolean; evidence_signature_key_id?: string | null; evidence_signature_public_fingerprint?: string | null; evidence_trusted_key_required?: boolean; evidence_trust_configured?: boolean; evidence_key_trusted?: boolean; evidence_trust_source?: string; evidence_trusted_key_ids?: string[]; evidence_key_revoked?: boolean; evidence_revocation_configured?: boolean; evidence_revoked_key_ids?: string[]; evidence_revoked_fingerprints?: number; evidence_verification?: { sha256_valid?: boolean; signature_configured?: boolean; signature_valid?: boolean; fingerprint_valid?: boolean; trusted_key_required?: boolean; trust_configured?: boolean; key_trusted?: boolean; trust_source?: string; key_revoked?: boolean; revocation_configured?: boolean; overall_verified?: boolean; reason?: string }; audit_bundle_ready?: boolean; private_key_exposed?: false }; rollout_approval?: { required: boolean; configured_required: boolean; ttl_minutes: number; two_person_rule: boolean; audit_diff: boolean; environment: string; require_healthy_preflight?: boolean; reject_supported?: boolean; cancel_supported?: boolean; proposal_sha256?: boolean; proposal_sha256_valid?: boolean; proposal_envelope_sha256?: boolean; proposal_envelope_sha256_valid?: boolean; decision_receipt_sha256?: boolean; decision_receipt_sha256_valid?: boolean; decision_receipt_fail_closed?: boolean; decision_receipt_hash_chain?: boolean; decision_receipt_chain_fail_closed?: boolean; receipt_chain_valid?: boolean; receipt_chain_reason?: string; receipt_chain_length?: number; receipt_chain_head_sha256?: string | null; receipt_chain_history_limit?: number; proposal?: ProbeRolloutProposalAdmin | null; secret_values_exposed?: false }; remediation_backlog?: { open_actions: number; overdue_actions: number; due_soon_actions: number; unowned_actions: number; owners: Record<string, number>; by_severity?: Record<string, number>; sla_hours?: Record<string, number>; sla_breached_actions?: number; sla_due_soon_actions?: number; acknowledged_services?: number; retry_pending_services?: number; service_status?: Record<string, { breached_actions?: number; fingerprint?: string; escalation_level?: number; acknowledged?: boolean; snooze_until?: string | null; retry_pending?: boolean; next_retry_at?: string | null }>; on_call_escalation_ready: boolean; on_call_escalation?: { enabled: boolean; interval_hours: number; daily_dedup: boolean; ack_snooze_hours?: number; retry_base_minutes?: number; fingerprint_dedup?: boolean; acknowledgement_supported?: boolean }; items?: Array<Record<string, unknown>> }; on_call_v3160?: { calendar_format: string; calendar_import_export: boolean; handoff_report: boolean; escalation_acknowledgment: boolean; active_overrides: number; current: OpsOnCallScheduleAdmin[] }; dlq_keyring?: DlqKeyringAdmin; replay_jobs?: { cho_xu_ly: number; dang_xu_ly: number }; on_call?: { at: string; service: string; current: OpsOnCallScheduleAdmin[]; policies: OpsEscalationPolicyAdmin[] }; archive?: { retention: Record<string, number>; partitioned_store: string; verify_before_prune: boolean } };
...
export type NhaCungCapAdmin = { id: string; ma_nha_cung_cap: string; ten_nha_cung_cap: string; nguoi_lien_he?: string | null; so_dien_thoai?: string | null; thu_dien_tu?: string | null; dia_chi?: string | null; ghi_chu?: string | null; dang_hoat_dong: boolean; thoi_gian_giao_hang_ngay: number; so_phieu_nhap: number; so_don_mua: number; ngay_tao: string; ngay_cap_nhat: string };
...
export type KeHoachNhapKhoAdmin = {
  phien_ban: string; tao_luc: string; history_days?: number; forecast_days?: number; forecast_basis?: string; tong_bien_the_can_nhap: number; tong_so_luong_de_xuat: number; het_hang: number; rui_ro_du_bao?: number; ap_luc_gio_hang?: number; tong_nhu_cau_gio_hang_mo?: number; legacy_unmapped_sales_lines?: number; khong_co_nha_cung_cap: number; nha_cung_cap_ngung_hoat_dong: number; supplier_inference: string; write_operation: boolean; auto_purchase_order?: boolean; cart_demand_is_reservation?: boolean;
  tong_nhom_nha_cung_cap?: number; nhom_san_sang?: number; nhom_can_xu_ly_thu_cong?: number; grouped_supplier_procurement_plan?: boolean; reorder_point_enabled?: boolean; supplier_lead_time_enabled?: boolean;
  theo_nha_cung_cap: Array<{ nha_cung_cap: { id: string | null; ma_nha_cung_cap: string | null; ten_nha_cung_cap: string; dang_hoat_dong: boolean | null } | null; so_bien_the: number; so_luong_de_xuat: number; trang_thai_nhom?: "SAN_SANG" | "CHUA_GAN_NCC" | "NCC_NGUNG_HOAT_DONG" | string; ready_to_send?: boolean; manual_supplier_resolution_required?: boolean; lead_time_days?: number }>;
  items: Array<{
    bien_the_id: string; ma_bien_the: string; ma_san_pham: string; ten_san_pham: string; vat_lieu: string; mau_sac: string;
    ton_hien_tai: number; ton_toi_thieu: number; ton_toi_da: number; muc_tieu_sau_nhap: number; so_luong_de_xuat: number; de_xuat_theo_dinh_muc?: number; de_xuat_theo_du_bao?: number; muc_do: string;
    ban_30_ngay?: number; ban_trung_binh_ngay?: number; forecast_days?: number; forecast_demand?: number; days_of_cover?: number | null; active_cart_demand?: number; cart_pressure?: boolean; cart_demand_is_reservation?: boolean;
    nha_cung_cap: { id: string | null; ma_nha_cung_cap: string | null; ten_nha_cung_cap: string; dang_hoat_dong: boolean | null } | null;
    lead_time_days?: number; safety_stock?: number; reorder_point?: number; de_xuat_theo_reorder_point?: number;
    nguon_nha_cung_cap: string; phieu_nhap_gan_nhat: { ma_phieu: string; ngay_tao: string } | null;
  }>;
};

export type DonMuaHangAdmin = { id:string; ma_don_mua:string; trang_thai:string; ngay_du_kien?:string|null; ghi_chu?:string|null; tong_so_luong:number; tong_gia_tri:number|string; ngay_tao:string; nha_cung_cap:{id:string;ma_nha_cung_cap:string;ten_nha_cung_cap:string;thoi_gian_giao_hang_ngay:number}; chi_tiet:Array<{id:string;bien_the_id:string;ma_bien_the:string;so_luong_dat:number;so_luong_da_nhan:number;don_gia_nhap:number|string;ghi_chu?:string|null}>; phieu_nhap?:Array<{id:string;ma_phieu:string;tong_so_luong:number;ngay_tao:string}> };
export type PhienKiemKeKhoAdmin = { id:string; ma_phien:string; trang_thai:string; ghi_chu?:string|null; tong_dong:number; tong_chenh_lech_tuyet_doi:number; ngay_tao:string; ngay_gui_duyet?:string|null; ngay_duyet?:string|null; chi_tiet:Array<{id:string;bien_the_id:string;ma_bien_the:string;ton_snapshot:number;ton_thuc_te:number;chenh_lech:number;ly_do?:string|null}> };
...
export const capNhatProbeDesiredStateAdmin = (payload: { target_version: string; interval_seconds: number; rollout_percent: number; canary_agents: string[]; paused: boolean; note?: string }) => goi<ProbeDesiredStateUpdateAdmin>("/quan-tri/he-thong/ops/probe-desired-state", { method: "POST", body: JSON.stringify(payload) });
...
export const approveProbeRolloutAdmin = (proposal_id: string, note = "") => goi<ProbeDesiredStateUpdateAdmin>("/quan-tri/he-thong/ops/probe-desired-state/proposal/approve", { method: "POST", body: JSON.stringify({ proposal_id, note }) });
export const rejectProbeRolloutAdmin = (proposal_id: string, note = "") => goi<{ proposal?: ProbeRolloutProposalAdmin | null; proposal_sha256_valid?: boolean }>("/quan-tri/he-thong/ops/probe-desired-state/proposal/reject", { method: "POST", body: JSON.stringify({ proposal_id, note }) });
export const cancelProbeRolloutAdmin = (proposal_id: string, note = "") => goi<{ proposal?: ProbeRolloutProposalAdmin | null; proposal_sha256_valid?: boolean }>("/quan-tri/he-thong/ops/probe-desired-state/proposal/cancel", { method: "POST", body: JSON.stringify({ proposal_id, note }) });
export const xuatRecoveryEvidenceBundleAdmin = () => goi<{ ten_file: string; mime_type: string; base64: string; verification?: Record<string, unknown> }>("/quan-tri/he-thong/ops/recovery/evidence-bundle");
export const verifyRecoveryEvidenceAdmin = () => goi<{ overall_verified: boolean; sha256_valid: boolean; signature_configured: boolean; signature_valid: boolean; fingerprint_valid: boolean; trusted_key_required?: boolean; trust_configured?: boolean; key_trusted?: boolean; trust_source?: string; reason: string }>("/quan-tri/he-thong/ops/recovery/evidence-verify");
export const xuatRemediationBacklogExcelAdmin = () => goi<{ ten_file: string; mime_type: string; base64: string; tong: number; sla_breached: number; acknowledged_services?: number; retry_pending_services?: number }>("/quan-tri/he-thong/ops/remediation/excel");
export const escalateRemediationBacklogAdmin = () => goi<{ sent: number; skipped: number; failures?: string[]; details?: Array<Record<string, unknown>> }>("/quan-tri/he-thong/ops/remediation/escalate", { method: "POST" });
export const acknowledgeRemediationBacklogAdmin = (service: string, note = "", snooze_hours = 24) => goi<{ service: string; snooze_until: string; fingerprint: string }>("/quan-tri/he-thong/ops/remediation/acknowledge", { method: "POST", body: JSON.stringify({ service, note, snooze_hours }) });
...
export const layXungDotPhanCaAdmin = (tu_ngay = "", den_ngay = "") => { const q = new URLSearchParams(); if (tu_ngay) q.set("tu_ngay", tu_ngay); if (den_ngay) q.set("den_ngay", den_ngay); return goi<XungDotPhanCaAdmin>(`/quan-tri/phan-ca/xung-dot${q.size ? `?${q}` : ""}`); };
...
export type TaoNhaCungCapAdminPayload = Pick<NhaCungCapAdmin, "ma_nha_cung_cap" | "ten_nha_cung_cap" | "nguoi_lien_he" | "so_dien_thoai" | "thu_dien_tu" | "dia_chi" | "ghi_chu" | "dang_hoat_dong" | "thoi_gian_giao_hang_ngay">;
export const taoNhaCungCapAdmin = (payload: TaoNhaCungCapAdminPayload) => goi<NhaCungCapAdmin>("/quan-tri/nha-cung-cap", { method: "POST", body: JSON.stringify(payload) });
export const capNhatNhaCungCapAdmin = (id: string, payload: Partial<Pick<NhaCungCapAdmin, "ten_nha_cung_cap" | "nguoi_lien_he" | "so_dien_thoai" | "thu_dien_tu" | "dia_chi" | "ghi_chu" | "dang_hoat_dong" | "thoi_gian_giao_hang_ngay">>) => goi<NhaCungCapAdmin>(`/quan-tri/nha-cung-cap/${id}/cap-nhat`, { method: "POST", body: JSON.stringify(payload) });
...
export const nhapKhoTheoLoAdmin = (payload: { ma_lo?: string; nha_cung_cap_id?: string; nha_cung_cap?: string; ghi_chu?: string; don_mua_hang_id?: string; cho_phep_vuot_don_mua?: boolean; dong: Array<{ ma_bien_the: string; so_luong_nhap: number; ly_do?: string }> }) => goi<PhieuNhapKhoAdmin>("/quan-tri/kho/nhap-lo", { method: "POST", body: JSON.stringify(payload) });
...
export const layDonMuaHangAdmin = (trang_thai="") => goi<DonMuaHangAdmin[]>(`/quan-tri/kho/don-mua${trang_thai?`?trang_thai=${encodeURIComponent(trang_thai)}`:""}`);
export const taoDonMuaHangAdmin = (payload:{nha_cung_cap_id:string;ngay_du_kien?:string;ghi_chu?:string;dong:Array<{bien_the_id:string;so_luong_dat:number;don_gia_nhap:number;ghi_chu?:string}>}) => goi<DonMuaHangAdmin>("/quan-tri/kho/don-mua",{method:"POST",body:JSON.stringify(payload)});
export const capNhatTrangThaiDonMuaAdmin = (id:string,trang_thai:"DA_DUYET"|"DA_DAT"|"HUY",ghi_chu?:string) => goi<DonMuaHangAdmin>(`/quan-tri/kho/don-mua/${id}/trang-thai`,{method:"POST",body:JSON.stringify({trang_thai,ghi_chu})});
export const xuatDonMuaExcelAdmin = () => goi<{ten_file:string;mime_type:string;base64:string}>("/quan-tri/kho/don-mua-excel");
export const layPhienKiemKeKhoAdmin = () => goi<PhienKiemKeKhoAdmin[]>("/quan-tri/kho/kiem-ke/phien");
export const taoPhienKiemKeKhoAdmin = (dong:KiemKeKhoDongAdmin[],ghi_chu?:string) => goi<PhienKiemKeKhoAdmin>("/quan-tri/kho/kiem-ke/phien",{method:"POST",body:JSON.stringify({dong,ghi_chu})});
export const guiDuyetPhienKiemKeKhoAdmin = (id:string) => goi<PhienKiemKeKhoAdmin>(`/quan-tri/kho/kiem-ke/phien/${id}/gui-duyet`,{method:"POST"});
export const duyetApDungPhienKiemKeKhoAdmin = (id:string) => goi<PhienKiemKeKhoAdmin & {ket_qua_ap_dung?:KiemKeKhoApplyAdmin}>(`/quan-tri/kho/kiem-ke/phien/${id}/duyet-ap-dung`,{method:"POST"});
export const huyPhienKiemKeKhoAdmin = (id:string) => goi<PhienKiemKeKhoAdmin>(`/quan-tri/kho/kiem-ke/phien/${id}/huy`,{method:"POST"});
export const layGoiYNhapKhoAdmin = () => goi<KeHoachNhapKhoAdmin>("/quan-tri/kho/goi-y-nhap");
