...
export type WebhookDeadLetterAdmin = LichSuVanHanhAdmin & { da_replay: boolean; da_ack: boolean; het_han: boolean; trang_thai_dlq: "CHO_REPLAY" | "DA_REPLAY" | "DA_ACK" | "HET_HAN" | string; idempotency_key: string; het_han_luc: string };
export type EndpointSloAdmin = SloEndpointCheckAdmin & { tong_mau: number; tong_thoi_gian_phut: number; downtime_phut: number; budget_downtime_phut: number; excluded_maintenance_phut: { availability: number; error_budget: number }; availability_percent: number | null; dat_muc_tieu: boolean | null; error_budget_da_tieu_percent: number | null; error_budget_con_lai_percent: number | null; latency: { samples: number; sli_percent: number | null; avg_ms: number | null; p50_ms: number | null; p95_ms: number | null; p99_ms: number | null; target_ms: number; dat_p95: boolean | null; histogram: Record<string, number> } };
...
export type AdminSucKhoeHeThong = {
  ...
  webhook: { bat: boolean; san_sang: boolean; endpoint: string; timeout_ms: number; max_retries?: number; backoff_ms?: number; co_hmac?: boolean; adapter?: "GENERIC" | "SLACK" | "TEAMS" | "DISCORD" | string; dlq_retention_days?: number; replay_allow_duplicate?: boolean };
  van_de?: string[];
