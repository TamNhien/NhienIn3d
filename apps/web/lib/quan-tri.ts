...
export type ProbeRolloutProposalAdmin = { id: string; status: "PENDING" | "APPLIED" | "EXPIRED" | "CANCELLED"; base_revision: number; proposed_at: string; expires_at: string; proposed_by_id: string; proposed_by: string; approved_at?: string | null; approved_by?: string | null; applied_revision?: number | null; diff: Record<string, { before: unknown; after: unknown }> };
export type ProbeDesiredStateUpdateAdmin = ProbeDesiredStateAdmin & { pending_approval?: boolean; rollout_approval?: { required: boolean; ttl_minutes: number; two_person_rule: boolean; proposal?: ProbeRolloutProposalAdmin | null; secret_values_exposed?: false } };
...
export const xuatRecoveryEvidenceBundleAdmin = () => goi<{ ten_file: string; mime_type: string; base64: string }>("/quan-tri/he-thong/ops/recovery/evidence-bundle");
export const xuatRemediationBacklogExcelAdmin = () => goi<{ ten_file: string; mime_type: string; base64: string; tong: number; sla_breached: number }>("/quan-tri/he-thong/ops/remediation/excel");
export const escalateRemediationBacklogAdmin = () => goi<{ sent: number; skipped: number; failures?: string[] }>("/quan-tri/he-thong/ops/remediation/escalate", { method: "POST" });
