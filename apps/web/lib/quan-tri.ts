...
export type BaoTriHeThongAdmin = { bat: boolean; bat_dau: string | null; ket_thuc: string | null; ly_do: string; dang_bao_tri: boolean; sap_bao_tri: boolean; da_ket_thuc: boolean; nguon_cau_hinh: string; ngay_cap_nhat?: string | null };
...
export type SlaVanHanhAdmin = { so_ngay: number; tu_ngay: string; tao_luc: string; dinh_nghia: { sla: string; uptime: string; error_budget: string }; muc_tieu: { sla_muc_tieu_percent: number; uptime_muc_tieu_percent: number; canh_bao_xu_huong: boolean; nguon_cau_hinh: string }; tong_quan: SlaTongQuanAdmin; xu_huong: { bay_ngay: SlaTongQuanAdmin; ba_muoi_ngay: SlaTongQuanAdmin }; ngan_sach_loi: { sla: ErrorBudgetAdmin; uptime: ErrorBudgetAdmin }; burn_rate: { sla: { mot_gio: BurnRateWindowAdmin; sau_gio: BurnRateWindowAdmin; hai_muoi_bon_gio: BurnRateWindowAdmin }; uptime: { mot_gio: BurnRateWindowAdmin; sau_gio: BurnRateWindowAdmin; hai_muoi_bon_gio: BurnRateWindowAdmin } }; canh_bao: string[]; theo_ngay: Array<{ ngay: string; tong: number; tot: number; canh_bao: number; loi: number; sla_percent: number | null; uptime_percent: number | null }> };
...
export type AdminSucKhoeHeThong = {
  ...
  webhook: { bat: boolean; san_sang: boolean; endpoint: string; timeout_ms: number };
  van_de?: string[];
...
export const layDanhSachSuCoVanHanhAdmin = (gioi_han = 20, trang_thai_xu_ly = "") => { const q = new URLSearchParams({ gioi_han: String(gioi_han) }); if (trang_thai_xu_ly) q.set("trang_thai_xu_ly", trang_thai_xu_ly); return goi<{ du_lieu: SuCoVanHanhTomTat[]; gioi_han: number; nguon: string }>(`/quan-tri/he-thong/su-co?${q}`); };
...
export const xuatDanhSachSuCoExcelAdmin = (trang_thai_xu_ly = "") => { const q = new URLSearchParams(); if (trang_thai_xu_ly) q.set("trang_thai_xu_ly", trang_thai_xu_ly); return goi<{ ten_file: string; mime_type: string; base64: string }>(`/quan-tri/he-thong/su-co/excel${q.size ? `?${q}` : ""}`); };
