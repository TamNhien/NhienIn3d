...
export type SlaVanHanhAdmin = { so_ngay: number; tu_ngay: string; tao_luc: string; dinh_nghia: { sla: string; uptime: string }; muc_tieu: { sla_muc_tieu_percent: number; uptime_muc_tieu_percent: number; canh_bao_xu_huong: boolean; nguon_cau_hinh: string }; tong_quan: SlaTongQuanAdmin; xu_huong: { bay_ngay: SlaTongQuanAdmin; ba_muoi_ngay: SlaTongQuanAdmin }; canh_bao: string[]; theo_ngay: Array<{ ngay: string; tong: number; tot: number; canh_bao: number; loi: number; sla_percent: number | null; uptime_percent: number | null }> };
