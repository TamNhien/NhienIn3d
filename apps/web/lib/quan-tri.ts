...
export type CauHinhSloNangCaoAdmin = { burn_windows: Array<{ gio: number; nguong: number; muc_do: "CANH_BAO" | "CAO" | "NGHIEM_TRONG" | string }>; service_targets: { api: number; postgresql: number; backup: number; smtp: number }; nguon_cau_hinh: string; ngay_cap_nhat?: string | null };
...
export type SlaVanHanhAdmin = { so_ngay: number; tu_ngay: string; tao_luc: string; dinh_nghia: { sla: string; uptime: string; error_budget: string }; muc_tieu: { sla_muc_tieu_percent: number; uptime_muc_tieu_percent: number; canh_bao_xu_huong: boolean; nguon_cau_hinh: string }; cau_hinh_nang_cao?: CauHinhSloNangCaoAdmin; tong_quan: SlaTongQuanAdmin; xu_huong: { bay_ngay: SlaTongQuanAdmin; ba_muoi_ngay: SlaTongQuanAdmin }; ngan_sach_loi: { sla: ErrorBudgetAdmin; uptime: ErrorBudgetAdmin }; ngan_sach_dich_vu?: Record<string, ErrorBudgetAdmin & { muc_tieu_percent: number }>; burn_rate: { sla: { mot_gio: BurnRateWindowAdmin; sau_gio: BurnRateWindowAdmin; hai_muoi_bon_gio: BurnRateWindowAdmin }; uptime: { mot_gio: BurnRateWindowAdmin; sau_gio: BurnRateWindowAdmin; hai_muoi_bon_gio: BurnRateWindowAdmin } }; burn_rate_policy?: Array<{ gio: number; nguong: number; muc_do: string; sla: BurnRateWindowAdmin & { vuot_nguong: boolean; muc_do_policy: string }; uptime: BurnRateWindowAdmin & { vuot_nguong: boolean; muc_do_policy: string } }>; incident_metrics?: { tong_incident: number; dang_mo: number; da_khac_phuc: number; mtta_phut: number | null; mtta_p95_phut: number | null; mttr_phut: number | null; mttr_p95_phut: number | null }; canh_bao: string[]; theo_ngay: Array<{ ngay: string; tong: number; tot: number; canh_bao: number; loi: number; sla_percent: number | null; uptime_percent: number | null }> };
...
export type AdminSucKhoeHeThong = {
  ...
  webhook: { bat: boolean; san_sang: boolean; endpoint: string; timeout_ms: number; max_retries?: number; backoff_ms?: number; co_hmac?: boolean };
  van_de?: string[];
...
export const capNhatCauHinhSloNangCaoAdmin = (payload: Pick<CauHinhSloNangCaoAdmin, "burn_windows" | "service_targets">) => goi<CauHinhSloNangCaoAdmin>("/quan-tri/he-thong/cau-hinh-slo-nang-cao", { method: "POST", body: JSON.stringify(payload) });
