...
export type OpsRuntimeAdmin = { phien_ban: string; probe_agent: { agent_id: string; region: string; node_name: string }; endpoint_samples: number; dlq: { chu_ky_phut: number; max_attempts: number; payload_encryption_ready: boolean; key_id?: string | null; key_source?: string | null }; ops_metrics: { refresh_phut: number; retention_days: number; cache?: Record<string, unknown> | null }; rbac: { active_assignments: number; roles: string[] } };
