...
try {
  if (-not $KhongKiemTraBackupRestore) {
    Write-Host "[E2E v3.6.7] Backup/SHA/restore runtime nền..." -ForegroundColor Cyan
  ...
  Write-Host "[E2E v3.6.7] Preflight API version để tránh kiểm tra nhầm container cũ..." -ForegroundColor Cyan
  $publicHealth = Invoke-RestMethod -Uri "$base/suc-khoe" -Method Get -TimeoutSec 20
  Assert-True ($publicHealth.phien_ban -eq "v3.6.7") "API đang chạy $($publicHealth.phien_ban), không phải v3.6.7. Docker build trước đó có thể đã thất bại và container cũ vẫn đang chạy. Hãy chạy lại: docker compose up -d --build --remove-orphans"

  Write-Host "[E2E v3.6.7] Đăng nhập Admin qua cookie session..." -ForegroundColor Cyan
  $session = New-Object Microsoft.PowerShell.Commands.WebRequestSession
  $loginBody = @{ thu_dien_tu = $adminEmail; mat_khau = $adminPassword; trinh_duyet_hien_thi = "NhienIn3d E2E v3.6.7" } | ConvertTo-Json
  ...
  Write-Host "[E2E v3.6.7] Kiểm tra đơn hàng + sản phẩm..."
  ...
  Write-Host "[E2E v3.6.7] Preview nhập kho CSV, không thay đổi tồn kho..."
  $variant = $null
  ...
  $csv = "ma_bien_the,so_luong_nhap,ly_do`r`n$($variant.ma_bien_the),1,E2E preview v3.6.7`r`n"
  $base64 = [Convert]::ToBase64String([Text.Encoding]::UTF8.GetBytes($csv))
  $previewBody = @{ ten_file = "e2e-v367.csv"; du_lieu_base64 = $base64 } | ConvertTo-Json
  ...
  Write-Host "[E2E v3.6.7] Kiểm tra báo cáo Excel + phiếu nhập..."
  ...
  Write-Host "[E2E v3.6.7] Kiểm tra Ops v3.6: maintenance, error budget, burn-rate, Incident Excel, webhook..."
  ...
  Assert-True ($health.phien_ban -eq "3.6.7") "Health endpoint chưa lên v3.6.7."
  ...
  Assert-True ($null -ne $health.webhook.san_sang) "Health chưa trả trạng thái webhook v3.6.7."
  ...
  Assert-True ($incidents.nguon -eq "BANG_TONG_HOP") "Incident chưa đọc từ bảng tổng hợp v3.6.7."
  ...
  Assert-True ($health.database.migration_gan_nhat.ten -eq "202609010001_v350_incident_slo_rollup") "Migration mới nhất phải vẫn là 202609010001_v350_incident_slo_rollup vì v3.6.7 dùng cau_hinh_he_thong, không cần migration mới."
...
VALUES ('$sig','MOI','["E2E browser v3.6.7 synthetic incident"]'::jsonb,now(),now(),1,1,0,'CANH_BAO',now(),now())
...
VALUES ('HEALTH','CANH_BAO','E2E browser v3.6.7 synthetic incident','{"van_de":["E2E browser v3.6.7 synthetic incident"]}'::jsonb,'$sig',now(),now());
"@
    $sql | docker compose exec -T postgres psql -v ON_ERROR_STOP=1 -U $dbUser -d $dbName | Out-Null
    if ($LASTEXITCODE -ne 0) { throw "Không seed được incident E2E v3.6.7 cho browser CI." }
    Write-Host "Synthetic incident : PASS ($sig)"
  }

  Write-Host "Runtime E2E v3.6.7 PASS ✅" -ForegroundColor Green
