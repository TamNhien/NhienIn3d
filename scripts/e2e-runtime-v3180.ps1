...
try {
  if (-not $KhongKiemTraBackupRestore) {
    Write-Host "[E2E v3.18.0] Backup/SHA/restore runtime nền..." -ForegroundColor Cyan
  ...
  Write-Host "[E2E v3.18.0] Preflight API version để tránh kiểm tra nhầm container cũ..." -ForegroundColor Cyan
  $publicHealth = Invoke-RestMethod -Uri "$base/suc-khoe" -Method Get -TimeoutSec 20
  Assert-True ($publicHealth.phien_ban -eq "v3.18.0") "API đang chạy $($publicHealth.phien_ban), không phải v3.18.0. Docker build trước đó có thể đã thất bại và container cũ vẫn đang chạy. Hãy chạy lại: docker compose up -d --build --remove-orphans"

  Write-Host "[E2E v3.18.0] Đăng nhập Admin qua cookie session..." -ForegroundColor Cyan
  $session = New-Object Microsoft.PowerShell.Commands.WebRequestSession
  $loginBody = @{ thu_dien_tu = $adminEmail; mat_khau = $adminPassword; trinh_duyet_hien_thi = "NhienIn3d E2E v3.18.0" } | ConvertTo-Json
  ...
  Write-Host "[E2E v3.18.0] Kiểm tra đơn hàng + sản phẩm..."
  ...
  Write-Host "[E2E v3.18.0] Preview nhập kho CSV, không thay đổi tồn kho..."
  $variant = $null
  ...
  $csv = "ma_bien_the,so_luong_nhap,ly_do`r`n$($variant.ma_bien_the),1,E2E preview v3.18.0`r`n"
  ...
  Write-Host "[E2E v3.18.0] Kiểm tra báo cáo Excel + phiếu nhập..."
  ...
  Write-Host "[E2E v3.18.0] Kiểm tra Ops: persistent SLI/Apdex, encrypted DLQ scheduler, metrics cache, RBAC on-call..."
  ...
  Assert-True ($health.phien_ban -eq "3.18.0") "Health endpoint chưa lên v3.18.0."
  ...
  Assert-True ($null -ne $maintenanceList.du_lieu) "Danh sách maintenance v3.18.0 chưa phản hồi."
  ...
  Assert-True ($null -ne $sloAdvanced.maintenance_policy) "SLO nang cao chua tra maintenance policy v3.18.0."
  $endpoint0 = @($sloAdvanced.endpoint_checks)[0]
  Assert-True ($endpoint0.method -in @("GET","HEAD")) "Endpoint probe method khong hop le."
  Assert-True ($endpoint0.latency_target_ms -ge 50) "Endpoint probe chua co latency target v3.18.0."
  Assert-True ($null -ne $health.webhook.san_sang) "Health chưa trả trạng thái webhook v3.18.0."
  Assert-True ($health.webhook.adapter -in @("GENERIC","SLACK","TEAMS","DISCORD")) "Webhook adapter preset không hợp lệ."
  Assert-True ($health.webhook.dlq_retention_days -ge 1) "Health chua tra DLQ retention v3.18.0."
  Assert-True ($null -ne $health.webhook.dlq_encryption_ready) "Health chưa trả trạng thái mã hóa DLQ v3.18.0."
  Assert-True ($opsRuntime.phien_ban -eq "3.18.0") "Ops runtime chưa lên v3.18.0."
  ...
  Assert-True ($null -ne $opsRuntime.probe_fleet.agents) "Ops runtime chưa trả managed probe fleet v3.18.0."
  ...
  Assert-True ($null -ne $opsRuntime.multi_region_quorum.summary) "Ops runtime chưa trả multi-region quorum v3.18.0."
  ...
  Assert-True ($opsRuntime.asymmetric_probe_signing.hmac_backward_compatible -eq $true) "v3.18.0 phải giữ HMAC tương thích ngược."
  Assert-True ($opsRuntime.asymmetric_probe_signing.secret_values_exposed -eq $false) "Asymmetric signing không được lộ secret."
  Assert-True ($null -ne $opsRuntime.asymmetric_probe_signing.key_lifecycle.active_keys) "Ops runtime chưa trả Ed25519 key lifecycle v3.18.0."
  ...
  Assert-True ($null -ne $opsRuntime.probe_fleet.asymmetric_key_lifecycle.active_keys) "Managed fleet chưa trả key lifecycle summary v3.18.0."
  ...
  Assert-True ($null -ne $opsRuntime.quorum_alerting.enabled) "Ops runtime chưa trả quorum alert policy v3.18.0."
  Assert-True ($null -ne $opsRuntime.service_dependency.blast_radius) "Ops runtime chưa trả service dependency/blast radius v3.18.0."
  Assert-True ($null -ne $opsRuntime.service_dependency.dependencies) "Ops runtime chưa trả dependency graph v3.18.0."
  Assert-True ($null -ne $opsRuntime.probe_enrollment.configured) "Ops runtime chưa trả probe enrollment state v3.18.0."
  ...
  Assert-True ($null -ne $opsRuntime.probe_enrollment.require_device_id) "Probe enrollment chưa trả device identity policy v3.18.0."
  Assert-True ($opsRuntime.probe_enrollment.rotation_days -ge 1) "Probe enrollment rotation policy không hợp lệ."
  Assert-True ($opsRuntime.archive_portability.format -eq "JSONL+GZIP") "Archive portability v3.18.0 chưa sẵn sàng."
  Assert-True ($opsRuntime.archive_portability.restore_replay_supported -eq $true) "Archive restore/replay v3.18.0 chưa sẵn sàng."
  Assert-True ($null -ne $opsRuntime.probe_desired_state.current.revision) "Ops runtime chưa trả probe desired-state v3.18.0."
  ...
  Assert-True ($null -ne $opsRuntime.probe_health_gate.status) "Ops runtime chưa trả health-gated canary v3.18.0."
  ...
  Assert-True ($opsRuntime.database_recovery.pitr_target_time_supported -eq $true) "Recovery runtime chưa công bố target-time PITR rehearsal v3.18.0."
  ...
  Assert-True ($null -ne $opsRuntime.database_recovery.rpo_target_minutes) "Recovery readiness chưa trả RPO target v3.18.0."
  Assert-True ($null -ne $opsRuntime.database_recovery.rto_target_minutes) "Recovery readiness chưa trả RTO target v3.18.0."
  ...
  Assert-True ($opsRuntime.incident_postmortem.timeline_snapshot -eq $true) "Postmortem runtime chưa công bố timeline snapshot v3.18.0."
  ...
  Assert-True ($opsRuntime.on_call_v3160.calendar_import_export -eq $true) "On-call ICS import/export kế thừa contract v3.16 chưa sẵn sàng trong runtime v3.18.0."
  Assert-True ($opsRuntime.on_call_v3160.handoff_report -eq $true) "On-call handoff kế thừa contract v3.16 chưa sẵn sàng trong runtime v3.18.0."
  ...
  Assert-True ($opsOnCall.calendar_format -eq "ICS") "Ops on-call chưa công bố ICS calendar v3.18.0."
  ...
  Assert-True ($sla.endpoint_slo.maintenance_aware -eq $true) "Endpoint SLO chua bat maintenance-aware v3.18.0."
  ...
  Assert-True ($null -ne $endpointSla0.latency.apdex) "Endpoint SLO chưa trả Apdex v3.18.0."
  Assert-True ($null -ne $endpointSla0.persistent_samples) "Endpoint SLO chưa trả persistent sample count v3.18.0."
  Assert-True ($null -ne $endpointSla0.probe_agents) "Endpoint SLO chưa trả probe agents v3.18.0."
  Assert-True ($null -ne $endpointSla0.by_region) "Endpoint SLO chưa trả breakdown theo region v3.18.0."
  Assert-True ($null -ne $endpointSla0.by_node) "Endpoint SLO chưa trả breakdown theo node v3.18.0."
  ...
  Assert-True ($incidents.nguon -eq "BANG_TONG_HOP") "Incident chưa đọc từ bảng tổng hợp v3.18.0."
  ...
  if ($seedIncident) {
    ...
    $sql = @(
      ...
      "VALUES ('$sig','MOI','[`"E2E browser v3.18.0 synthetic incident`"]'::jsonb,now(),now(),1,1,0,'CANH_BAO',now(),now())",
      ...
      "VALUES ('HEALTH','CANH_BAO','E2E browser v3.18.0 synthetic incident','{`"van_de`":[`"E2E browser v3.18.0 synthetic incident`"]}'::jsonb,'$sig',now(),now());"
    ) -join "`n"
    $sql | docker compose exec -T postgres psql -v ON_ERROR_STOP=1 -U $dbUser -d $dbName | Out-Null
    if ($LASTEXITCODE -ne 0) { throw "Không seed được incident E2E v3.18.0 cho browser CI." }
  ...
  Write-Host "Runtime E2E v3.18.0 PASS ✅" -ForegroundColor Green
