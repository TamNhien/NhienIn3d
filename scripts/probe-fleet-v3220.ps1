...
try {
  ...
  function Invoke-ProbeFleetCycle {
    ...
    Write-Host "[Probe Fleet v3.22.0] cycle $($cycleStarted.ToString('yyyy-MM-ddTHH:mm:ssK')) · $($selected.Count) agent(s)" -ForegroundColor Cyan
    ...
    foreach ($prop in $selected) {
      ...
      & node (Join-Path $PSScriptRoot "probe-agent-v3220.mjs") --once
    ...
    if ($failures.Count -eq 0) {
      Write-Host "Probe Fleet v3.22.0 cycle PASS ✅" -ForegroundColor Green
      return $true
    }

    Write-Warning "Probe Fleet v3.22.0 cycle có $($failures.Count) lỗi: $($failures -join '; ')"
    return $false
  }

  if ($Once) {
    if (-not (Invoke-ProbeFleetCycle)) { throw "Probe Fleet v3.22.0 one-shot thất bại." }
    Write-Host "Probe Fleet v3.22.0 PASS ✅" -ForegroundColor Green
    return
  }

  Write-Host "[Probe Fleet v3.22.0] Managed mode: heartbeat mỗi $IntervalSeconds giây. Nhấn Ctrl+C để dừng." -ForegroundColor Cyan
  while ($true) {
