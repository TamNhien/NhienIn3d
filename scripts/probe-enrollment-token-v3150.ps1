...
try {
  ...
  $loginBody = @{ thu_dien_tu = $email; mat_khau = $password; trinh_duyet_hien_thi = "Probe Enrollment v3.15.0" } | ConvertTo-Json
  ...
  $headers = @{ Cookie = "nhienin3d_phien=$($accessMatch.Groups[1].Value)" }
  $body = @{ agent_id = $AgentId; region = $Region; node_name = $NodeName; expires_minutes = [Math]::Max(5, [Math]::Min(1440, $ExpiresMinutes)) } | ConvertTo-Json
  $result = Invoke-RestMethod -Uri "$base/quan-tri/he-thong/ops/probe-enrollment-token" -Method Post -Headers $headers -ContentType "application/json; charset=utf-8" -Body $body -TimeoutSec 20
  Write-Host "[Probe Enrollment v3.15.0] Token tạo thành công cho $AgentId@$Region/$NodeName" -ForegroundColor Green
