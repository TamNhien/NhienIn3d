...
try {
  Write-Host "[Recovery drill v3.24.0] Isolated logical restore + RPO/RTO readiness..." -ForegroundColor Cyan
  ...
  $path = Join-Path $backupDir "recovery-drill-v3240-latest.json"
  $report | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath $path -Encoding UTF8
  Write-Host "Recovery drill v3.24.0 PASS" -ForegroundColor Green
