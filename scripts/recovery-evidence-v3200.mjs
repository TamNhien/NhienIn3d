...
const logical = await readJson("recovery-drill-v3200-latest.json") || await readJson("recovery-drill-v3190-latest.json") || await readJson("recovery-drill-v3180-latest.json");
const pitr = await readJson("recovery-pitr-v3200-latest.json") || await readJson("recovery-pitr-v3190-latest.json") || await readJson("recovery-pitr-v3180-latest.json");
const old = await readJson("recovery-evidence-v3200.json") || await readJson("recovery-evidence-v3190.json") || { history: [] };
const entry = {
...
const evidence = { version: "3.20.0", retention_runs: 30, history };
const evidenceRaw = JSON.stringify(evidence, null, 2) + "\n";
const sha256 = createHash("sha256").update(evidenceRaw).digest("hex");
await writeFile(join(dir, "recovery-evidence-v3200.json"), evidenceRaw, "utf8");
await writeFile(join(dir, "recovery-evidence-v3200.sha256"), `${sha256}  recovery-evidence-v3200.json\n`, "utf8");
...
if (privateB64) {
  ...
  signature = {
    ...
    public_key_fingerprint_sha256: createHash("sha256").update(publicPem).digest("hex"),
  };
}
const bundle = {
  manifest: { version: "3.20.0", created_at: new Date().toISOString(), evidence_file: "recovery-evidence-v3200.json", retention_runs: 30 },
  evidence,
  integrity: { algorithm: "SHA-256", sha256 },
  signature,
  security: { private_key_embedded: false, remote_code_execution: false },
};
await writeFile(join(dir, "recovery-evidence-bundle-v3200.json"), JSON.stringify(bundle, null, 2) + "\n", "utf8");
