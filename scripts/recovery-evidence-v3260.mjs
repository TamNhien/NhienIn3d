...
const logical = await readJson("recovery-drill-v3260-latest.json") || await readJson("recovery-drill-v3220-latest.json") || await readJson("recovery-drill-v3210-latest.json") || await readJson("recovery-drill-v3200-latest.json");
const pitr = await readJson("recovery-pitr-v3260-latest.json") || await readJson("recovery-pitr-v3220-latest.json") || await readJson("recovery-pitr-v3210-latest.json") || await readJson("recovery-pitr-v3200-latest.json");
const old = await readJson("recovery-evidence-v3260.json") || await readJson("recovery-evidence-v3220.json") || await readJson("recovery-evidence-v3210.json") || await readJson("recovery-evidence-v3200.json") || { history: [] };
const entry = {
...
const evidence = { version: "3.26.0", retention_runs: 30, history };
const evidenceRaw = JSON.stringify(evidence, null, 2) + "\n";
const sha256 = createHash("sha256").update(evidenceRaw).digest("hex");
await writeFile(join(dir, "recovery-evidence-v3260.json"), evidenceRaw, "utf8");
await writeFile(join(dir, "recovery-evidence-v3260.sha256"), `${sha256}  recovery-evidence-v3260.json\n`, "utf8");
...
const bundle = {
  manifest: { version: "3.26.0", created_at: new Date().toISOString(), evidence_file: "recovery-evidence-v3260.json", retention_runs: 30, verification_required_before_export: true, external_trust_anchor_required_for_signed_bundle: true, revoked_key_fail_closed: true },
  evidence,
...
await writeFile(join(dir, "recovery-evidence-bundle-v3260.json"), JSON.stringify(bundle, null, 2) + "\n", "utf8");
