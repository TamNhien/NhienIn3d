...
const logical = await readJson("recovery-drill-v3220-latest.json") || await readJson("recovery-drill-v3210-latest.json") || await readJson("recovery-drill-v3200-latest.json");
const pitr = await readJson("recovery-pitr-v3220-latest.json") || await readJson("recovery-pitr-v3210-latest.json") || await readJson("recovery-pitr-v3200-latest.json");
const old = await readJson("recovery-evidence-v3220.json") || await readJson("recovery-evidence-v3210.json") || await readJson("recovery-evidence-v3200.json") || { history: [] };
const entry = {
...
const evidence = { version: "3.22.0", retention_runs: 30, history };
const evidenceRaw = JSON.stringify(evidence, null, 2) + "\n";
const sha256 = createHash("sha256").update(evidenceRaw).digest("hex");
await writeFile(join(dir, "recovery-evidence-v3220.json"), evidenceRaw, "utf8");
await writeFile(join(dir, "recovery-evidence-v3220.sha256"), `${sha256}  recovery-evidence-v3220.json\n`, "utf8");
...
const bundle = {
  manifest: { version: "3.22.0", created_at: new Date().toISOString(), evidence_file: "recovery-evidence-v3220.json", retention_runs: 30, verification_required_before_export: true, external_trust_anchor_required_for_signed_bundle: true },
  evidence,
  integrity: { algorithm: "SHA-256", sha256 },
  signature,
  trust: { embedded_public_key_is_not_trust_anchor: true, trusted_key_source: "environment/current-signing-key" },
  security: { private_key_embedded: false, remote_code_execution: false },
};
await writeFile(join(dir, "recovery-evidence-bundle-v3220.json"), JSON.stringify(bundle, null, 2) + "\n", "utf8");
