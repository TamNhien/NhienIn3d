...
try {
  Write-Host "[NhienIn3d v3.15.0] Verification" -ForegroundColor Cyan
  ...
  if ($Full) {
    ...
    & (Join-Path $PSScriptRoot "e2e-runtime-v3150.ps1")
    if ($LASTEXITCODE -ne 0) { throw "Runtime E2E v3.15.0 thất bại." }
    Run-Step "Playwright Chromium" { npx playwright install chromium }
    Run-Step "Browser E2E" { npm run e2e:browser }
  }

  Write-Host "`nNhienIn3d v3.15.0 verification PASS ✅" -ForegroundColor Green
