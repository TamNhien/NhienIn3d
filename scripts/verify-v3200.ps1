...
try {
  Write-Host "[NhienIn3d v3.20.0] Verification" -ForegroundColor Cyan
  ...
  if ($Full) {
    ...
    & (Join-Path $PSScriptRoot "e2e-runtime-v3200.ps1")
    if ($LASTEXITCODE -ne 0) { throw "Runtime E2E v3.20.0 thất bại." }
    Write-Host "`n=== Recovery drill RPO/RTO ===" -ForegroundColor Cyan
    & (Join-Path $PSScriptRoot "recovery-drill-v3200.ps1")
    if ($LASTEXITCODE -ne 0) { throw "Recovery drill v3.20.0 thất bại." }
    Write-Host "`n=== Target-time PITR drill (opt-in) ===" -ForegroundColor Cyan
    & (Join-Path $PSScriptRoot "recovery-pitr-drill-v3200.ps1")
    if ($LASTEXITCODE -ne 0) { throw "Target-time PITR drill v3.20.0 thất bại." }
    Run-Step "Recovery evidence history" { node scripts/recovery-evidence-v3200.mjs }
    Run-Step "Playwright Chromium" { npx playwright install chromium }
    Run-Step "Browser E2E" { npm run e2e:browser }
  }

  Write-Host "`nNhienIn3d v3.20.0 verification PASS ✅" -ForegroundColor Green
