...
if (!password) throw new Error("Thiếu ADMIN_PASSWORD trong biến môi trường hoặc file .env để chạy browser E2E v3.7.2");
...
try {
  ...
  if (health.phien_ban !== "v3.7.2") throw new Error(`API đang chạy ${health.phien_ban || "không rõ version"}, không phải v3.7.2. Docker có thể vẫn dùng container/image cũ.`);
  ...
  if (mutateIncident) {
    ...
    if (!synthetic) throw new Error("Không tìm thấy synthetic incident v3.7.2 do runtime E2E seed cho browser CI.");
    ...
    await openSynthetic();
    await page.getByLabel("Ghi chú xử lý / khắc phục", { exact: true }).fill("Browser E2E v3.7.2 acknowledge persistence");
  ...
  console.log("Browser E2E v3.7.2 PASS ✅");
  ...
  console.log("Ops Dashboard v3.7.2             : PASS");
} finally {
  if (originalSlo) {
    try {
