...
try {
  if (-not $KhongKiemTraBackupRestore) {
    Write-Host "[E2E v3.4.1] Backup/SHA/restore runtime nền..." -ForegroundColor Cyan
  ...
  Write-Host "[E2E v3.4.1] Đăng nhập Admin qua cookie session..." -ForegroundColor Cyan
  $session = New-Object Microsoft.PowerShell.Commands.WebRequestSession
  $loginBody = @{ thu_dien_tu = $adminEmail; mat_khau = $adminPassword; trinh_duyet_hien_thi = "NhienIn3d E2E v3.4.1" } | ConvertTo-Json
  ...
  Write-Host "[E2E v3.4.1] Kiểm tra đơn hàng + sản phẩm..."
  ...
  Write-Host "[E2E v3.4.1] Preview nhập kho CSV, không thay đổi tồn kho..."
  $variant = $null
  ...
  $csv = "ma_bien_the,so_luong_nhap,ly_do`r`n$($variant.ma_bien_the),1,E2E preview v3.4.1`r`n"
  $base64 = [Convert]::ToBase64String([Text.Encoding]::UTF8.GetBytes($csv))
  $previewBody = @{ ten_file = "e2e-v341.csv"; du_lieu_base64 = $base64 } | ConvertTo-Json
  ...
  Write-Host "[E2E v3.4.1] Kiểm tra báo cáo Excel + phiếu nhập..."
  ...
  Write-Host "[E2E v3.4.1] Kiểm tra Ops v3.4: config, SLA, incident, cursor..."
  ...
  Assert-True ($health.phien_ban -eq "3.4.1") "Health endpoint chưa lên v3.4.1."
  ...
  Write-Host "Runtime E2E v3.4.1 PASS ✅" -ForegroundColor Green
