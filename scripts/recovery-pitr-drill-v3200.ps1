...
try {
  ...
  if (@("1","true","yes","on") -notcontains $enabled) {
    Write-Host "[PITR drill v3.20.0] SKIP - SYSTEM_DB_PITR_DRILL_ENABLED=false" -ForegroundColor DarkGray
    exit 0
  }
  ...
  Write-Host "[PITR drill v3.20.0] Tạo database cô lập + base backup..." -ForegroundColor Cyan
  ...
  $path = Join-Path $backupDir "recovery-pitr-v3200-latest.json"
  $report | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath $path -Encoding UTF8
  Write-Host "Target-time PITR v3.20.0 PASS ✅" -ForegroundColor Green
