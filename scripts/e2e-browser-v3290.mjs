...
if (!password) throw new Error("Thiếu ADMIN_PASSWORD trong biến môi trường hoặc file .env để chạy browser E2E v3.29.0");
...
try {
  ...
  if (health.phien_ban !== "v3.29.0") throw new Error(`API đang chạy ${health.phien_ban || "không rõ version"}, không phải v3.29.0. Docker có thể vẫn dùng container/image cũ.`);
  ...
  await page.getByText("Lợi nhuận gộp 30 ngày", { exact: true }).waitFor({ timeout: 30_000 });
  await page.getByRole("button", { name: "Excel lợi nhuận", exact: true }).waitFor();

  await page.getByRole("button", { name: "Đơn hàng", exact: true }).click();
  await page.getByRole("heading", { name: "Quản trị đơn hàng", exact: true }).waitFor({ timeout: 30_000 });
  await page.getByRole("button", { name: "Excel đổi/trả", exact: true }).waitFor();
  const firstOrder = page.locator(".cine-order-admin-item-v212").first();
  if (await firstOrder.count()) {
    await firstOrder.click().catch(() => {});
    const rmaHeading = page.getByRole("heading", { name: "Đổi / trả hàng (RMA)", exact: true });
    if (await rmaHeading.count()) await rmaHeading.waitFor({ timeout: 30_000 });
  }

  const rmaResponse = await context.request.get(`${base}/api/v1/quan-tri/doi-tra`);
  if (!rmaResponse.ok()) throw new Error(`RMA API trả ${rmaResponse.status()}.`);
  const rmaContract = await rmaResponse.json();
  if (rmaContract.phien_ban !== "3.29.0" || rmaContract.partial_refund_ledger !== true || rmaContract.auto_gateway_refund !== false) throw new Error("RMA/partial refund v3.29.0 chưa đúng fail-safe contract.");
  ...
  if (refundQueue.phien_ban !== "3.29.0" || refundQueue.gateway_auto_refund !== false || refundQueue.manual_confirmation_required !== true) throw new Error("Refund reconciliation v3.29.0 chưa fail-safe/manual đúng contract.");
  ...
  // v3.29.0 hotfix: Kho có 2 nút "Tải CSV mẫu" (kiểm kê và nhập kho theo lô).
  ...
  if (mutateIncident) {
    ...
    if (!synthetic) throw new Error("Không tìm thấy synthetic incident v3.29.0 do runtime E2E seed cho browser CI.");
    const openSynthetic = async () => {
      // v3.29.0 CI hotfix: các bước kiểm tra Kho/Xếp ca đổi tab trước khi mutation incident.
    ...
    await openSynthetic();
    await page.getByLabel("Ghi chú xử lý / khắc phục", { exact: true }).fill("Browser E2E v3.29.0 acknowledge persistence");
  ...
  if (pmBadgeStyle.height > 24 || pmBadgeStyle.justify !== "center" || pmBadgeStyle.align !== "center" || pmBadgeStyle.text !== "center") throw new Error("Badge COMPLETE/DRAFT chưa được thu nhỏ và canh giữa v3.29.0.");
  await page.getByRole("heading", { name: "Managed probe fleet", exact: true }).waitFor();
  await page.getByRole("heading", { name: "Multi-region quorum · anomaly detection", exact: true }).waitFor();
  ...
  if (addWindowBackground === "rgb(232, 238, 252)") throw new Error("Nút Thêm window vẫn dùng nền sáng legacy thay vì dark action v3.29.0.");
  ...
  console.log("RMA / partial refund UI+contract : PASS");
  console.log("Gross margin dashboard UI        : PASS");
  console.log("Shift overlap safety UI        : PASS");
  console.log("Browser E2E v3.29.0 PASS ✅");
  ...
  console.log("Ops Dashboard v3.29.0           : PASS");
} finally {
  if (originalSlo) {
    try {
