...
const logical = await readJson("recovery-drill-v3241-latest.json") || await readJson("recovery-drill-v3220-latest.json") || await readJson("recovery-drill-v3210-latest.json") || await readJson("recovery-drill-v3200-latest.json");
const pitr = await readJson("recovery-pitr-v3241-latest.json") || await readJson("recovery-pitr-v3220-latest.json") || await readJson("recovery-pitr-v3210-latest.json") || await readJson("recovery-pitr-v3200-latest.json");
const old = await readJson("recovery-evidence-v3241.json") || await readJson("recovery-evidence-v3220.json") || await readJson("recovery-evidence-v3210.json") || await readJson("recovery-evidence-v3200.json") || { history: [] };
const entry = {
...
const evidence = { version: "3.24.1", retention_runs: 30, history };
const evidenceRaw = JSON.stringify(evidence, null, 2) + "\n";
const sha256 = createHash("sha256").update(evidenceRaw).digest("hex");
await writeFile(join(dir, "recovery-evidence-v3241.json"), evidenceRaw, "utf8");
await writeFile(join(dir, "recovery-evidence-v3241.sha256"), `${sha256}  recovery-evidence-v3241.json\n`, "utf8");
...
const bundle = {
  manifest: { version: "3.24.1", created_at: new Date().toISOString(), evidence_file: "recovery-evidence-v3241.json", retention_runs: 30, verification_required_before_export: true, external_trust_anchor_required_for_signed_bundle: true, revoked_key_fail_closed: true },
  evidence,
...
await writeFile(join(dir, "recovery-evidence-bundle-v3241.json"), JSON.stringify(bundle, null, 2) + "\n", "utf8");
