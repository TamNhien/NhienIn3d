...
try {
  Write-Host "[Recovery drill v3.29.0] Isolated logical restore + RPO/RTO readiness..." -ForegroundColor Cyan
  ...
  $path = Join-Path $backupDir "recovery-drill-v3290-latest.json"
  $report | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath $path -Encoding UTF8
  Write-Host "Recovery drill v3.29.0 PASS" -ForegroundColor Green
